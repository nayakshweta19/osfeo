...
while {$j < $nH} {
  ...
  element quad3D [expr $j*$nL+$i+1] [expr $j*($nL+1)+$i+1] [expr $j*($nL+1)+$i+2] [expr ($j+1)*($nL+1)+$i+2] [expr ($j+1)*($nL+1)+$i+1]  $t "PlaneStress" 15 +0.0  +0.0  +0.0 +0.0 
  
  # element shell02 eleID node1 node2 node3 node4 secTag
...
# perform the analysis
# analyze 17380
#                  1     -1    +2    -2     +4    -4   +8     -8   12.5   -12.5    17.1   -17.6   -7.6    -7.7
set numSteps   { 100     400  300    400  600    800 1200   1600   2050    2500    2960   3470   1000      10}
set numIters   { 100     100  100    100  100    100    0    100    100     300     600    200    100     100}
