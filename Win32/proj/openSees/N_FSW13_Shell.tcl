...
# Section "Column&t":    secTag    matTag    string ...    -section secTag 
section  Aggregator       4  3 T  -section 1
# Section "Beam&t":    secTag    matTag    string ...    -section secTag 
section  Aggregator       5  3 T  -section 2
# -------------------------------------------------------
...
while {$j < [expr $nH]} {
	...
	element $beamType [expr $eleStartID+$j*2] [expr $nodeStartID+$j*2] [expr $nodeStartID+$j*2+2] $np $columnSec 1 -iter $iterNum $iterTol
	element $beamType [expr $eleStartID+$j*2+1] [expr $nodeStartID+$j*2+1] [expr $nodeStartID+$j*2+3] $np $columnSec 1 -iter $iterNum $iterTol
    } else {
	element $beamType [expr $eleStartID+$j*2] [expr $nodeStartID+$j*2] [expr $nodeStartID+$j*2+2] $np $columnSec 1 -iter $iterNum $iterTol
	element $beamType [expr $eleStartID+$j*2+1] [expr $nodeStartID+$j*2+1] [expr $nodeStartID+$nH*2+$nL] $np $columnSec 1 -iter $iterNum $iterTol
    }
    set j [expr $j+1]
}
...
while {$i < [expr $nL]} {
    ...
    element $beamType [expr $eleStartID+$j*2+$i] [expr $nodeStartID+$j*2+$i] [expr $nodeStartID+$j*2+1+$i] $np $beamSec 2 -iter $iterNum $iterTol
    #   element  elasticBeamColumn [expr $eleStartID+$j*2+$i] [expr $nodeStartID+$j*2+$i] [expr $nodeStartID+$j*2+1+$i] $bA $bE $bI 2
 
    set i [expr $i+1]
}  
 
element $beamType 19 17 27 $np $beamSec 2 -iter $iterNum $iterTol
element $beamType 20 27 28 $np $beamSec 2 -iter $iterNum $iterTol
element $beamType 21 28 18 $np $beamSec 2 -iter $iterNum $iterTol
...
equalDOF 1  17 1 2 3 4 5 6
equalDOF 4  18 1 2 3 4 5 6
equalDOF 5  19 1 2 3 4 5 6
equalDOF 8  20 1 2 3 4 5 6
equalDOF 9  21 1 2 3 4 5 6
equalDOF 12 22 1 2 3 4 5 6
equalDOF 13 23 1 2 3 4 5 6
equalDOF 14 24 1 2 3 4 5 6
equalDOF 15 25 1 2 3 4 5 6
equalDOF 16 26 1 2 3 4 5 6

equalDOF 2  27 1 2 3 4 5 6
equalDOF 3  28 1 2 3 4 5 6
...
while {$j < $nH} {
	...
	element Shell02 [expr $j*$nL+$i+1] [expr $j*($nL+1)+$i+1] [expr $j*($nL+1)+$i+2] [expr ($j+1)*($nL+1)+$i+2] [expr ($j+1)*($nL+1)+$i+1] 6
...
pattern Plain 1 "Linear" {
    ...
    load 13 0 [expr  -$N]  0  0 0 0 
    load 16 0 [expr  -$N]  0  0 0 0 
}
...
pattern Plain 2 "Linear" {
    ...
    # Create the nodal load - command: load nodeID xForce yForce
    load 13 [expr   $P/2] 0 0 0 0 0
    load 16 [expr   $P/2] 0 0 0 0 0
    load 4  [expr  -$P/2] 0 0 0 0 0 
}
...
set numIters   { 100      0    0     0    0   100    0   300   300   300  500   500  500   300}
