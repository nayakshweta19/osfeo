...
// $Source: /usr/local/cvs/OpenSees/SRC/element/shell/Shell02.cpp,v $
...
Vector  Shell02::resid(24) ;
Matrix  Shell02::mass(24,24) ;

//initialize pointers to zero using initialization list 
Matrix**  Shell02::GammaB1pointer = 0 ;
Matrix**  Shell02::GammaD1pointer = 0 ;
Matrix**  Shell02::GammaA2pointer = 0 ;
Matrix**  Shell02::GammaC2pointer = 0 ; 
Matrix**  Shell02::Bhat           = 0 ;
    
//quadrature data
const double  Shell02::root3 = sqrt(3.0) ;
const double  Shell02::one_over_root3 = 1.0 / root3 ;
...
const double  Shell02::wg[] = { 1.0, 1.0, 1.0, 1.0 } ;
*/
  

//null constructor
Shell02::Shell02( ) :
Element( 0, ELE_TAG_Shell02 ),
...
//set domain
void  Shell02::setDomain( Domain *theDomain ) 
{  
  int i, j ;
  static Vector eig(3) ;
...
int  Shell02::getNumExternalNodes( ) const
{
  return 4 ;
} 
...
const ID&  Shell02::getExternalNodes( ) 
{
...
Node **
Shell02::getNodePtrs(void) 
{
  return nodePointers;
} 

//return number of dofs
int  Shell02::getNumDOF( ) 
{
  return 24 ;
}
...
//commit state
int  Shell02::commitState( )
{
  int success = 0 ;

  // call element commitState to do any base class stuff
  if ((success = this->Element::commitState()) != 0) {
    opserr << "Shell02::commitState () - failed in base class";
  }    
...
int  Shell02::revertToLastCommit( ) 
{
  int i ;
  int success = 0 ;
...
Shell02::setResponse(const char **argv, int argc, OPS_Stream &output)
{
  ...
  if (strcmp(argv[0],"force") == 0 || strcmp(argv[0],"forces") == 0 ||
    ...
    if (argc < 2) {
      opserr << "Shell02::setResponse() - need to specify more data\n";
      return 0;
    }
...
int
Shell02::getResponse(int responseID, Information &eleInfo)
{
  int cnt = 0;
...
const Matrix&  Shell02::getTangentStiff( ) 
{
...
const Matrix&  Shell02::getInitialStiff( ) 
{
  if (Ki != 0)
    return *Ki;
...
int 
Shell02::addLoad(ElementalLoad *theLoad, double loadFactor)
{
  opserr << "Shell02::addLoad - load type unknown for ele with tag: " << this->getTag() << endln;
  return -1;
}
...
int 
Shell02::addInertiaLoadToUnbalance(const Vector &accel)
{
  int tangFlag = 1 ;
...
void   
Shell02::formInertiaTerms( int tangFlag ) 
{
...
void  
Shell02::formResidAndTangent( int tang_flag ) 
{
  //
...
double*
Shell02::computeBdrill( int node, const double shp[3][4] )
{
...
void   
Shell02::computeGamma( const double xl[2][4], const Matrix &J )
{
...
const Matrix&  
Shell02::assembleB( const Matrix &Bmembrane,
                               const Matrix &Bbend, 
                               const Matrix &Bshear ) 
{
...
const Matrix&   
Shell02::computeBmembrane( int node, const double shp[3][4] ) 
{
...
const Matrix&   
Shell02::computeBbend( int node, const double shp[3][4] )
{
...
const Matrix&  
Shell02::computeBshear( int node, const double shp[3][4] )
{
...
int  Shell02::sendSelf (int commitTag, Channel &theChannel)
{
  int res = 0;
  ...
  if (res < 0) {
    opserr << "WARNING Shell02::sendSelf() - " << this->getTag() << " failed to send ID\n";
    return res;
  }
  ...
  for (i = 0; i < 4; i++) {
    ...
    if (res < 0) {
      opserr << "WARNING Shell02::sendSelf() - " << this->getTag() << " failed to send its Material\n";
      return res;
    }
  }
		       ...
		       FEM_ObjectBroker &theBroker)
{
  ...
  if (res < 0) {
    opserr << "WARNING Shell02::recvSelf() - " << this->getTag() << " failed to receive ID\n";
    return res;
  }
