...
// Created: 09/09
// Modified by: Li Ning 
// Description: This file contains the class implementation of Timoshenko3d04.Based on Timoshenko3d04.cpp.
...
Matrix Timoshenko3d04::K(12,12);
Vector Timoshenko3d04::P(12);
double Timoshenko3d04::workArea[200];
...
Timoshenko3d04::Timoshenko3d04(int tag, 
					 ...
					 BeamIntegration& bi,
					 double r)
    :Element (tag, ELE_TAG_Timoshenko3d04), 
	...
	Q(12), q(6), rho(r), parameterID(0)
{
  ...
  if (theSections == 0) {
    opserr << "Timoshenko3d04::Timoshenko3d04 - failed to allocate section model pointer\n";
    exit(-1);
  }
  ...
  if (beamInt == 0) {
	opserr << "Timoshenko3d04::Timoshenko3d04() - failed to copy beam integration\n";
	exit(-1);
  }
...
Timoshenko3d04::Timoshenko3d04()
	:Element (0, ELE_TAG_Timoshenko3d04),
...
int
Timoshenko3d04::getNumExternalNodes() const
{
    return 2;
}

const ID&
Timoshenko3d04::getExternalNodes()
{
    return connectedExternalNodes;
}

Node **
Timoshenko3d04::getNodePtrs()
{
    return theNodes;
}

int
Timoshenko3d04::getNumDOF()
{
    return 12;
}

void
Timoshenko3d04::setDomain(Domain *theDomain)
{
...
int
Timoshenko3d04::commitState()
{
    int retVal = 0;

    // call element commitState to do any base class stuff
    if ((retVal = this->Element::commitState()) != 0) {
      opserr << "Timoshenko3d04::commitState () - failed in base class";
    }    
...
int
Timoshenko3d04::revertToLastCommit()
{
    int retVal = 0;
...
const Matrix&
Timoshenko3d04::getTangentStiff(void)
{
...
const Matrix&
Timoshenko3d04::getInitialBasicStiff()
{
  static Matrix kb(6,6);
...
const Matrix&
Timoshenko3d04::getInitialStiff()
{
...
int 
Timoshenko3d04::addLoad(ElementalLoad *theLoad, double loadFactor)
{
  int type;
  ...
  if (type == LOAD_TAG_Beam3dUniformLoad) {
    ...
    opserr << "Timoshenko3d04::Timoshenko3d04 -- load type unknown for element with tag: "
	   << this->getTag() << "Timoshenko3d04::addLoad()\n"; 
...
int 
Timoshenko3d04::addInertiaLoadToUnbalance(const Vector &accel)
{
...
const Vector&
Timoshenko3d04::getResistingForce()
{
...
const Vector&
Timoshenko3d04::getResistingForceIncInertia()
{
...
int
Timoshenko3d04::sendSelf(int commitTag, Channel &theChannel)
{
  ...
  if (theChannel.sendID(dbTag, commitTag, idData) < 0) {
    opserr << "Timoshenko3d04::sendSelf() - failed to send ID data\n";
     return -1;
  }    
  ...
  if (crdTransf->sendSelf(commitTag, theChannel) < 0) {
     opserr << "Timoshenko3d04::sendSelf() - failed to send crdTranf\n";
     return -1;
  }      
  ...
  for (j = 0; j<numSections; j++) {
    if (theSections[j]->sendSelf(commitTag, theChannel) < 0) {
      opserr << "Timoshenko3d04::sendSelf() - section " << 
...
int
Timoshenko3d04::recvSelf(int commitTag, Channel &theChannel,
			   FEM_ObjectBroker &theBroker)
{
  ...
  if (theChannel.recvID(dbTag, commitTag, idData) < 0)  {
    opserr << "Timoshenko3d04::recvSelf() - failed to recv ID data\n";
    return -1;
  }    
  ...
  if (crdTransf == 0 || crdTransf->getClassTag() != crdTransfClassTag) {
      ...
      if (crdTransf == 0) {
	opserr << "Timoshenko3d04::recvSelf() - failed to obtain a CrdTrans object with classTag " <<
  ...
  if (crdTransf->recvSelf(commitTag, theChannel, theBroker) < 0) {
    opserr << "Timoshenko3d04::sendSelf() - failed to recv crdTranf\n";
    return -3;
  }      
  ...
  if (numSections != idData(3)) {
    ...
    if (theSections == 0) {
      opserr << "Timoshenko3d04::recvSelf() - out of memory creating sections array of size " <<
      idData(3) << endln;
      return -1;
    }    
    ...
    for (i=0; i<numSections; i++) {
	  ...
	  if (theSections[i] == 0) {
		opserr << "Timoshenko3d04::recvSelf() Broker could not create Section of class type" <<
		sectClassTag << endln;
	    exit(-1);
      }
	...
	opserr << "Timoshenko3d04::recvSelf() - section " << i << " failed to recv itself\n";
	return -1;
      }     
    }
...
void
Timoshenko3d04::Print(OPS_Stream &s, int flag)		
{
  s << "\nTimoshenko3d04, element id:  " << this->getTag() << endln;
...
int
Timoshenko3d04::displaySelf(Renderer &theViewer, int displayMode, float fact)
{
...
Response*
Timoshenko3d04::setResponse(const char **argv, int argc, OPS_Stream &s)
{
...
int 
Timoshenko3d04::getResponse(int responseID, Information &eleInfo)		//LN modify
{
  double N, V, M1, M2, T;
  double L = crdTransf->getInitialLength();
  double oneOverL = 1.0/L;
...
Matrix
Timoshenko3d04::getBd(int sec, const Vector &v, double L)
{
  double pts[maxNumSections];
  beamInt->getSectionLocations(numSections, L, pts);
  ...
  Bd(2,2) = phi3; 
  Bd(2,4) = phi4; 
  Bd(3,1) = phi1p-phi3; // shear components
  Bd(3,3) = phi2p-phi4; // shear components
  Bd(4,2) = phi1p-phi3; // shear components 
  Bd(4,4) = phi2p-phi4; // shear components 
...
const Matrix &
Timoshenko3d04::getKiSensitivity(int gradNumber)
{
	K.Zero();
	return K;
}
...
const Matrix &
Timoshenko3d04::getMassSensitivity(int gradNumber)
{
	K.Zero();
	return K;
}
...
const Vector &
Timoshenko3d04::getResistingForceSensitivity(int gradNumber)
{
...
// NEW METHOD
int
Timoshenko3d04::commitSensitivity(int gradNumber, int numGrads)
{
 	return 0;
}
