...
// $Source: /usr/local/cvs/OpenSees/SRC/element/Timoshenko/TimoshenkoLinearCrdTransf2d.cpp,v $
...
// TimoshenkoLinearCrdTransf2d class. TimoshenkoLinearCrdTransf2d is a linear
...
TimoshenkoLinearCrdTransf2d::TimoshenkoLinearCrdTransf2d(int tag):
  CrdTransf(tag, CRDTR_TAG_TimoshenkoLinearCrdTransf2d),
  ...
  cosTheta(0), sinTheta(0), L(0)
{
	...
	if (&rigJntOffset1 == 0 || rigJntOffset1.Size() != 2 ) {
		opserr << "TimoshenkoLinearCrdTransf2d::TimoshenkoLinearCrdTransf2d:  Invalid rigid joint offset vector for node I\n";
   ...
   // check rigid joint offset for node J
	if (&rigJntOffset2 == 0 || rigJntOffset2.Size() != 2 ) {
		opserr << "TimoshenkoLinearCrdTransf2d::TimoshenkoLinearCrdTransf2d:  Invalid rigid joint offset vector for node J\n";
		opserr << "Size must be 2\n";      
	}
...
TimoshenkoLinearCrdTransf2d::TimoshenkoLinearCrdTransf2d():
  CrdTransf(0, CRDTR_TAG_TimoshenkoLinearCrdTransf2d),
  nodeIPtr(0), nodeJPtr(0),
  nodeIOffset(0), nodeJOffset(0),
  cosTheta(0), sinTheta(0), L(0)
{
...
{       
   ...
   {
      opserr << "\nTimoshenkoLinearCrdTransf2d::initialize";
...
TimoshenkoLinearCrdTransf2d::computeElemtLengthAndOrient()
{
   ...
   if (L == 0.0) 
   {
      opserr << "\nTimoshenkoLinearCrdTransf2d::computeElemtLengthAndOrien: 0 length\n";
      return -2;  
   }
...
const Matrix &
TimoshenkoLinearCrdTransf2d::getGlobalStiffMatrix (const Matrix &kb, const Vector &pb)
{
...
{
  // create a new instance of TimoshenkoLinearCrdTransf2d 

  TimoshenkoLinearCrdTransf2d *theCopy;
  ...
  theCopy = new TimoshenkoLinearCrdTransf2d(this->getTag(), offsetI, offsetJ);
...
CrdTransf *
TimoshenkoLinearCrdTransf2d::getCopy2d(void)		
{
  // create a new instance of TimoshenkoLinearCrdTransf2d 
...
int 
TimoshenkoLinearCrdTransf2d::sendSelf(int cTag, Channel &theChannel)	
{
  int res = 0;
  ...
  if (res < 0) {
    opserr << "TimoshenkoLinearCrdTransf2d::sendSelf - failed to send Vector\n";
    return res;
  }
...
TimoshenkoLinearCrdTransf2d::recvSelf(int cTag, Channel &theChannel, FEM_ObjectBroker &theBroker)	
{
  ...
  if (res < 0) {
    opserr << "TimoshenkoLinearCrdTransf2d::recvSelf - failed to receive Vector\n";
    return res;
  }
...
void
TimoshenkoLinearCrdTransf2d::Print(OPS_Stream &s, int flag)
{
   s << "\nCrdTransf: " << this->getTag() << " Type: TimoshenkoLinearCrdTransf2d";
   if (nodeIOffset != 0)
...
const Vector &
TimoshenkoLinearCrdTransf2d::getBasicDisplSensitivity(int gradNumber)
{
...
const Vector &
TimoshenkoLinearCrdTransf2d::getBasicTrialVel(void)
{
