// $Source: /usr/local/cvs/OpenSees/SRC/element/Timoshenko/FASTMFiberSection2d.cpp,v $
// $Revision: 1.1 $
// $Date: 2009/01/10 21:22:20 $

// Created: 09/09
// Modified by: Li Ning 
// Description: This file contains the class implementation of FASTMFiberSection2d.Based on FiberSection2d.cpp.
...
ID FASTMFiberSection2d::code(3);

// constructors:
FASTMFiberSection2d::FASTMFiberSection2d(int tag, int num, Fiber **fibers, int Hnum, Fiber **Hfibers, int strip1, double t1, int strip2, double t2, int strip3, double t3): 
  SectionForceDeformation(tag, SEC_TAG_FASTMFiberSection2d),
  ...
  iterFile(0),exf(0),e1f(0),e2f(0),eyf(0),sxf(0),s1f(0),s2f(0),syf(0)
{
  ...
  if (numFibers != 0) {
    ...
    if (theMaterials1 == 0) {
      opserr << "FASTMFiberSection2d::FASTMFiberSection2d -- failed to allocate Material pointers";
      exit(-1);
    }
    ...
    if (matData == 0) {
      opserr << "FASTMFiberSection2d::FASTMFiberSection2d -- failed to allocate double array for material data\n";
      exit(-1);
    }
	...
	opserr << "FASTMFiberSection2d::FASTMFiberSection2d -- failed to get copy of a Material\n";
	exit(-1);
      }
  ...
  if (numHFibers != 0) {
    ...
    if (theHMaterials == 0) {
      opserr << "FASTMFiberSection2d::FASTMFiberSection2d -- failed to allocate HMaterial pointers";
      exit(-1);
    }
    ...
    if (matHData == 0) {
      opserr << "FASTMFiberSection2d::FASTMFiberSection2d -- failed to allocate double array for Hmaterial data\n";
      exit(-1);
    }
	...
	if (theHMaterials[i*numHFibers + jj] == 0) {
	  opserr << "FASTMFiberSection2d::FASTMFiberSection2d -- failed to get copy of a HMaterial\n";
	  exit(-1);
	}
      }
    }    
...
FASTMFiberSection2d::FASTMFiberSection2d():
  SectionForceDeformation(0, SEC_TAG_FASTMFiberSection2d),
...
int
FASTMFiberSection2d::addFiber(Fiber &newFiber)		
{
  ...
  if (newArray1[numFibers] == 0) {
    opserr <<"FASTMFiberSection2d::addFiber -- failed to get copy of a Material\n";
    delete [] newMatData;
    return -1;
  }
...
int FASTMFiberSection2d::setTrialSectionDeformation (const Vector &deforms)	
{
  return 0;
}

int FASTMFiberSection2d::revertToLastCommit (void)	
{
  return 0;
}

int FASTMFiberSection2d::revertToStart (void)	
{
...
int
FASTMFiberSection2d::commitState(void)			
{
	int err = 0;
...
const Vector&
FASTMFiberSection2d::getSigmaY(void)	
{
  return *sigmaY;
}

const Vector&
FASTMFiberSection2d::getTau(void)	
{
  return *tau;
}

const Vector&
FASTMFiberSection2d::getAlpha(void)	
{
  return *alpha;
}

const Vector&
FASTMFiberSection2d::getIter(void)	
{
  return *iterFile;
}

const Vector&
FASTMFiberSection2d::getEX(void)	
{
  return *exf;
}

const Vector&
FASTMFiberSection2d::getEY(void)	
{
  return *eyf;
}

const Vector&
FASTMFiberSection2d::getE1(void)	
{
  return *e1f;
}

const Vector&
FASTMFiberSection2d::getE2(void)	
{
  return *e2f;
}
...
const Vector&
FASTMFiberSection2d::getSX(void)	
{
  return *sxf;
}

const Vector&
FASTMFiberSection2d::getSY(void)	
{
  return *syf;
}

const Vector&
FASTMFiberSection2d::getS1(void)
{
  return *s1f;
}

const Vector&
FASTMFiberSection2d::getS2(void)	
{
  return *s2f;
...
void
FASTMFiberSection2d::beta(double e0, double e2, double &sc1, double &tc1, double &tc12, double &beta)	
{
...
const Vector &
FASTMFiberSection2d::getSectionDeformation (void)
{
	return e;
}

int
FASTMFiberSection2d::setTrialSectionDeformation (const Vector &deforms, double L)
{
  int res = 0;
...
const Matrix&
FASTMFiberSection2d::getInitialTangent(void)	
{
  kData[0] = 0.0; kData[1] = 0.0; kData[2] = 0.0; kData[3] = 0.0; kData[4] = 0.0; kData[5] = 0.0;
  kData[6] = 0.0; kData[7] = 0.0; kData[8] = 0.0;
...
const Matrix&
FASTMFiberSection2d::getSectionTangent(void)	
{
...
const Vector&
FASTMFiberSection2d::getStressResultant(void)	
{
  return *s;
}

SectionForceDeformation*
FASTMFiberSection2d::getCopy(void)
{
  FASTMFiberSection2d *theCopy = new FASTMFiberSection2d ();
  ...
  if (numFibers != 0) {
    ...
    if (theCopy->theMaterials1 == 0) {
      opserr <<"FASTMFiberSection2d::getCopy -- failed to allocate Material pointers\n";
      exit(-1);
    }
    ...
    if (theCopy->matData == 0) {
      opserr << "FASTMFiberSection2d::getCopy -- failed to allocate double array for material data\n";
      exit(-1);
    }
	...
	opserr <<"FASTMFiberSection2d::getCopy -- failed to get copy of a Material";
	exit(-1);
      }
    }  
  }
  ...
  if (numHFibers != 0) {
    ...
    if (theCopy->theHMaterials == 0) {
      opserr <<"FASTMFiberSection2d::getCopy -- failed to allocate HMaterial pointers\n";
      exit(-1);
    }
    ...
    if (theCopy->matHData == 0) {
      opserr << "FASTMFiberSection2d::getCopy -- failed to allocate double array for Hmaterial data\n";
      exit(-1);
    }
	...
	if (theCopy->theHMaterials[i * numHFibers + jj] == 0) {
	  opserr <<"FASTMFiberSection2d::getCopy -- failed to get copy of a HMaterial";
	  exit(-1);
	}
      }
    }  
...
const ID&
FASTMFiberSection2d::getType ()			
{
  return code;
}

int
FASTMFiberSection2d::getOrder () const	
{
  return 3;								
}

int
FASTMFiberSection2d::revertToLastCommit(double L)	
{
  int err = 0;
  ...
  for (int jj = 0; jj < NStrip; jj++) {
	...
	if ((fabs(gamma) <= DBL_EPSILON)||(fabs(alfa[jj]) <= DBL_EPSILON)||(fabs(alfa[jj]-PI/2.0) <= DBL_EPSILON)) {
	  ...
	  dTdEy = (-((Fcu1F - Fcu2F)*(pow(ex,2) - 2*ex*ey + pow(ey,2) + pow(gamma,2))*
		  ...
		  sin(2*alfa[jj])))/(2.*(-2*gamma*(pow(ex - ey,2) + pow(gamma,2))*stifcu21F*pow(cos(alfa[jj]),2) + 
		  (-ex + ey)*gamma*R2*(2*gamma*(-stifcu21F + stifcu22F)*pow(cos(alfa[jj]),2) + 8*(Fcu1F - Fcu2F)*pow(cos(alfa[jj]),3)*sin(alfa[jj]) + 
		  2*gamma*(-stifcu11F + stifcu12F)*pow(sin(alfa[jj]),2)) + (-pow(-ex + ey,2) - pow(gamma,2))*(8*(-Fcu1F + Fcu2F)*pow(cos(alfa[jj]),3)*sin(alfa[jj]) + 
		  2*gamma*(2*stifstxF + stifcu22F*pow(cos(alfa[jj]),2) + (stifcu11F + stifcu12F)*pow(sin(alfa[jj]),2)))));
	  ...
	  dSydEy = (-(gamma*(pow(ex - ey,2) + pow(gamma,2))*((stifcu11F + stifcu12F + stifcu21F + stifcu22F)*stifstyF + 
		  stifstxF*(stifcu11F + stifcu12F + stifcu21F + stifcu22F + 4*stifstyF))) + 
		  gamma*(pow(ex - ey,2) + pow(gamma,2))*(-stifcu11F - stifcu12F + stifcu21F + stifcu22F)*(stifstxF - stifstyF)*cos(2*alfa[jj]) + 
		  2*(Fcu1F - Fcu2F)*(pow(ex - ey,2) + pow(gamma,2))*(stifcu11F + stifcu12F + stifcu21F + stifcu22F + stifstxF + stifstyF)*sin(2*alfa[jj]) +
		  (Fcu1F - Fcu2F)*(pow(ex - ey,2) + pow(gamma,2))*(stifcu11F + stifcu12F + stifcu21F + stifcu22F + stifstxF + stifstyF)*sin(4*alfa[jj]) + 
		  (-ex + ey)*gamma*R2*(gamma*(stifcu11F - stifcu12F + stifcu21F - stifcu22F)*(stifstxF - stifstyF) + 
		  gamma*(-((stifcu21F - stifcu22F)*(stifstxF + stifstyF)) - stifcu12F*(4*stifcu21F + stifstxF + stifstyF) + 
		  stifcu11F*(4*stifcu22F + stifstxF + stifstyF))*cos(2*alfa[jj]) + 
		  2*(Fcu1F - Fcu2F)*(stifcu11F + stifcu12F + stifcu21F + stifcu22F + stifstxF + stifstyF)*sin(2*alfa[jj]) + 
...
int
FASTMFiberSection2d::sendSelf(int commitTag, Channel &theChannel)	
{
  int res = 0;
  ...
  if (res < 0) {
    opserr <<  "FASTMFiberSection2d::sendSelf - failed to send ID data\n";
    return res;
  }    
  ...
  if (numFibers != 0) {
    ...
    if (res < 0) {
      opserr <<  "FASTMFiberSection2d::sendSelf - failed to send material data\n";
      return res;
    }    
...
int
FASTMFiberSection2d::recvSelf(int commitTag, Channel &theChannel,
			 FEM_ObjectBroker &theBroker)						
{
  ...
  if (res < 0) {
    opserr <<  "FASTMFiberSection2d::recvSelf - failed to recv ID data\n";
    return res;
  }    
  this->setTag(data(0));
  ...
  if (data(1) != 0) {
    ...
    if (res < 0) {
      opserr <<  "FASTMFiberSection2d::recvSelf - failed to recv material data\n";
      return res;
    }    
	...
	if (theMaterials1 == 0) {
	  opserr <<"FASTMFiberSection2d::recvSelf -- failed to allocate Material pointers\n";
	  exit(-1);
	}
	...
	if (matData == 0) {
	  opserr <<"FASTMFiberSection2d::recvSelf  -- failed to allocate double array for material data\n";
	  exit(-1);
	}
      }
    }
...
void
FASTMFiberSection2d::Print(OPS_Stream &s, int flag)
{
...
Response*
FASTMFiberSection2d::setResponse(const char **argv, int argc, OPS_Stream &s)	
{
...
int 
FASTMFiberSection2d::getResponse(int responseID, Information &sectInfo)
{
...
// AddingSensitivity:BEGIN ////////////////////////////////////
const Vector &
FASTMFiberSection2d::getSectionDeformationSensitivity(int gradNumber)
{
...
const Vector &
FASTMFiberSection2d::getStressResultantSensitivity(int gradNumber, bool conditional)
{
	static Vector dummy(2);	
	return dummy;
}
...
const Matrix &
FASTMFiberSection2d::getSectionTangentSensitivity(int gradNumber)
{
...
int
FASTMFiberSection2d::commitSensitivity(const Vector& defSens, int gradNumber, int numGrads)
{
	return 0;
}
// AddingSensitivity:END ///////////////////////////////////
