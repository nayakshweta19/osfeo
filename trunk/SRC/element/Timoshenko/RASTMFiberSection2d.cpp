// $Source: /usr/local/cvs/OpenSees/SRC/element/Timoshenko/RASTMFiberSection2d.cpp,v $
// $Revision: 1.1 $
// $Date: 2009/01/10 21:22:20 $

// Created: 09/09
// Modified by: Li Ning 
// Description: This file contains the class implementation of RASTMFiberSection2d.Based on FiberSection2d.cpp.
...
ID RASTMFiberSection2d::code(2);			

// constructors:
RASTMFiberSection2d::RASTMFiberSection2d(int tag, int num, Fiber **fibers, int Hnum, Fiber **Hfibers, int strip1, double t1, int strip2, double t2, int strip3, double t3): 
  SectionForceDeformation(tag, SEC_TAG_RASTMFiberSection2d),
  ...
  iterFile(0),exf(0),e1f(0),e2f(0),eyf(0),sxf(0),s1f(0),s2f(0),syf(0)
{
    ...
    if (theMaterials1 == 0) {
      opserr << "RASTMFiberSection2d::RASTMFiberSection2d -- failed to allocate Material pointers";
      exit(-1);
    }
    ...
    if (matData == 0) {
      opserr << "RASTMFiberSection2d::RASTMFiberSection2d -- failed to allocate double array for material data\n";
      exit(-1);
    }
	...
	opserr << "RASTMFiberSection2d::RASTMFiberSection2d -- failed to get copy of a Material\n";
	exit(-1);
      }
  ...
  if (numHFibers != 0) {
    ...
    if (theHMaterials == 0) {
      opserr << "RASTMFiberSection2d::RASTMFiberSection2d -- failed to allocate HMaterial pointers";
      exit(-1);
    }
    ...
    if (matHData == 0) {
      opserr << "RASTMFiberSection2d::RASTMFiberSection2d -- failed to allocate double array for Hmaterial data\n";
      exit(-1);
    }
	...
	if (theHMaterials[i*numHFibers + jj] == 0) {
	  opserr << "RASTMFiberSection2d::RASTMFiberSection2d -- failed to get copy of a HMaterial\n";
	  exit(-1);
	}
      }
    }    
...
RASTMFiberSection2d::RASTMFiberSection2d():
  SectionForceDeformation(0, SEC_TAG_RASTMFiberSection2d),
...
int
RASTMFiberSection2d::addFiber(Fiber &newFiber)		
{
  ...
  if (newArray1[numFibers] == 0) {
    opserr <<"RASTMFiberSection2d::addFiber -- failed to get copy of a Material\n";
    delete [] newMatData;
    return -1;
  }
...
int RASTMFiberSection2d::setTrialSectionDeformation (const Vector &deforms)	
{
  return 0;
}

int RASTMFiberSection2d::revertToLastCommit (void)	
{
  return 0;
}

int RASTMFiberSection2d::revertToStart (void)	
{
  return 0;
}
...
int
RASTMFiberSection2d::commitState(void)			
{
  return 0;
}
...
const Vector&
RASTMFiberSection2d::getSigmaY(void)	
{
  return *sigmaY;
}

const Vector&
RASTMFiberSection2d::getTau(void)	
{
  return *tau;
}

const Vector&
RASTMFiberSection2d::getAlpha(void)	
{
  return *alpha;
}

const Vector&
RASTMFiberSection2d::getIter(void)	
{
  return *iterFile;
}

const Vector&
RASTMFiberSection2d::getEX(void)	
{
  return *exf;
}

const Vector&
RASTMFiberSection2d::getEY(void)	
{
  return *eyf;
}

const Vector&
RASTMFiberSection2d::getE1(void)	
{
  return *e1f;
}

const Vector&
RASTMFiberSection2d::getE2(void)	
{
  return *e2f;
}
...
const Vector&
RASTMFiberSection2d::getSX(void)	
{
  return *sxf;
}

const Vector&
RASTMFiberSection2d::getSY(void)	
{
  return *syf;
}

const Vector&
RASTMFiberSection2d::getS1(void)
{
  return *s1f;
}

const Vector&
RASTMFiberSection2d::getS2(void)	
{
  return *s2f;
...
void
RASTMFiberSection2d::beta(double e0, double e2, double &sc1, double &tc1, double &tc12, double &beta)	
{
...
const Vector &
RASTMFiberSection2d::getSectionDeformation (void)
{
	return e;
}

int RASTMFiberSection2d::setTrialSectionDeformationB (const Vector &deforms, double L)
{
  int res = 0;
...
const Matrix&
RASTMFiberSection2d::getInitialTangent(void)	
{
  kData[0] = 0.0; kData[1] = 0.0; kData[2] = 0.0; kData[3] = 0.0; kData[4] = 0.0; kData[5] = 0.0;
  kData[6] = 0.0; kData[7] = 0.0; kData[8] = 0.0;
...
const Matrix&
RASTMFiberSection2d::getSectionTangent(void)	
{
...
const Vector&
RASTMFiberSection2d::getStressResultant(void)	
{
  return *s;
}

SectionForceDeformation *
RASTMFiberSection2d::getCopy(void)
{
  RASTMFiberSection2d *theCopy = new RASTMFiberSection2d ();
  ...
  if (numFibers != 0) {
    ...
    if (theCopy->theMaterials1 == 0) {
      opserr <<"RASTMFiberSection2d::getCopy -- failed to allocate Material pointers\n";
      exit(-1);
    }
    ...
    if (theCopy->matData == 0) {
      opserr << "RASTMFiberSection2d::getCopy -- failed to allocate double array for material data\n";
      exit(-1);
    }
	...
	opserr <<"RASTMFiberSection2d::getCopy -- failed to get copy of a Material";
	exit(-1);
      }
    }  
  }
  ...
  if (numHFibers != 0) {
    ...
    if (theCopy->theHMaterials == 0) {
      opserr <<"RASTMFiberSection2d::getCopy -- failed to allocate HMaterial pointers\n";
      exit(-1);
    }
    ...
    if (theCopy->matHData == 0) {
      opserr << "RASTMFiberSection2d::getCopy -- failed to allocate double array for Hmaterial data\n";
      exit(-1);
    }
	...
	if (theCopy->theHMaterials[i * numHFibers + jj] == 0) {
	  opserr <<"RASTMFiberSection2d::getCopy -- failed to get copy of a HMaterial";
	  exit(-1);
	}
      }
    }  
...
const ID&
RASTMFiberSection2d::getType ()			
{
  return code;
}

int
RASTMFiberSection2d::getOrder () const	
{
  return 3;								
}

int
RASTMFiberSection2d::commitStateB(void)			
{
  int err = 0;
...
int
RASTMFiberSection2d::revertToLastCommitB(double L)	
{
  int err = 0;
...
int
RASTMFiberSection2d::revertToStartB(void)		
{
...
int
RASTMFiberSection2d::sendSelf(int commitTag, Channel &theChannel)	
{
  int res = 0;
  ...
  if (res < 0) {
    opserr <<  "RASTMFiberSection2d::sendSelf - failed to send ID data\n";
    return res;
  }    
  ...
  if (numFibers != 0) {
    ...
    if (res < 0) {
      opserr <<  "RASTMFiberSection2d::sendSelf - failed to send material data\n";
      return res;
    }    
...
int
RASTMFiberSection2d::recvSelf(int commitTag, Channel &theChannel,
			 FEM_ObjectBroker &theBroker)						
{
  ...
  if (res < 0) {
    opserr <<  "RASTMFiberSection2d::recvSelf - failed to recv ID data\n";
    return res;
  }    
  this->setTag(data(0));
  ...
  if (data(1) != 0) {
    ...
    if (res < 0) {
      opserr <<  "RASTMFiberSection2d::recvSelf - failed to recv material data\n";
      return res;
    }    
	...
	if (theMaterials1 == 0) {
	  opserr <<"RASTMFiberSection2d::recvSelf -- failed to allocate Material pointers\n";
	  exit(-1);
	}
	...
	if (matData == 0) {
	  opserr <<"RASTMFiberSection2d::recvSelf  -- failed to allocate double array for material data\n";
	  exit(-1);
	}
      }
    }
...
void
RASTMFiberSection2d::Print(OPS_Stream &s, int flag)
{
...
Response*
RASTMFiberSection2d::setResponse(const char **argv, int argc, OPS_Stream &s)	
{
...
int 
RASTMFiberSection2d::getResponse(int responseID, Information &sectInfo)
{
...
// AddingSensitivity:BEGIN ////////////////////////////////////
const Vector &
RASTMFiberSection2d::getSectionDeformationSensitivity(int gradNumber)
{
...
const Vector &
RASTMFiberSection2d::getStressResultantSensitivity(int gradNumber, bool conditional)
{
	static Vector dummy(2);	
	return dummy;
}
...
const Matrix &
RASTMFiberSection2d::getSectionTangentSensitivity(int gradNumber)
{
...
int
RASTMFiberSection2d::commitSensitivity(const Vector& defSens, int gradNumber, int numGrads)
{
	return 0;
}
// AddingSensitivity:END ///////////////////////////////////
