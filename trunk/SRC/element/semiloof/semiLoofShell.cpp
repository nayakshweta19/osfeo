...
// $Source: /usr/local/cvs/OpenSees/SRC/element/femski/semiLoofShell.cpp,v $

//
// Description: This file contains the implementation for the semiLoofShell class.
// semi-loof shell

#include "semiLoofShell.h"
#include <stdio.h> 
#include <stdlib.h> 
#include <math.h> 
...
static int numSemiLoofShell = 0;

void *
OPS_NewSemiLoofShell(void)
{
  if (numSemiLoofShell == 0) {
    //    opserr << "Using SemiLoofShell - Developed by: Ning Li, neallee@tju.edu.cn\n";
    numSemiLoofShell++;
  }
  ...
  if (numArgs < 11) {
    opserr << "Want: element SemiLoofShell eleTag? node1? node2? .... node9? secTag?";
    return 0;
  }
  ...
  if (OPS_GetInt(&numData, iData) != 0) {
    opserr << "WARNING invalid integer tag: element SemiLoofShell \n";
    return 0;
  }

  SectionForceDeformation *theSection = OPS_GetSectionForceDeformation(iData[10]);
  if (theSection == 0) {
    opserr << "ERROR:  element SemiLoofShell " << iData[0] << "section " << iData[10] << " not found\n";
    return 0;
  }
  ...
  if (numArgs > 11) {
    ...
    if (OPS_GetDouble(&numData, dData) != 0) {
      opserr << "WARNING invalid rho parameter of element semiLoofShell\n";
      return 0;
    }
    rho = dData[0];
  }

  theElement = new semiLoofShell(iData[0], iData[1], iData[2], iData[3],
...
//static data
Matrix  semiLoofShell::stiff(54, 54);
Vector  semiLoofShell::resid(54);
Matrix  semiLoofShell::mass(54, 54);

//quadrature data
const double  semiLoofShell::root3 = sqrt(3.0);
const double  semiLoofShell::root3_over_root5 = root3 / sqrt(5.0);

double semiLoofShell::sg[9];
double semiLoofShell::tg[9];
double semiLoofShell::wg[9];

//Node semiLoofElement::*theNodes[9];

semiLoofShell::semiLoofShell(int tag,
  int nd1, int nd2, int nd3, int nd4, int nd5, int nd6, int nd7, int nd8, int nd9,
  SectionForceDeformation &theMaterial, double rho):
  semiLoofElement(tag, ELE_TAG_semiLoofShell, 1, 3, 9, 2, 2, 0, 0),
  theLoad(0), Ki(0)
{
  for (int i = 0; i < 9; i++) {
    materialPointers[i] = theMaterial.getCopy();
    if (materialPointers[i] == 0) {
      opserr << "semiLoofShell::constructor - failed to get a material of type: ShellSection\n";
    } //end if
  } //end for i
  ...
  (*connectedNodes).resize(9);
  (*connectedNodes)(0) = nd1;
  (*connectedNodes)(1) = nd2;
  (*connectedNodes)(2) = nd3;
  (*connectedNodes)(3) = nd4;
  (*connectedNodes)(4) = nd5;
  (*connectedNodes)(5) = nd6;
  (*connectedNodes)(6) = nd7;
  (*connectedNodes)(7) = nd8;
  (*connectedNodes)(8) = nd9;
}

semiLoofShell::semiLoofShell()
  :semiLoofElement(ELE_TAG_semiLoofShell)
{
  // does nothing
}
...
int
semiLoofShell::addLoad(ElementalLoad *theLoad, double loadFactor)
{
...
//get residual
const Vector&
semiLoofShell::getResistingForce()
{
...
const Vector& 
semiLoofShell::getResistingForceIncInertia()
{
...
//form inertia terms
void
semiLoofShell::formInertiaTerms(int tangFlag)
{
...
void
semiLoofShell::formResidAndTangent(int tang_flag)
{
  //
...
//compute Bdrill
double*
semiLoofShell::computeBdrill(int node, const double shp[3][9])
{
...
//assemble a B matrix
const Matrix&
semiLoofShell::assembleB(const Matrix &Bmembrane,
const Matrix &Bbend,
const Matrix &Bshear)
{
...
const Matrix&
semiLoofShell::computeBmembrane(int node, const double shp[3][9])
{
...
const Matrix&
semiLoofShell::computeBbend(int node, const double shp[3][9])
{
...
const Matrix&
semiLoofShell::computeBshear(int node, const double shp[3][9])
{
...
void
semiLoofShell::shape2d(double ss, double tt, const double x[2][9],
...
Matrix
semiLoofShell::transpose(int dim1, int dim2, const Matrix &M)
{
  int i;
  int j;
...
Response*
semiLoofShell::setResponse(const char **argv, int argc, OPS_Stream &output)
{
...
int
semiLoofShell::getResponse(int responseID, Information &eleInfo)
{
  int i;
  int cnt = 0;
...
int  
semiLoofShell::sendSelf(int commitTag, Channel &theChannel)
{
  int res = 0;
...
int  
semiLoofShell::recvSelf(int commitTag, Channel &theChannel,
...
//set domain
void
semiLoofShell::setDomain(Domain *theDomain)
{
  int i, j;
  static Vector eig(3);
  ...
  for (i = 0; i < 9; i++) {
    ...
    if (theNodes[i] == 0) {
      opserr << "semiLoofShell::setDomain - no node " << (*connectedNodes)(i);
  ...
  Ktt = fmin(eig(2), fmin(eig(0), eig(1)));
...
//int  
//semiLoofShell::getNumExternalNodes() const
//{
//  return 9;
//}
...
//const ID&  
//semiLoofShell::getExternalNodes()
//{
//  return *connectedNodes;
//}

Node **
semiLoofShell::getNodePtrs(void)
{
  return theNodes;
}
...
void
semiLoofShell::Print(OPS_Stream &s, int flag)
{
  if (flag == -1) {
    int eleTag = this->getTag();
    s << "semiLoofShell\t" << eleTag << "\t";
    s << eleTag << "\t" << 1;
    s << "\t" << (*connectedNodes)(0) << "\t" << (*connectedNodes)(1);
    s << "\t" << (*connectedNodes)(2) << "\t" << (*connectedNodes)(3);
    s << "\t" << (*connectedNodes)(4) << "\t" << (*connectedNodes)(5);
    s << "\t" << (*connectedNodes)(6) << "\t" << (*connectedNodes)(7);
    s << "\t" << (*connectedNodes)(8) << "\t0.00";
    s << endln;
    s << "PROP_3D\t" << eleTag << "\t";
    s << eleTag << "\t" << 1;
    s << "\t" << -1 << "\tSHELL\t1.0\0.0";
    ...
    s << endln;
    s << "Nine Node semiLoofShell \n";
    s << "Element Number: " << this->getTag() << endln;
    s << "Node 1 : " << (*connectedNodes)(0) << endln;
    s << "Node 2 : " << (*connectedNodes)(1) << endln;
    s << "Node 3 : " << (*connectedNodes)(2) << endln;
    s << "Node 4 : " << (*connectedNodes)(3) << endln;
    s << "Node 5 : " << (*connectedNodes)(4) << endln;
    s << "Node 6 : " << (*connectedNodes)(5) << endln;
    s << "Node 7 : " << (*connectedNodes)(6) << endln;
    s << "Node 8 : " << (*connectedNodes)(7) << endln;
    s << "Node 9 : " << (*connectedNodes)(8) << endln;
...
int
semiLoofShell::displaySelf(Renderer &theViewer, int displayMode, float fact)
{
...
//commit state
int  
semiLoofShell::commitState()
{
  int success = 0;

  // call element commitState to do any base class stuff
  if ((success = this->Element::commitState()) != 0) {
    opserr << "semiLoofShell::commitState () - failed in base class";
  }
...
int  
semiLoofShell::revertToLastCommit()
{
  int i;
  int success = 0;
...
int
semiLoofShell::addInertiaLoadToUnbalance(const Vector &accel)
{
  opserr << "semiLoofShell::addInertiaLoadToUnbalance" << endln;
  return 0;
}