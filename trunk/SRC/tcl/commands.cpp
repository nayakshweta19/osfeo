		  ...
		  TCL_Char **argv)
{
  ...
  else if (strcmp(argv[1],"DisplacementPath") == 0) {
    int node;
    int dof;
    int numIncrement;
    Vector IncrVector;
    if (argc < 5) {
  opserr << "WARNING integrator DisplacementPath node dof numIncr \n";
  opserr << "numStep <Steps> increment <Incrs>\n";
  return TCL_ERROR;
	}
    if (Tcl_GetInt(interp, argv[2], &node) != TCL_OK)
  return TCL_ERROR;
    if (Tcl_GetInt(interp, argv[3], &dof) != TCL_OK)
  return TCL_ERROR;
    if (Tcl_GetInt(interp, argv[4], &numIncrement) == 0) {
  IncrVector.resize(numIncrement);
	} else {
  return TCL_ERROR;
	}
	if (strcmp(argv[5],"numStep") == 0 && strcmp(argv[(argc-5)/2+5],"increment") ==0 ) {
  int i=0;
  while ( i < numIncrement ) {
    double newValue;
    Tcl_GetDouble(interp, argv[6+i], &newValue);
	IncrVector(i) = newValue;

	i++;
  }
	} else {
  opserr << "ERROR integrator DisplacementPath With Different numIncr of numSteps and Increments \n";
  return TCL_ERROR;
	}
...
//							       numIter, minIncr, maxIncr);
      ...
      Node *theNode = theDomain.getNode(node);
      if (theNode == 0) {
	opserr << "WARNING integrator DisplacementPath node dof numIncr : Node does not exist\n";
	return TCL_ERROR;	  
      }

      int numDOF = theNode->getNumberDOF();
      if (dof <= 0 || dof > numDOF) {
	opserr << "WARNING integrator DisplacementPath node dof numIncr : invalid dof given\n";
	return TCL_ERROR;	  
      }

      theStaticIntegrator = new DisplacementPath(node, dof-1, IncrVector, &theDomain);
#endif
...
///*/
int
ritzAnalysis(ClientData clientData, Tcl_Interp *interp, int argc,
			 TCL_Char **argv)
{
	// make sure at least one other argument to contain type of system
	if (argc < 2) {
		opserr << "WARNING want - ritz option(n?/K/M/B/All) <-file ?filename>\n";
		return TCL_ERROR;
	}

	int typeAlgo = 0; // 0 - generalized (default)
	int typeSolver = 0; // 0 - fullGenLapack(default)
	int typeTrans = 0; // 0 - Transformation type Transform (default)
	...
	while (loc < argc) {
		...
		if ((strcmp(argv[loc],"file") == 0) || (strcmp(argv[loc],"-file") == 0) ||
			(strcmp(argv[loc],"File") == 0) || (strcmp(argv[loc],"-File") == 0)) {
			loc++;
			if (opserr.setFile(argv[loc], OVERWRITE) != 0) {
				opserr << "file <filename> .. - failed to open file: " << argv[loc] << endln;
				return TCL_ERROR;
			}
			loc++;
			isfile = 1;
		...
		if ((strcmp(argv[loc],"K") == 0) || (strcmp(argv[loc],"-K") == 0) ||
			(strcmp(argv[loc],"k") == 0) || (strcmp(argv[loc],"-k") == 0))
			type = 1;
		else if ((strcmp(argv[loc],"M") == 0) || (strcmp(argv[loc],"-M") == 0) || 
			(strcmp(argv[loc],"m") == 0) || (strcmp(argv[loc],"-m") == 0))
			type = 2;
		else if ((strcmp(argv[loc],"B") == 0) || (strcmp(argv[loc],"-B") == 0) || 
		    (strcmp(argv[loc],"b") == 0) || (strcmp(argv[loc],"-b") == 0))
			type = 3;
		else if ((strcmp(argv[loc],"All") == 0) || (strcmp(argv[loc],"-All") == 0) ||
			(strcmp(argv[loc],"all") == 0) || (strcmp(argv[loc],"-all") == 0) ||
			(strcmp(argv[loc],"a") == 0) || (strcmp(argv[loc],"-a") == 0))
			type = 0;

		else {
			opserr << "ritz - unknown option specified " << argv[loc] << endln;
			return TCL_ERROR;
		}
		loc++;
	}
	...
	//int numRitz;
	//if ((Tcl_GetInt(interp, argv[loc], &numRitz) != TCL_OK) || numRitz < 0) {
		//opserr << "WARNING ritz nums?  - illegal nums\n";
		//return TCL_ERROR;
	//int numRitz = 0;
	//}

	RitzAlgorithm *theRitzAlgo = 0;
	RitzSOE       *theRitzSOE = 0;
	AnalysisModel *theModel = new AnalysisModel();

	// create the algorithm
	//if (typeAlgo == 0)
	theRitzAlgo = new RitzAlgo();
	//else if (typeAlgo == 1) {
	//	theRitzAlgo = new RitzAlgo();
	//}
	...
	//	if (typeSolver == 0) {
			FullGenRitzLapackSolver *theRitzSOESolver = new FullGenRitzLapackSolver();
			theRitzSOE = new FullGenRitzSOE(*theRitzSOESolver);
	//	} else {
	//		opserr << "ERROR Solver  - no SOE solver specified!";
	//		return TCL_ERROR;
	//	}
	//} else {
	//	opserr << "ERROR Algorithm - no Algorithm specified!";
	//	return TCL_ERROR;
	//}
	// create the rest of components of an Ritz analysis
	RitzIntegrator  *theRitzIntegrator = new RitzIntegrator();
	DOF_Numberer *theEigenNumberer = 0;
	//if ( typeNumberer == 0 ) {
		theEigenNumberer = new PlainNumberer();
	//}
	//else if ( typeNumberer == 1) {
	//	RCM *theRCM = new RCM(false);
	//	theEigenNumberer = new DOF_Numberer(*theRCM);
	//}
	//else {
	//	opserr << "ERROR ritz - no Numberer Type specified!";
	//	return TCL_ERROR;
	//}

	ConstraintHandler *theEigenHandler = 0;
	//if ( typeTrans == 0 ) {
		theEigenHandler = new TransformationConstraintHandler();
	//}
	//else if ( typeTrans == 1) {
	//	theEigenHandler = new LagrangeConstraintHandler(1.0,1.0);
	//} else if ( typeTrans == 2) {
	//	theEigenHandler = new PenaltyConstraintHandler(10e-6,10e-6);
	//} 
	//else {
	//	opserr << "ERROR ritz - no TransformationHandler specified!";
	//	return TCL_ERROR;
	//}

	// create the Ritz analysis
	theRitzAnalysis = new RitzAnalysis(theDomain,
		*theEigenHandler,
		*theEigenNumberer,
		*theModel,
		*theRitzAlgo,
	...
	if (theRitzAnalysis->analyze(numRitz) == 0) {
		...
		if (type == 1) {
			//print K
			Matrix K = theRitzSOE->getK();
			if (isfile == 0) {
				char buffer [40];
				for (int i=0; i<K.noRows(); i++) {
					for (int j=0; j<K.noCols(); j++) {
						sprintf(buffer, "%16.8e\t", K(i,j));
			...
			//print M
			Matrix M = theRitzSOE->getM();
			if (isfile == 0) {
				char buffer [40];
				for (int i=0; i<M.noRows(); i++) {
					for (int j=0; j<M.noCols(); j++) {
						sprintf(buffer, "%16.8e\t", M(i,j));
						Tcl_AppendResult(interp, buffer, NULL);
					}
					Tcl_AppendResult(interp, endln, NULL);
				}
			...
			theRitzIntegrator->formUnbalance(); // NOTE: this assumes unbalance at last was 0
			Vector B = theRitzSOE->getB();
			currentLambda -= 1.0;
			theModel->setCurrentDomainTime(currentLambda);
			if (isfile == 0) {
				char buffer [40];
				for (int i=0; i<B.Size(); i++) {
					sprintf(buffer, "%16.8e\t", B(i));
					Tcl_AppendResult(interp, buffer, NULL);
				}
			Tcl_AppendResult(interp, endln, NULL);
			} else if (isfile == 1) {
			...
			opserr << "ERROR ritz - Tcl_AppendResult() method !\n";
			return TCL_ERROR;
		}

	} else {
		opserr << "ERROR ritz - command error with RitzAnalysis-analyze method\n!";
	}

	// finally invoke the destructor on the Ritz analysis
	delete theRitzAnalysis;
	     ...
	     TCL_Char **argv)
{
  ...
  if ((strcmp(argv[1],"element") == 0) || (strcmp(argv[1],"ele") == 0)) {
    ...
    if (argc == 3) {
      if (Tcl_GetInt(interp, argv[2], &tag) != TCL_OK) {
	opserr << "WARNING remove SPconstraint spTag? failed to read spTag: " << argv[2] << endln;
	return TCL_ERROR;
      }      
	...
	opserr << "WARNING remove SPconstraint spTag? dof? failed to read dof tag: " << argv[3] << endln;
	return TCL_ERROR;
      }      
      
      if (argc == 5) {
	if (Tcl_GetInt(interp, argv[4], &patternTag) != TCL_OK) {
	  opserr << "WARNING remove SPconstraint tag? failed to read SPconstraint tag: " << argv[4] << endln;
	  return TCL_ERROR;
	}      
...
printModelGID(ClientData clientData, Tcl_Interp *interp, int argc, TCL_Char **argv)
{
  // This function print's a file with node and elements in a format useful for GID
	...
	if (argc >= 3)
	{
		...
		else if (strcmp(argv[i],"-eleRange") == 0) {
...
printModelGIDnow(ClientData clientData, Tcl_Interp *interp, int argc, TCL_Char **argv)
{
  // This function print's a file with node and elements in a format useful for GID
	...
	if (argc < 2) {
		opserr << "WARNING printGIDnow fileName? - no filename supplied\n";
		return TCL_ERROR;
	}
	openMode mode = OVERWRITE;
	outputFile.precision(0);
    ...
    if (outputFile.setFile(argv[1], mode) < 0) {
        opserr << "WARNING printGIDnow " << argv[1] << " failed to set the file\n";
		return TCL_ERROR;
	}
	...
	if (hasLinear == 1) {
		...
		while ((theNode = theNodes()) != 0) {
			...
			outputFile.write(tag);
			outputFile << "\t\t";
			const Vector &displ = theNode->getDisp();
			for (int ii = 0; ii<l_tmp; ii++) {
				outputFile << crds(ii)+displ(ii) << "\t";
			};
			for (int ii = l_tmp; ii<3; ii++) {
				outputFile << 0.0 << "\t";
			};
