...
{   
   opserr << "ConcreteZ01::setTrialStrain() - wrong function invoked\n";
   return 0;
}

int ConcreteZ01::setTrial (double strain, double &stress, double &tangent, double strainRate)
{
   opserr << "ConcreteZ01::setTrial() - wrong function invoked\n";
   return 0;
}
...
void ConcreteZ01::getApproachFiveToComStrain( )
{
    ...
    double tempK;  // slope of the line y=kx+b
	double b;      // distance of the line

	double fiveToOneStrain = 0.0;
	double fiveToTwoStrain = 0.0;
	double fiveToTwoStress;
...
double ConcreteZ01::getPD ()
{
	...
	else
	{
		...
		{
			if ( Ttangent = 0.0 ) // at the end platum part of descending branch
			{
				PD = 0.0;
			}
			else
