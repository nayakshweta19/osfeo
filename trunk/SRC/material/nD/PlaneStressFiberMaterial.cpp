...
PlaneStressFiberMaterial::setTrialStrain(const Vector &strainFromElement)
{
  ...
  static Matrix twoDtgLastCommit(3,3);
  ...
  if (theMaterial->setTrialStrain(twoDstrainTrial) < 0) {
    opserr << "PlaneStressFiberMaterial::setTrialStrain - setStrain failed in material with strain " << twoDstrain;
    return -1;   
  }
