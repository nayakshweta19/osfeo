...
// $Revision: 1.6 $
// $Date: 2014-05-16 16:22:17 $
// $Source: /usr/local/cvs/OpenSees/SRC/material/nD/ConcreteDPM2.cpp,v $
...
OPS_NewConcreteDPM2()
{
  ...
  if (numRemainingArgs < 4) {
    opserr << "Want: NDMaterial ConcreteDPM2 tag? fc? ft? epsc0? <ecc? nu? AHard? BHard? CHard? DHard? yieldHardInitial? ASoft? helem? href?>\n";
	return 0;
  }
  ...
  if (OPS_GetInt(&numData, &tag) != 0) {
    opserr << "WARNING invalid NDMaterial ConcreteDPM2 tag" << endln;
    return 0;
  }
  ...
  if (numRemainingArgs == 4) {
    ...
    opserr << "OPS_NewConcreteDPM2, wrong material parameters!"<< endln;
  }


  if (theMaterial == 0) {
    opserr << "WARNING ran out of memory creating material\n";
    opserr << "ConcreteDPM2: " << tag << endln;
    return 0;
  }
...
ConcreteDPM2::ConcreteDPM2(int tag, 
					double fc,
					double ft,
                    double epsc0,
					double ecc,
                    ...
                    double helem,
                    double href)
:NDMaterial(tag, MAT_TAG_ConcreteDPM2),
...
double 
ConcreteDPM2::signum(double value)
{
	if (value > 0.0) {
		return 1.0;
	}
...
int 
ConcreteDPM2::setTrialStrain (const Vector &strain_from_element)
{
...
int
ConcreteDPM2::setTrialStrain( const Vector &v, const Vector &r )
{ 
   return this->setTrialStrain( v ) ;
}

int
ConcreteDPM2::setTrialStrainIncr(const Vector &v) 
{
  return -1;
}

int
ConcreteDPM2::setTrialStrainIncr(const Vector &v, const Vector &r) 
{
  return -1;
}

const Vector& 
ConcreteDPM2::getStress(void)
{
...
const Matrix& 
ConcreteDPM2::getInitialTangent(void)
{
...
const Matrix& 
ConcreteDPM2::getTangent(void)
{
    return Ttangent;
}

const Vector& 
ConcreteDPM2::getStrain(void)
{
    return Tstrain;
}

int 
ConcreteDPM2::commitState(void)
{
...
int 
ConcreteDPM2::revertToLastCommit(void)
{
	// Nothing to do here
    return 0;
}
...
int 
ConcreteDPM2::revertToStart(void)
{
	Tstrain.Zero();
	Cstrain.Zero();
	Tz.Zero();
    ...
    if (yieldHardPrimePeak < 0) {
      ...
      opserr << "ConcreteDPM2: kPrimePeak cannot be less than zero\n" << endln;
    }
    else if (yieldHardPrimePeak >(1. - yieldHardInitial)) {
      yieldHardPrimePeak = 1. - yieldHardInitial;
      opserr << "ConcreteDPM2: kPrimePeak cannot be greater than 1.-kinit\n" << endln;
    }
    AHard = 8.e-2;
    BHard = 3.e-3;
    CHard = 2.;
    ...
    m = 3. * (pow(fc, 2.) - pow(ft, 2.)) / (fc * ft) * ecc / (ecc + 1.);//Compute m
...
NDMaterial *
ConcreteDPM2::getCopy()
{
    ConcreteDPM2 *theCopy =
      new ConcreteDPM2(this->getTag(), fc, ft, epsc0, ecc, nu, AHard, BHard, CHard, DHard, yieldHardInitial, ASoft, helem, href);
...
NDMaterial *
ConcreteDPM2::getCopy(const char *type)
{
  if (strcmp(type, "ThreeDimensional") == 0 || strcmp(type, "3D") == 0) {
    ConcreteDPM2 *clone;
    clone = new ConcreteDPM2(this->getTag(), fc, ft, epsc0, ecc, nu, AHard, BHard, CHard, DHard, yieldHardInitial, ASoft, helem, href);
    return clone;
  }
    ...
    opserr << "ConcreteDPM2::getCopy failed to get copy: " << type << endln;
    return 0;
  }
}
...
const char*
ConcreteDPM2::getType( ) const 
{
  return "ThreeDimensional" ;
}

int 
ConcreteDPM2::sendSelf(int cTag, Channel &theChannel)
{
  return 0;
}

int 
ConcreteDPM2::recvSelf(int cTag, Channel &theChannel, FEM_ObjectBroker &theBroker)
{
  return 0;
}

void 
ConcreteDPM2::Print(OPS_Stream &s, int flag)
{
    s << "ConcreteDPM2, tag: " << this->getTag() << endln;
    s << "  fc: " << fc << endln;
    s << "  ft: " << ft << endln;
    s << "  epsc0: " << epsc0 << endln;
    s << "  nu: " << nu << endln;
...
int
ConcreteDPM2::setParameter(const char **argv, int argc, Parameter &param)
{
...
int
ConcreteDPM2::updateParameter(int parameterID, Information &info)
{
	switch (parameterID) {
	case 1:
...
int
ConcreteDPM2::activateParameter(int passedParameterID)
{
...
const Vector & 
ConcreteDPM2::getStrainSensitivity(int gradIndex)
{
...
const Vector & 
ConcreteDPM2::getStressSensitivity(int gradIndex, bool conditional)
{
  	return 0; //sensitivity;
}

const Matrix &
ConcreteDPM2::getInitialTangentSensitivity(int gradIndex)
{
...
int
ConcreteDPM2::commitSensitivity(Vector & strainGradient, int gradIndex, int numGrads)
{
 	return 0;
}
...
void
ConcreteDPM2::giveRealStressVector_1d(Vector &answer, const Vector &strainVector)
{
  ...
  //initGpForNewStep();
  Tstrain = strainVector;

  //Calculate the strain increment!
  Vector deltaStrain(matMode);
  deltaStrain = Tstrain - Cstrain;
...
//  }
  ...
  performPlasticityReturn(deltaStrain);

  // compute the nominal stress
  Vector stress(matMode);
  Vector effectiveStress(matMode);

  // compute elastic strains and trial stress
  Vector elasticStrain = deltaStrain;
  //elasticStrain.subtract(status->giveTempPlasticStrain());
  //elasticStrain.applyElasticStiffness(effectiveStress, eM, nu);


  Vector effectiveStressTension(matMode);
  Vector effectiveStressCompression(matMode);
  double alpha = 0.;
  if (effectiveStress(0) >= 0) { //1D tensile stress state
    alpha = 0.;
    effectiveStressTension = effectiveStress;
    effectiveStressCompression.Zero();
  }
  else if (effectiveStress(0) < 0) {      //1D compressive stress state
    alpha = 1.;
...
void
ConcreteDPM2::giveRealStressVector(Vector &answer, const Vector &strainVector)
{
...
double 
ConcreteDPM2::computeDamage(const Vector &strain)
{
  double equivStrain;
...
void
ConcreteDPM2::computeEquivalentStrain(double &tempEquivStrain, const Vector &strain)
{
...
double
ConcreteDPM2::computeInverseDamage(double dam)
{
  if (le == 0.) {
    if (helem > 0.) {
      le = helem;
...
double
ConcreteDPM2::computeDamageParam(double kappa)
{
  double omega = 0.;
  if (kappa > 0.) {
...
void
ConcreteDPM2::initDamaged(double kappaD, const Vector &strain)
{
  if (kappaD <= 0.) {
    return;
  }
...
double
ConcreteDPM2::computeDuctilityMeasureDamage(const Vector &strain)
{
...
void
ConcreteDPM2::performPlasticityReturn(Vector &strain)
{
  //initTempStatus();
...
bool
ConcreteDPM2::checkForVertexCase(double &answer, const double sig, const double tempKappa)
{
...
void
ConcreteDPM2::performRegularReturn(Vector &effectiveStress)
{
  //Define status
...
void
ConcreteDPM2::performVertexReturn(Vector &effectiveStress, double apexStress)
{
...
double
ConcreteDPM2::computeTempKappa(const double kappaInitial, const double sigTrial, const double rhoTrial, const double sig)
{
...
double
ConcreteDPM2::computeYieldValue(const double sig, const double rho, const double theta, const double tempKappa) const
{
  //compute yieldHard
  const double yieldHardOne = computeHardeningOne(tempKappa);
...
double
ConcreteDPM2::computeDFDKappa(const double sig, const double rho, const double tempKappa)
{
...
void
ConcreteDPM2::computeDFDInv(Vector &answer, const double sig, const double rho, const double tempKappa) const
{
  const double theta = thetaTrial;
...
double
ConcreteDPM2::computeDKappaDDeltaLambda(const double sig, const double rho, const double tempKappa)
{
  //Variables
...
void
ConcreteDPM2::computeDDKappaDDeltaLambdaDInv(Vector &answer, const double sig, const double rho, const double tempKappa)
{
  //Variables
  double equivalentDGDStress;
  Vector dGDInv(2);
...
double
ConcreteDPM2::computeDDKappaDDeltaLambdaDKappa(const double sig, const double rho, const double tempKappa)
{
  //Variables
  double equivalentDGDStress;
  Vector dGDInv(2);
...
double
ConcreteDPM2::computeDuctilityMeasure(const double sig, const double rho, const double theta)
{
...
void
ConcreteDPM2::computeDDuctilityMeasureDInv(Vector &answer, const double sig, const double rho, const double tempKappa)
{
...
void
ConcreteDPM2::computeDGDInv(Vector &answer, const double sig, const double rho, const double tempKappa)
{
...
double
ConcreteDPM2::computeRatioPotential(const double sig, const double tempKappa)
{
  //Compute dilation parameter
  const double R = (sig - ft / 3.) / fc;
  const double mQTension = (3. * ft / fc + m / 2.);
...
void
ConcreteDPM2::computeDDGDInvDKappa(Vector &answer, const double sig, const double rho, const double tempKappa)
{
  //Compute dilation parameter
  const double R = (sig - ft / 3.) / fc;
  const double mQTension = (3. * ft / fc + m / 2.);
...
void
ConcreteDPM2::computeDDGDDInv(Matrix &answer, const double sig, const double rho, const double tempKappa)
{
  //Compute dilation parameter
  const double R = (sig - ft / 3.) / fc;
  const double mQTension = (3. * ft / fc + m / 2.);
...
void
ConcreteDPM2::computeAMatrix(Matrix &answer, const double sig, const double rho, const double tempKappa)
{
...
void
ConcreteDPM2::give3dMaterialStiffnessMatrix(Matrix &answer)
{
...
void
ConcreteDPM2::computeTrialCoordinates(const Vector &stress)
{
...
void
ConcreteDPM2::computeDRhoDStress(Vector &answer, const Vector &stress) const
{
  int size = 6;
...
void
ConcreteDPM2::computeDSigDStress(Vector &answer) const
{
  int size = 6;
  for (int i = 0; i < 3; i++) {
    answer(i) = 1. / 3.;
...
void
ConcreteDPM2::computeDDRhoDDStress(Matrix &answer, const Vector &stress) const
{
  int size = 6;
...
void
ConcreteDPM2::computeDCosThetaDStress(Vector &answer, const Vector &stress) const
{
