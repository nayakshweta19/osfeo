  ...
  fy(FY), E0(E), fpc(FPC), rou(ROU), ac(AC), rc(RC)
{
  ...
  B = pow((fcr/fy),1.5)/rou;
  epsn = epsy*(0.91-2.0*B)/(0.98-0.25*B);
...
int 
SteelZ02::setTrial (double strain, double &stress, double &tangent, double strainRate)
{ /*
  // Reset history variables to last converged state
  TminStrain = CminStrain;  
  TmaxStrain = CmaxStrain;  
  ...
  tangent = Ttangent;
  */
  opserr << "SteelZ02::setTrial(strain, &stress, &tangent, strainRate), \n wrong set trial strain subroutine!" << endln;

  return 0;
}

void 
SteelZ02::determineTrialState (double dStrain)
{
  tt1 = Tstrain;
  tt2 = 0.0;
  ttStrain = dStrain;
...
void 
SteelZ02::determineTrialLoop(double dStrain)
{
  //if ( Tstrain > TreverseTopStrain[TreverseTopNum] )
  //{		
  //opserr << " reverseTopNum = " << reverseTopNum << endln;
...
void 
SteelZ02::determineDownPathPoint( )
{
...
void 
SteelZ02::reverseFromTenEnvelope( )
{   
...
void 
SteelZ02::reverseFromComEnvelope( )
{
...
int 
SteelZ02::sendSelf (int commitTag, Channel& theChannel)
{
  int res = 0;
...
int 
SteelZ02::recvSelf (int commitTag, Channel& theChannel,
...
void
SteelZ02::Print (OPS_Stream& s, int flag)
{
