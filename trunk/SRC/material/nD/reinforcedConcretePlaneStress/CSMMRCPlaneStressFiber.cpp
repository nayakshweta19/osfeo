...
// $Source: /usr/local/cvs/OpenSees/SRC/material/nD/CSMMRCPlaneStressFiber.cpp,v $
// File: CSMMRCPlaneStressFiber.cpp
//
// Written: Lining
// Created: 2010.11
//
...
#include "CSMMRCPlaneStressFiber.h"
...
#define ND_TAG_CSMMRCPlaneStressFiber 131

static int numCSMMRCPlaneStressFiberMaterials = 0;

OPS_Export void *
OPS_NewCSMMRCPlaneStressFiberMaterial()
{
  ...
  if (numRemainingArgs < 14) {
    opserr << "Invalid Args want: NDMaterial CSMMRCPlaneStressFiber matTag? rho? UniaxiaMatTag1? UniaxiaMatTag2? UniaxiaMatTag3? UniaxiaMatTag4? \
  ...
  if (OPS_GetInt(&numData, &tag) != 0) {
    opserr << "WARNING invalid nDMaterial CSMMRCPlaneStressFiber tag" << endln;
    return 0;
  }

  numData = 1;
  if (OPS_GetDouble(&numData, &rho) != 0) {
    opserr << "WARNING invalid parameter rho CSMMRCPlaneStressFiber tag:" << tag << endln;
    return 0;	
  }

  numData = 4;
  if (OPS_GetInt(&numData, iData) != 0) {
    opserr << "WARNING invalid nDMaterial CSMMRCPlaneStressFiber tag: " << tag << endln;
    return 0;
  }

  numData = 8;
  if (OPS_GetDouble(&numData, dData) != 0) {
    opserr << "WARNING invalid data CSMMRCPlaneStressFiber tag: " << tag << endln;
    return 0;
  }
  ...
  if (theUniaxialMaterial1 == 0) {
    ...
    opserr << "\nCSMMRCPlaneStressFiber tag: " << tag << endln;
    return 0;
  }
  ...
  //now create the CSMMRCPlaneStressFiber
  theMaterial = new CSMMRCPlaneStressFiber (tag, 
						     rho,
  ...
  if (theMaterial == 0) {
    ...
    opserr << "CSMMRCPlaneStressFiber: " << tag << endln;
    return 0;
  }
...
CSMMRCPlaneStressFiber ::CSMMRCPlaneStressFiber (int      tag, 
								   double   RHO,
								   UniaxialMaterial *s1,
								   UniaxialMaterial *s2,
								   UniaxialMaterial *c1,
  ...
  fiberStrain(2), fiberStress(2), fiberTangent(2,2)
{
    ...
    if ( theMaterial == 0 ) {
      opserr << " CSMMRCPlaneStressFiber::CSMMRCPlaneStressFiber - failed allocate material array\n";
      exit(-1);
    }
    ...
    // Check allocation    
    if ( theMaterial[0] == 0 ) {
      opserr << " CSMMRCPlaneStressFiber::CSMMRCPlaneStressFiber - failed to get a copy for steel1\n";
      exit(-1);
    }
    ...
    // Check allocation    
    if ( theMaterial[1] == 0 ) {
      opserr << " CSMMRCPlaneStressFiber::CSMMRCPlaneStressFiber - failed to get a copy for steel2\n";
      exit(-1);
    }
    ...
    // Check allocation    
    if ( theMaterial[2] == 0 ) {
      opserr << " CSMMRCPlaneStressFiber::CSMMRCPlaneStressFiber - failed to get a copy for concrete1\n";
      exit(-1);
    }
    ...
    // Check allocation    
    if ( theMaterial[3] == 0 ) {
      opserr << " CSMMRCPlaneStressFiber::CSMMRCPlaneStressFiber - failed to get a copy for concrete2\n";
      exit(-1);
    }
    ...
    if ( theResponses == 0) {
      opserr << " CSMMRCPlaneStressFiber::CSMMRCPlaneStressFiber - failed allocate responses array\n";
      exit(-1);
    }
	...
	(theResponses[4] == 0) || (theResponses[5] == 0)) {
      opserr << " CSMMRCPlaneStressFiber::CSMMRCPlaneStressFiber - failed to set appropriate materials tag: " << tag << "\n";
      exit(-1);
    }
...
double 
CSMMRCPlaneStressFiber::getRho(void)
{
	return rho;
}
...
int 
CSMMRCPlaneStressFiber::setTrialStrainIncr(const Vector &v)
{
  opserr << "error: CSMMRCPlaneStressFiber::setTrialStrainIncr(&v) -- not really responsibility" << endln;
  return 0;
}

int 
CSMMRCPlaneStressFiber::setTrialStrainIncr(const Vector &v, const Vector &r)
{
  opserr << "error: CSMMRCPlaneStressFiber::setTrialStrainIncr(&v, &r) -- not really responsibility" << endln;
  return 0;
}
...
int 
CSMMRCPlaneStressFiber::revertToLastCommit(void)
{
...
NDMaterial* 
CSMMRCPlaneStressFiber::getCopy(void)
{
  CSMMRCPlaneStressFiber* theCopy =
    new CSMMRCPlaneStressFiber( this->getTag(), 
					 rho, 
					 theMaterial[0], 
					 theMaterial[1], 
					 theMaterial[2], 
...
NDMaterial* 
CSMMRCPlaneStressFiber::getCopy(const char *type)
{
	CSMMRCPlaneStressFiber* theModel =
		new CSMMRCPlaneStressFiber( this->getTag(), 
		                 rho, 
		                 theMaterial[0], 
		                 theMaterial[1], 
...
int 
CSMMRCPlaneStressFiber::sendSelf(int commitTag, Channel &theChannel)
{
  int res = 0;
  ...
  if (res < 0) {
    opserr << "WARNING CSMMRCPlaneStressFiber::sendSelf() - " << this->getTag() << " failed to send Vector\n";
    return res;
  }	      
  ...
  if (res < 0) {
    opserr << "WARNING CSMMRCPlaneStressFiber::sendSelf() - " << this->getTag() << " failed to send ID\n";
    return res;
  }
  ...
  for (i = 0; i < 4; i++) {
    ...
    if (res < 0) {
      opserr << "CSMMRCPlaneStressFiber::sendSelf() - " << this->getTag() << " failed to send its Material\n";
      return res;
    }
  }	
...
int 
CSMMRCPlaneStressFiber::recvSelf(int commitTag, Channel &theChannel, FEM_ObjectBroker &theBroker)
{
  int res = 0;
  ...
  if (res < 0) {
    opserr << "WARNING CSMMRCPlaneStressFiber::recvSelf() - failed to receive Vector\n";
    return res;
  }
  ...
  if (res < 0) {
    opserr << "WARNING CSMMRCPlaneStressFiber::recvSelf() - " << this->getTag() << " failed to receive ID\n";
    return res;
  }
  ...
  if (theMaterial == 0) {
    ...
    theMaterial = new UniaxialMaterial *[4];
    if (theMaterial == 0) {
      opserr << "CSMMRCPlaneStressFiber::recvSelf() - Could not allocate UniaxialMaterial* array\n";
      return -1;
    }
    for (int i = 0; i < 4; i++) {
      ...
      if (theMaterial[i] == 0) {
	     opserr << "CSMMRCPlaneStressFiber::recvSelf() - Broker could not create NDMaterial of class type " << matClassTag << endln;
	     return -1;
      }
      ...
      if (res < 0) {
         opserr << "CSMMRCPlaneStressFiber::recvSelf() - material " << i << "failed to recv itself\n";
	     return res;
      }
    }
  }
  ...
  // materials exist , ensure materials of correct type and recvSelf on them
    ...
    for (int i = 0; i < 4; i++) {
      ...
      if (theMaterial[i]->getClassTag() != matClassTag) {
	     ...
	     theMaterial[i] = theBroker.getNewUniaxialMaterial(matClassTag);
	     if (theMaterial[i] == 0) {
            opserr << "CSMMRCPlaneStressFiber::recvSelf() - material " << i << "failed to create\n";
	        return -1;
		 }
	  }
...
{ 
  ...
  if (_isnan(citaR) != 0) {
    opserr << "CSMMRCPlaneStressFiber::determineTrialStress: Failure to calculate citaR\n";
  ...
  if ( status == 0 ) {  // does not get converged after iteration
      ...
      // if ( minError > 0.05 )
      //    opserr << "CSMMRCPlaneStressFiber::determineTrialStress: Warning, failure to get converged principal stress direction\n";
  } 
...
double 
CSMMRCPlaneStressFiber::getAngleError(double inputCita)
{
	double outputCita;
...
double 
CSMMRCPlaneStressFiber::getPrincipalStressAngle(double inputAngle)
{   
	...
	if ( (epslonOne > 0.0) && (epslonTwo > 0.0) ) {  // both tension
      ...
      opserr << "CSMMRCPlaneStressFiber::getPrincipalStressAngle: failure to get Hsu/Zhu ratio!\n";
    }
	...
	V.Zero();
    if ( v12*v21==1.0 ) {
	  opserr << "CSMMRCPlaneStressFiber::getPrincipalStressAngle: failure to get matrix [V]!\n";
    ...
    tangent_matrix(0,2) = 0.5*tangent_matrix(0,2);
	tangent_matrix(1,2) = 0.5*tangent_matrix(1,2);
	tangent_matrix(2,2) = 0.5*tangent_matrix(2,2);
	...
	double ratio;
	double AA = (Tstress(0)-Tstress(1))/2.;
	double BB = Tstress(2);
	//double CC = pow(A,2.)+pow(B,2.);

    if ( fabs(Tstress(0)-Tstress(1)) < DBL_EPSILON ) {
      citaOut = 0.25*PI;
	} else {    // Tstrain(0) != Tstrain(1)
      ratio = BB/AA;
      citaOut = atan(ratio)/2.;
	}

	if (_isnan(citaOut) != 0) {
	  opserr << "CSMMRCPlaneStressFiber::getPrincipalStressAngle: Failure to calculate principal stress direction\n";
