...
OPS_Export void *
OPS_NewMCFTSteel01Material(void)
{
  ...
  if (numRemainingArgs < 4 || numRemainingArgs > 12) {
    opserr << "Invalid Args want: uniaxialMaterial MCFTSteel01 tag? fy? E? b? "
  ...
  if (OPS_GetIntInput(&numData, iData) != 0) {
    opserr << "WARNING invalid uniaxialMaterial MCFTSteel01 tag" << endln;
    return 0;
  }
  ...
  if (numRemainingArgs == 3) {
    if (OPS_GetDoubleInput(&numRemainingArgs, dData) != 0) {
      opserr << "Invalid Args want: uniaxialMaterial MCFTSteel01 tag? fy? E? b? " << endln;
      return 0;	
    } else
      theMaterial = new MCFTSteel01(iData[0], dData[0], dData[1], dData[2]);
    ...
    if (OPS_GetDoubleInput(&numRemainingArgs, dData) != 0) {
      ...
      theMaterial = new MCFTSteel01(iData[0], dData[0], dData[1], dData[2], dData[3], dData[4], dData[5]);
  ...
  if (theMaterial == 0) {
    opserr << "WARNING could not create uniaxialMaterial of type MCFTSteel01\n";
    return 0;
  }
...
MCFTSteel01::MCFTSteel01(int tag,
...
UniaxialMaterial(tag, MAT_TAG_MCFTSteel01),
...
MCFTSteel01::MCFTSteel01(void):
UniaxialMaterial(0, MAT_TAG_MCFTSteel01)
{
  konP = 0;
}

MCFTSteel01::~MCFTSteel01()
{
	// does nothing;
}

UniaxialMaterial*
MCFTSteel01::getCopy(void)
{
  MCFTSteel01 *theCopy = new MCFTSteel01(this->getTag(), Fy, E0, b, R0, cR1, cR2, a1, a2, a3, a4, sigini);
  
  return theCopy;
}

double
MCFTSteel01::getInitialTangent(void)
{
  return E0;
}

int
MCFTSteel01::setTrialStrain(double trialStrain, double strainRate)
{
  double Esh = b * E0;
  double epsy = Fy / E0;
...
double 
MCFTSteel01::getStrain(void)
{
  return eps;
}

double 
MCFTSteel01::getStress(void)
{
  return sig;
}

double 
MCFTSteel01::getTangent(void)
{
  return e;
}

double
MCFTSteel01::getSecant ()
{
	if ( eps == 0.0 ) {
		return E0;
	} else {
...
int 
MCFTSteel01::revertToLastCommit(void)
{
  epsmin = epsminP;
  epsmax = epsmaxP;
  epspl = epsplP;
...
int 
MCFTSteel01::revertToStart(void)
{
  eP = E0;
  epsP = 0.0;
  sigP = 0.0;
...
int 
MCFTSteel01::sendSelf(int commitTag, Channel &theChannel)
{
  ...
  if (theChannel.sendVector(this->getDbTag(), commitTag, data) < 0) {
    opserr << "MCFTSteel01::sendSelf() - failed to sendSelf\n";
    return -1;
  }
  return 0;
}
...
int 
MCFTSteel01::recvSelf(int commitTag, Channel &theChannel, 
	     FEM_ObjectBroker &theBroker)
{
  static Vector data(23);

  if (theChannel.recvVector(this->getDbTag(), commitTag, data) < 0) {
    opserr << "MCFTSteel01::recvSelf() - failed to recvSelf\n";
    return -1;
  }
...
Response* 
MCFTSteel01::setResponse(const char **argv, int argc,
			 OPS_Stream &theOutput)
{
  ...
  } else if (strcmp(argv[0],"getOffsetStrain") == 0) {
	double data = 0.0;
...
int 
MCFTSteel01::getResponse(int responseID, Information &matInfo)
{
  if (responseID == 100) {
...
void 
MCFTSteel01::Print(OPS_Stream &s, int flag)
{
  s << "MCFTSteel01:(strain, stress, tangent) " << eps << " " << sig << " " << e << endln;
}
