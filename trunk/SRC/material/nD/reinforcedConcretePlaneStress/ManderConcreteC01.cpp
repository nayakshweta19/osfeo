...
OPS_ManderConcreteC01()
{
  ...
  if (OPS_GetIntInput(&numData, iData) != 0) {
    opserr << "WARNING invalid uniaxialMaterial ManderConcreteC01 tag \n" << endln;
    return 0;
  }

  numData = 9;
  if (OPS_GetDoubleInput(&numData, dData) != 0) {
    opserr << "WARNING invalid input, want: uniaxialMaterial ManderConcreteC01 tag fcc ecc Ec rn_pre rn_post ft et rp xp_cr \n";
    return 0;	
  }
  ...
  theMaterial = new ManderConcreteC01(iData[0], dData[0], dData[1], dData[2], dData[3], dData[4], dData[5], dData[6], dData[7], dData[8], xn_cr);

  if (theMaterial == 0) {
    opserr << "WARNING could not create uniaxialMaterial of type ManderConcreteC01\n";
    return 0;
  }
...
ManderConcreteC01::ManderConcreteC01(int tag, double i1, double i2, double i3, double i4, double i5, double i6, double i7, double i8, double i9, double i10)
:UniaxialMaterial(tag,MAT_TAG_ManderConcreteC01),
...
ManderConcreteC01::ManderConcreteC01()
:UniaxialMaterial(0,MAT_TAG_ManderConcreteC01),
...
int 
ManderConcreteC01::setTrialStrain(double strain, double strainRate)
{
...
double 
ManderConcreteC01::getStrain(void)
{
  return Tstrain;
}

double 
ManderConcreteC01::getStress(void)
{
  return Tstress;
}

double 
ManderConcreteC01::getTangent(void)
{
  return Ttangent;
}

double 
ManderConcreteC01::getInitialTangent(void)
{
  return Ec;
}

int 
ManderConcreteC01::commitState(void)
{
	this->commitStateVar();
	return 0;
}	
...
int 
ManderConcreteC01::revertToLastCommit(void)
{
...
int 
ManderConcreteC01::revertToStart(void)
{
...
UniaxialMaterial *
ManderConcreteC01::getCopy(void)
{
  ManderConcreteC01 *theCopy = new ManderConcreteC01(this->getTag(), Fc_n, ec_n, Ec, r_n_pre, r_n_post, Fc_p, ec_p, r_p, x_p_cr, x_n_cr);
...
int 
ManderConcreteC01::sendSelf(int cTag, Channel &theChannel)
{
  int res = 0;
  ...
  if (res < 0) 
    opserr << "ManderConcreteC01::sendSelf() - failed to send data\n";

  return res;
}

int 
ManderConcreteC01::recvSelf(int cTag, Channel &theChannel, FEM_ObjectBroker &theBroker)
{
  int res = 0;
  static Vector data(6);
  res = theChannel.recvVector(this->getDbTag(), cTag, data);
  if (res < 0) 
    opserr << "ManderConcreteC01::recvSelf() - failed to recv data\n";
  else {
    this->setTag(data(0));
...
void 
ManderConcreteC01::Print(OPS_Stream &s, int flag)
{
	s<<"ManderConcreteC01, tag: "<<this->getTag()<<endln;
	s<<" Ec:      "<<Ec<<endln;
	s<<" Fcn:     "<<Fc_n<<endln;
	s<<" ecn:     "<<ec_n<<endln;
...
void 
ManderConcreteC01::negativeEnvelope(double strain, double &stress, double &tangent)
{	
	double x_n, n_n, y, z;
	x_n = strain/ec_n;
...
void 
ManderConcreteC01::positiveEnvelope(double strain, double &stress, double &tangent)
{
	double n_p, x_p;
...
void 
ManderConcreteC01::transitionCurve(double Tstrain, double &Tstress, double &Ttangent, 
...
void 
ManderConcreteC01::tsaiEquation(double x, double r, double n, double &y, double &z)
{
	double D;
	if( r == 1 ) {
	    D = 1 + ( n - 1 + log( x ) ) * x;
...
void
ManderConcreteC01::backToCommitStateVar(void){
...
void
ManderConcreteC01::commitStateVar(void){
...
double 
ManderConcreteC01::getZeta ()
{
	return zeta;
}

Response* 
ManderConcreteC01::setResponse(const char **argv, int argc,
...
int 
ManderConcreteC01::getResponse(int responseID, Information &matInfo)
{
	if (responseID == 100) {
