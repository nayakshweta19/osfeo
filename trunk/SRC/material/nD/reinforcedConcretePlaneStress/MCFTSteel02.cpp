...
OPS_NewMCFTSteel02Material(void)
{
  ...
  if (numRemainingArgs < 5) {
    opserr << "Invalid Args want: uniaxialMaterial MCFTSteel02 tag? fy? E0? fpc? rou? <ac?> <rc?>" << endln;
    return 0;	
  }
  ...
  if (numRemainingArgs == 4) {
    if (OPS_GetDoubleInput(&numRemainingArgs, dData) != 0) {
      ...
      theMaterial = new MCFTSteel02(iData[0], dData[0], dData[1], dData[2], dData[3]);
  } else if (numRemainingArgs == 6) {
    if (OPS_GetDoubleInput(&numRemainingArgs, dData) != 0) {
      opserr << "Invalid Args want: uniaxialMaterial MCFTSteel02 tag? fy? E0? fpc? rou? <ac?> <rc?>" << endln;
      return 0;	
    } else
      theMaterial = new MCFTSteel02(iData[0], dData[0], dData[1], dData[2], dData[3], dData[4], dData[5]);
  } else {
...
MCFTSteel02::MCFTSteel02(): UniaxialMaterial(0, MAT_TAG_MCFTSteel02),
...
int MCFTSteel02::revertToLastCommit ()
{
...
double MCFTSteel02::getCommittedStrain()
{
   return Cstrain;
}

double MCFTSteel02::getStrain ()
{
   return Tstrain;
}

double MCFTSteel02::getStress ()
{
   return Tstress;
}

double MCFTSteel02::getTangent ()
{
   // temp add
   if ( Ttangent == 0.0 )
   {
   	   opserr << " MCFTSteel02:getTangent() -- Ttangent = 0.0\n";
...
int MCFTSteel02::setTrialStrain (double strain, double strainRate)
{
...
int MCFTSteel02::setTrial (double strain, double &stress, double &tangent, double strainRate)
{
   // Reset history variables to last converged state
	TminStrain = CminStrain;  
    TmaxStrain = CmaxStrain;  
...
void MCFTSteel02::determineTrialState (double dStrain)
{
    tt1 = Tstrain;
	tt2 = 0.0;
	ttStrain = dStrain;
	...
	else if ( TloadingState == 4 ) // In the loop reversed from tension envelope
	{
		...
		else
		{
			...
			//{
				//opserr << " MCFTSteel02 : determinTrialState \n";
	...
	{
		opserr << "MCFTSteel02::determineTrialState -- Improper TloadingState : " 
			 << TloadingState << " for MCFTSteel02\n" ;
	}
...
void MCFTSteel02::initialEnvelope( )
{
	//Set parameter values
	double epsy = fy/E0;
	double fcr = 0.31*sqrt(fpc);
...
void MCFTSteel02::tensionEnvelope( )
{
	double epsy = fy/E0;
	double fcr = 0.31*sqrt(fpc);
	if ( rou < 0.0025 ) rou = 0.0025;
...
void MCFTSteel02::compressionEnvelope( )
{
	double epsy = fy/E0;
	Tstress = 0.001*E0*(Tstrain+epsy) -fy;
	/*Ttangent = Tstress/Tstrain;
...
void MCFTSteel02::determineTrialLoop(double dStrain)
{
	...
	else // create a new inner loop or continue on previous loop
	{
		 ...
		 if (( TloopPathState == 2 || TloopPathState == 3 ) && ( dStrain > 0)) // reverse up at down path
		{
			...
			{
				opserr << " MCFTSteel02::determineTrialLoop -- overflowed the size of the array storing the loop!\n"
		...
		else // continue on previous path of the loop
		{
			...
			{
				  opserr << " MCFTSteel02::determineTrialLoop -- improper TloopPathState : " 
					   << TloopPathState << "\n" ;
			}
		}
...
void MCFTSteel02::determineDownPathPoint( )
{
...
void MCFTSteel02::determineUpPathPoint( )
{
	double epsy = fy/E0;
	double fcr = 0.31*sqrt(fpc);
	if ( rou < 0.0025 ) rou = 0.0025;
...
void MCFTSteel02::reverseFromTenEnvelope( )
{   
...
void MCFTSteel02::reverseFromComEnvelope( )
{
...
void MCFTSteel02::reverseLoopSetZero( )
{
	TloopPathState = 0;
	TreverseTopNum = 0;
	TreverseBottomNum = 0;
...
UniaxialMaterial* MCFTSteel02::getCopy ()
{
   MCFTSteel02* theCopy = new MCFTSteel02(this->getTag(), fy, E0, fpc, rou, ac,rc);     
...
int MCFTSteel02::sendSelf (int commitTag, Channel& theChannel)
{
   int res = 0;
...
int MCFTSteel02::recvSelf (int commitTag, Channel& theChannel,
...
void MCFTSteel02::Print (OPS_Stream& s, int flag)
{
