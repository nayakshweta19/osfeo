...
// $Revision: 1.2 $
// $Date: 2008-09-10 19:05:53 $
// $Source: /scratch/slocal/chroot/cvsroot/openseescomp/OpenSEESComp/OpenSees/SRC/material/uniaxial/CCFT_stlMaterial.cpp,v $
...
CCFT_stlMaterial::CCFT_stlMaterial( int tag, double Fy, double Fu, double es, double depth, double thickness, double Eplo )
  :UniaxialMaterial(tag, MAT_TAG_CCFT_stl), fy(Fy), fu(Fu), Ee(es), D(depth), t(thickness), epo(Eplo) 
{
       if(fy > 52){
         Rbso =  1.06 * fy;
         //Rbso = 1.15 * fy;
...
CCFT_stlMaterial::CCFT_stlMaterial()
  :UniaxialMaterial(0,MAT_TAG_CCFT_stl),
...
double 
CCFT_stlMaterial::getStress(void)
{
  return strs;
}

double 
CCFT_stlMaterial::getTangent(void)
{
  return tgnt;
}

double 
CCFT_stlMaterial::getInitialTangent(void)
{
  return Ee;
}

double 
CCFT_stlMaterial::getStrain(void)
{
  return Tstrain;
}
...
int 
CCFT_stlMaterial::revertToStart(void)
{
  return lb;
}

void
CCFT_stlMaterial::commitStatevar(void)
{
   CRlso = Rlso;
   CRls = Rls;
   Cls_p = Tls_p;
...
void
CCFT_stlMaterial::backtocommitStatevar(void)
{
   Rlso = CRlso;
   Rls = CRls;
   Tls_p = Cls_p;
...
UniaxialMaterial *
CCFT_stlMaterial::getCopy(void)
{
  CCFT_stlMaterial *theCopy = new CCFT_stlMaterial(this->getTag(), fy, fu, Ee, D, t, epo);
...
int 
CCFT_stlMaterial::sendSelf(int cTag, Channel &theChannel)
{
  ...
  if(res < 0)
  opserr<<"CCFT_stlMaterial::sendSelf() - failed to send data\n";
  return res;
}

int 
CCFT_stlMaterial::recvSelf(int cTag, Channel &theChannel, 
			      FEM_ObjectBroker &theBroker)
{
  static Vector data(102);
  int res = theChannel.recvVector(this->getDbTag(),cTag, data);
  if(res<0) {
    opserr<<"CCFT_stlMaterial::recvSelf() - failed to receive data \n";
    this->setTag(0);
  }
  else{
...
void 
CCFT_stlMaterial::Print(OPS_Stream &s, int flag){
  s<<"CCFT_stl, tag:"<<this->getTag()<<endln;
  s<<" fy:"<<fy<<endln;
  s<<" fu:"<<fu<<endln;
  s<<" Ee: "<<Ee<<endln;
  s<<" Ksft:"<<Ksft<<endln;
