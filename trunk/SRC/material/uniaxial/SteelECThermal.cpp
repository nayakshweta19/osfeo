...
int SteelECThermal::setTrialStrain(double strain, double FiberTemperature, double strainRate)
{
  ...
  Ttemp = FiberTemperature;
   ...
   if (fabs(dStrain) > DBL_EPSILON || dStrain == 0||Ttemp>Ctemp) {
     // Set trial strain
     Tstrain = strain;
	...
	else 
	{
	  ...
	  if (fabsTstrain <= EpsiPT) 
	  ...
	  else if (fabsTstrain <= EpsiYT )
	 	{
	 	 ...
	 	 {
		  Tstress = fy*(1- (fabsTstrain - EpsiT)/(EpsiU -EpsiT));
          //opserr<<"Error: Stiffness of SteelECthermal is negative"<<endln;
		  Ttangent = -fy/(EpsiU-EpsiT);
	  	}
	  else 
		{ 
			 Tstress = 1E-10;
...
double 
SteelECThermal::getThermalElongation(void) //***JZ
{
...
double 
SteelECThermal::getElongTangent(double TempT, double &ET, double &Elong, double TempTmax) //PK add to include max temp
{
	double *FyRfactors;
	double *FpRfactors;
	double *E0Rfactors;
  // opserr <<"typeTag"<<typeTag;
   ...
   //Now Updating modulus, strengths
	for( int i=0; i<13; i++) {
		...
		{
			if (i==0) {
				fy = fyT*(1.0 - TempT*(1.0-FyRfactors[0])/100);
				fp = fyT*(1.0 - TempT*(1.0-FpRfactors[0])/100);
				E0 = E0T*(1.0 - TempT*(1.0-E0Rfactors[0])/100);
			}
			else if (i==12) {
				opserr << "Warning:The temperature for SteelECthermal is out of range\n"; 
			}
			else {
