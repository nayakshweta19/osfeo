...
// $Source: /scratch/slocal/chroot/cvsroot/openseescomp/OpenSEESComp/OpenSees/SRC/material/section/repres/patch/iPatch.cpp,v $
...
iPatch::iPatch(): matID(0)
{

}


iPatch::iPatch(int materialID, double D, double B, double TF, double TW, double FILLET, int SFL_D, int SFL_B,
		int SWL_D, int SWL_B):
...
void iPatch::setMaterialID(int materialID)
{
   matID = materialID;
}


void iPatch::setDiscretization(double D, double B, double TF, double TW, double FILLET, int SFL_D, int SFL_B, 
		int SWL_D, int SWL_B)
{
   d = D;
   b = B;
...
int iPatch::getMaterialID(void) const
{
   return matID;
}
 
void iPatch::getDiscretization(double &D, double &B, double &TF, double &TW, double &FILLET, int &SFL_D, 
...
int iPatch::getNumCells (void) const
{
   return 4 + 2* sfl_d * sfl_b + swl_d * swl_b;	      
}


Cell ** iPatch::getCells (void) const
{ 
  ...
  cells[0] = new iCell(matID, cellcentroid, area_temp);
      
  cellcentroid(0) = y_temp;
  cellcentroid(1) = z_temp;
  cells[1] = new iCell(matID, cellcentroid, area_temp);

  cellcentroid(0) = - y_temp;
  cellcentroid(1) = z_temp;
  cells[2] = new iCell(matID, cellcentroid, area_temp);

  cellcentroid(0) = - y_temp;
  cellcentroid(1) = - z_temp;
  cells[3] = new iCell(matID, cellcentroid, area_temp);
  ...
  for ( j = 1; j <= sfl_d; j++ )
  {
      ...
      for ( k = 1; k <= sfl_b; k++ )
      {
	   ...
	   cells[index-1] = new iCell(matID, cellcentroid, area_temp);
...
Patch * 
iPatch::getCopy (void) const
{
   iPatch *theCopy = new iPatch (matID, d, b, tf, tw, fillet, sfl_d, sfl_b, swl_d, swl_b);
   return theCopy;
}
 
void iPatch::Print(OPS_Stream &s, int flag) const
{
   s << "\nPatch Type: iPatch";
   s << "\nMaterial Id: " << matID;
   s << "\nDepth: "<< d;
...
OPS_Stream &operator<<(OPS_Stream &s, iPatch &i_Patch)
{
   i_Patch.Print(s);
   return s;
}
