...
CulaSparseSolverS6::CulaSparseSolverS6(double relTol,
  int maxInteration,
  int preCond,
  int solver,
  int single,
...
//int
//CulaSparseSolverS6::setLinearSOE(SparseGenColLinSOE &theLinearSOE)
int
CulaSparseSolverS6::setLinearSOE(SparseGenRowLinSOE &theLinearSOE)
{
  theSOE = &theLinearSOE;
  return 0;
}
...
int
CulaSparseSolverS6::sendSelf(int cTAg, Channel &theChannel)
{
  // doing nothing
  return 0;
}
...
int
CulaSparseSolverS6::recvSelf(int cTag,
...
int CulaSparseSolverS6::solve(void)
{
  if (theSOE == 0) {
