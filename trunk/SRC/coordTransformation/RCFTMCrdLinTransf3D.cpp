...
// $Source: /scratch/slocal/chroot/cvsroot/openseescomp/OpenSEESComp/OpenSees/SRC/coordTransformation/RCFTMCrdLinTransf3D.cpp,v $
...
Matrix RCFTMCrdLinTransf3D::Tlg(12, 12);
Matrix RCFTMCrdLinTransf3D::kg(12, 12);

// constructor:
RCFTMCrdLinTransf3D::RCFTMCrdLinTransf3D(int tag, const Vector &vecInLocXZPlane):
  CrdTransf(tag, CRDTR_TAG_RCFTMCrdLinTransf3D),
...
RCFTMCrdLinTransf3D::RCFTMCrdLinTransf3D():
  CrdTransf(0, CRDTR_TAG_RCFTMCrdLinTransf3D),
...
int
RCFTMCrdLinTransf3D::commitState(void)
{
   return 0;
}
...
int
RCFTMCrdLinTransf3D::revertToLastCommit(void)
{
   return 0;
}
...
int
RCFTMCrdLinTransf3D::revertToStart(void)
{
   return 0;
}
...
int
RCFTMCrdLinTransf3D::initialize(Node *nodeIPointer, Node *nodeJPointer)
{
   int error;
...
int
RCFTMCrdLinTransf3D::update(void)
{
   return 0;
}
...
int
RCFTMCrdLinTransf3D::computeElemtLengthAndOrient()
{
   // element projection
   static Vector dx(3);
...
void
RCFTMCrdLinTransf3D::compTransfMatrixLocalGlobal(Matrix &Tlg)
{
...
int
RCFTMCrdLinTransf3D::getLocalAxes(Vector &XAxis, Vector &YAxis, Vector &ZAxis)
{
...
double
RCFTMCrdLinTransf3D::getInitialLength(void)
{
   return L;
}

double
RCFTMCrdLinTransf3D::getDeformedLength(void)
{
   return L;
}
...
const Vector &
RCFTMCrdLinTransf3D::getBasicTrialDisp (void)
{
   Vector ub(12);
   ub.Zero();
   return ub;
...
const Vector &
RCFTMCrdLinTransf3D::getBasicIncrDisp (void)
{
   Vector ub(12);
   ub.Zero();
   return ub;
...
const Vector &
RCFTMCrdLinTransf3D::getBasicIncrDeltaDisp(void)
{
   Vector ub(12);
   ub.Zero();
   return ub;
...
const Vector &
RCFTMCrdLinTransf3D::getBasicTrialVel(void)
{
   Vector ub(12);
   ub.Zero();
   return ub;
...
const Vector &
RCFTMCrdLinTransf3D::getBasicTrialAccel(void)
{
   Vector ub(12);
   ub.Zero();
   return ub;
...
const Vector&
RCFTMCrdLinTransf3D::getGlobalResistingForce(const Vector &sg, const Vector &p0)
{
...
const Matrix &
RCFTMCrdLinTransf3D::getInitialGlobalStiffMatrix (const Matrix &KB)
{
  ...
  double nat_to_local[11][18];   //Natural to local transformation
  double nat_to_local_T[18][11]; //Transpose of nat_to_local
  double temp_rt[11][18];        //Temporary storage for transformation
  ...
  nat_to_local[0][15]    = 1.0;
  nat_to_local[1][5]     = 1.0;
  nat_to_local[1][7]     = 1.0 / L;
  nat_to_local[1][16]    = - 1.0 / L;
  nat_to_local[2][4]     = - 1.0;
  nat_to_local[2][8]     = 1.0 / L;
  nat_to_local[2][17]    = - 1.0 / L;
  nat_to_local[3][7]     = 1.0 / L;
  nat_to_local[3][14]    = 1.0;
  nat_to_local[3][16]    = - 1.0 / L;
  nat_to_local[4][8]     = 1.0 / L;
  nat_to_local[4][13]    = - 1.0;
  nat_to_local[4][17]    = - 1.0 / L;
  nat_to_local[5][0]     = -1.0;
  nat_to_local[5][9]     = 1.0;
  nat_to_local[6][1]     = 1.0 / L;
  nat_to_local[6][5]     = 1.0;
  nat_to_local[6][10]    = - 1.0 / L;
  nat_to_local[7][2]     = 1.0 / L;
  nat_to_local[7][4]     = - 1.0;
  nat_to_local[7][11]    = - 1.0 / L;
  nat_to_local[8][1]     = 1.0 / L;
  nat_to_local[8][10]    = - 1.0 / L;
  nat_to_local[8][14]    = 1.0;
  nat_to_local[9][2]    = 1.0 / L;
  nat_to_local[9][11]   = - 1.0 / L;
  nat_to_local[9][13]   = - 1.0;
  nat_to_local[10][3]    = - 1.0;
  nat_to_local[10][12]   = 1.0;
...
const Matrix &
RCFTMCrdLinTransf3D::getLocalStiffMatrix (const Matrix &KB, const Vector &fk)
{
   static Matrix kg(18,18);     // Global stiffness for return
   static double kb[11][11];
...
RCFTMCrdLinTransf3D::getCopy3d(void)
{
  ...
  theCopy = new RCFTMCrdLinTransf3D(this->getTag(), xz);
...
int
RCFTMCrdLinTransf3D::sendSelf(int cTag, Channel &theChannel)
{
  int res = 0;
...
int
RCFTMCrdLinTransf3D::recvSelf(int cTag, Channel &theChannel, FEM_ObjectBroker &theBroker)
{
  int res = 0;
...
const Matrix &
RCFTMCrdLinTransf3D::getGlobalMatrixFromLocal(const Matrix &ml)
{
...
const Vector &
RCFTMCrdLinTransf3D::getPointGlobalCoordFromLocal(const Vector &xl)
{
   static Vector xg(3);
...
const Vector &
RCFTMCrdLinTransf3D::getPointGlobalDisplFromBasic (double xi, const Vector &uxb)
{
   return 0;
}
...
void
RCFTMCrdLinTransf3D::Print(OPS_Stream &s, int flag)
{
   s << "\nCrdTransf: " << this->getTag() << " Type: RCFTMLinCrdTransf3D" << endln;
}
