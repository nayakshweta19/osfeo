...
while {$j < [expr $nH+1]} {
	if {$j < [expr $nH] } {
		...
		puts $modelFile "node [expr $nodeStartID+$j*2+1] [expr $LB] [expr $deltaH*$j]"
	} else {
		set i 0;
		while {$i < [expr $nL+1]} {
			node [expr $nodeStartID+$j*2+$i] [expr $i*$deltaLB] [expr $H]
			puts $modelFile "node [expr $nodeStartID+$j*2+$i] [expr $i*$deltaLB] [expr $H]"
			set i [expr $i+1]
		}
	}
	set j [expr $j+1]
...
while {$i < [expr $nL]} {
	...
	puts $modelFile "element nonlinearBeamColumn [expr $eleStartID+$j*2+$i] [expr $nodeStartID+$j*2+$i] [expr $nodeStartID+$j*2+1+$i] $np 2 2 -iter $iterNum $iterTol"
	set i [expr $i+1]
}
# ------------------------------------------
...
puts $modelFile "node 42 -500.0 699.0"
puts $modelFile "node 43 -250.0 699.0"
puts $modelFile "node 44 1530.0 699.0"
puts $modelFile "node 45 1780.0 699.0"
...
puts $modelFile "element nonlinearBeamColumn 31 42 43 $np 2 2 -iter $iterNum $iterTol"
puts $modelFile "element nonlinearBeamColumn 32 43 35 $np 2 2 -iter $iterNum $iterTol"
puts $modelFile "element nonlinearBeamColumn 33 41 44 $np 2 2 -iter $iterNum $iterTol"
puts $modelFile "element nonlinearBeamColumn 34 44 45 $np 2 2 -iter $iterNum $iterTol"
# ------------------------------------------------------
...
puts $modelFile "nDMaterial FAReinforceConcretePlaneStress 15 0.0 11 12 13 14 [expr 0.25*$pi] [expr 0.75*$pi] $rou1 $rou2 $wfc $wfy $wE $ec"
# --------------------------------------------------------
