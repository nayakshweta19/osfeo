...
#    CornerBarArea - cross-sectional area of corner reinforcing bars
#    MiddleBarArea - cross-sectional area of middle reinforcing bars
...
# 
proc RCColumnsection {id h b cover coreID coverID steelID numBars CornerBarArea MiddleBarArea nfCoreY nfCoreZ nfCoverY nfCoverZ} {
   ...
   section fiberSec $id {
	...
	# Define the Corner steel bars
	layer straight $steelID 2 $CornerBarArea $ncoreY $coreZ $ncoreY $ncoreZ
	layer straight $steelID 2 $CornerBarArea $coreY $coreZ $coreY $ncoreZ

	# Determine the spacing for the remaining bars in the y direction
	set spacingZ [expr ($coreZ-$ncoreZ)/($numBars-1)]
	set spacingY [expr ($coreY-$ncoreY)/($numBars-1)]

	# Avoid double counting bars
	set numBars [expr $numBars-2]

	# Define remaining steel in the z direction
	layer straight $steelID $numBars $MiddleBarArea $ncoreY [expr $coreZ-$spacingZ] $ncoreY [expr $ncoreZ+$spacingZ]
	layer straight $steelID $numBars $MiddleBarArea $coreY [expr $coreZ-$spacingZ] $coreY [expr $ncoreZ+$spacingZ]

	# Define remaining steel in the y direction
	layer straight $steelID $numBars $MiddleBarArea [expr $coreY-$spacingY] $coreZ [expr $ncoreY+$spacingY] $coreZ
	layer straight $steelID $numBars $MiddleBarArea [expr $coreY-$spacingY] $ncoreZ [expr $ncoreY+$spacingY] $ncoreZ
   }
