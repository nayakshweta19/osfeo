 ...
 */
static __OCC_INLINE cudaOccError cudaOccPartitionsPerMultiprocessor(int *limit, const cudaOccDeviceProp *properties)
{
    int value;
    ...
    int                          blockSize)
{
    ...
    status = cudaOccPartitionsPerMultiprocessor(&numPartitions, properties);
    if (status != CUDA_OCC_SUCCESS) {
        return status;
    }
    ...
    if (properties->computeMajor > 1) {
        ...
        if (properties->regsPerBlock < regsAllocatedPerCTA) {
            maxBlocks = 0;
        }
        else {
            if (regsAllocatedPerWarp > 0) {
                // Registers are allocated in each partition. The max number of
                // warps that can fit on an SM is equal to the max number of
                // warps per partition x number of partitions.
                //
                numRegsPerPartition = properties->regsPerMultiprocessor / numPartitions;
                numWarpsPerPartition = numRegsPerPartition / regsAllocatedPerWarp;
                numWarpsPerSM = numWarpsPerPartition * numPartitions;
    ...
    size_t                       dynamicSmemSize)
{
    ...
    //
    status = cudaOccMaxBlocksPerSMWarpsLimit(&ctaLimitWarps, properties, blockSize);
    if (status != CUDA_OCC_SUCCESS) {
        return status;
    }
    ...
    //
    status = cudaOccMaxBlocksPerSMRegsLimit(&ctaLimitRegs, result, properties, attributes, blockSize);
    if (status != CUDA_OCC_SUCCESS) {
        return status;
    }
