...
// Revision: A
//
// Description: This file contains the class implementation for SectionForceDeformation2d.
//
// What: "@(#) SectionForceDeformation2d.C, revA"

#include <SectionForceDeformation2d.h>
...
SectionForceDeformation2d::SectionForceDeformation2d(int tag, int classTag)
  :Material(tag,classTag), fDefault(0), sDefault(0)
{

}

SectionForceDeformation2d::~SectionForceDeformation2d()
{
  if (fDefault != 0)
    delete fDefault;
  if (sDefault != 0)
...
const Matrix&
SectionForceDeformation2d::getSectionFlexibility ()
{
...
double 
SectionForceDeformation2d::getRho(void) 
{
  return 0.0 ;
}

Response*
SectionForceDeformation2d::setResponse(const char **argv, int argc,
				     OPS_Stream &output)
{
...
const Vector &
SectionForceDeformation2d::getStressResultantSensitivity(int gradIndex, bool conditional)
{
  if (sDefault == 0)
...
const Vector &
SectionForceDeformation2d::getSectionDeformationSensitivity(int gradIndex)
{
  if (sDefault == 0)
    sDefault = new Vector (this->getOrder());
  return *sDefault;
...
const Matrix &
SectionForceDeformation2d::getSectionTangentSensitivity(int gradIndex)
{
  int order = this->getOrder();
  ...
  if (fDefault == 0) {		
    fDefault = new Matrix(order,order);
    if (fDefault == 0) {
      opserr << "SectionForceDeformation2d::getSectionTangentSensitivity -- failed to allocate matrix\n";
      exit(-1);
    }
  }
...
const Matrix &
SectionForceDeformation2d::getInitialTangentSensitivity(int gradIndex)
{
  int order = this->getOrder();
  ...
  if (fDefault == 0) {		
    fDefault = new Matrix(order,order);
    if (fDefault == 0) {
      opserr << "SectionForceDeformation2d::getInitialTangentSensitivity -- failed to allocate matrix\n";
      exit(-1);
    }
  }
...
const Matrix&
SectionForceDeformation2d::getSectionFlexibilitySensitivity(int gradIndex)
{
  int order = this->getOrder();
  ...
  if (fDefault == 0) {		
    fDefault = new Matrix(order,order);
    if (fDefault == 0) {
      opserr << "SectionForceDeformation2d::getSectionFlexibilitySensitivity -- failed to allocate matrix\n";
      exit(-1);
    }
  }
...
const Matrix&
SectionForceDeformation2d::getInitialFlexibilitySensitivity(int gradIndex)
{
  int order = this->getOrder();
  ...
  if (fDefault == 0) {		
    fDefault = new Matrix(order,order);
    if (fDefault == 0) {
      opserr << "SectionForceDeformation2d::getInitialFlexibilitySensitivity -- failed to allocate matrix\n";
      exit(-1);
    }
  }
...
double
SectionForceDeformation2d::getRhoSensitivity(int gradIndex)
{
  return 0.0;
}

int
SectionForceDeformation2d::commitSensitivity(const Vector& defSens,
