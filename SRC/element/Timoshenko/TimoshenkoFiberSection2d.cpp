// $Source: /usr/local/cvs/OpenSees/SRC/element/Timoshenko/TimoshenkoFiberSection2d.cpp,v $
// $Revision: 1.1 $
// $Date: 2009/01/10 21:22:20 $

// Created: 09/09
// Modified by: Li Ning 
// Description: This file contains the class implementation of TimoshenkoFiberSection2d.Based on FiberSection2d.cpp.
...
#include <Fiber.h>
#include <classTags.h>
#include "TimoshenkoFiberSection2d.h"
#include <ID.h>
...
ID TimoshenkoFiberSection2d::code(2);			

// constructors:
TimoshenkoFiberSection2d::TimoshenkoFiberSection2d(int tag, int num, Fiber **fibers, int Hnum, Fiber **Hfibers, int strip1, double t1, int strip2, double t2, int strip3, double t3): 
  SectionForceDeformation(tag, SEC_TAG_TimoshenkoFiberSection2d),
  ...
  iterFile(0),exf(0),e1f(0),e2f(0),eyf(0),sxf(0),s1f(0),s2f(0),syf(0)
{
    ...
    if (theMaterials1 == 0) {
      opserr << "TimoshenkoFiberSection2d::TimoshenkoFiberSection2d -- failed to allocate Material pointers";
      exit(-1);
    }
    ...
    if (matData == 0) {
      opserr << "TimoshenkoFiberSection2d::TimoshenkoFiberSection2d -- failed to allocate double array for material data\n";
      exit(-1);
    }
	...
	opserr << "TimoshenkoFiberSection2d::TimoshenkoFiberSection2d -- failed to get copy of a Material\n";
	exit(-1);
      }
  ...
  if (numHFibers != 0) {
    ...
    if (theHMaterials == 0) {
      opserr << "TimoshenkoFiberSection2d::TimoshenkoFiberSection2d -- failed to allocate HMaterial pointers";
      exit(-1);
    }
    ...
    if (matHData == 0) {
      opserr << "TimoshenkoFiberSection2d::TimoshenkoFiberSection2d -- failed to allocate double array for Hmaterial data\n";
      exit(-1);
    }
	...
	if (theHMaterials[i*numHFibers + jj] == 0) {
	  opserr << "TimoshenkoFiberSection2d::TimoshenkoFiberSection2d -- failed to get copy of a HMaterial\n";
	  exit(-1);
	}
      }
    }    
...
TimoshenkoFiberSection2d::TimoshenkoFiberSection2d():
  SectionForceDeformation(0, SEC_TAG_TimoshenkoFiberSection2d),
...
int
TimoshenkoFiberSection2d::addFiber(Fiber &newFiber)		
{
  ...
  if (newArray1[numFibers] == 0) {
    opserr <<"TimoshenkoFiberSection2d::addFiber -- failed to get copy of a Material\n";
    delete [] newMatData;
    return -1;
  }
...
int TimoshenkoFiberSection2d::setTrialSectionDeformation (const Vector &deforms)	
{
  return 0;
}

int TimoshenkoFiberSection2d::revertToLastCommit (void)	
{
  return 0;
}

int TimoshenkoFiberSection2d::revertToStart (void)	
{
  return 0;
}
...
int
TimoshenkoFiberSection2d::commitState(void)			
{
  return 0;
}
...
const Vector&
TimoshenkoFiberSection2d::getSigmaY(void)	
{
  return *sigmaY;
}

const Vector&
TimoshenkoFiberSection2d::getTau(void)	
{
  return *tau;
}

const Vector&
TimoshenkoFiberSection2d::getAlpha(void)	
{
  return *alpha;
}

const Vector&
TimoshenkoFiberSection2d::getIter(void)	
{
  return *iterFile;
}

const Vector&
TimoshenkoFiberSection2d::getEX(void)	
{
  return *exf;
}

const Vector&
TimoshenkoFiberSection2d::getEY(void)	
{
  return *eyf;
}

const Vector&
TimoshenkoFiberSection2d::getE1(void)	
{
  return *e1f;
}

const Vector&
TimoshenkoFiberSection2d::getE2(void)	
{
  return *e2f;
}
...
const Vector&
TimoshenkoFiberSection2d::getSX(void)	
{
  return *sxf;
}

const Vector&
TimoshenkoFiberSection2d::getSY(void)	
{
  return *syf;
}

const Vector&
TimoshenkoFiberSection2d::getS1(void)
{
  return *s1f;
}

const Vector&
TimoshenkoFiberSection2d::getS2(void)	
{
  return *s2f;
...
void
TimoshenkoFiberSection2d::beta(double e0, double e2, double &sc1, double &tc1, double &tc12, double &beta)	
{
...
const Vector &
TimoshenkoFiberSection2d::getSectionDeformation (void)
{
	return e;
}

int TimoshenkoFiberSection2d::setTrialSectionDeformationB (const Vector &deforms, double L)
{
  int res = 0;
...
const Matrix&
TimoshenkoFiberSection2d::getInitialTangent(void)	
{
  kData[0] = 0.0; kData[1] = 0.0; kData[2] = 0.0; kData[3] = 0.0; kData[4] = 0.0; kData[5] = 0.0;
  kData[6] = 0.0; kData[7] = 0.0; kData[8] = 0.0;
...
const Matrix&
TimoshenkoFiberSection2d::getSectionTangent(void)	
{
...
const Vector&
TimoshenkoFiberSection2d::getStressResultant(void)	
{
  return *s;
}

SectionForceDeformation*
TimoshenkoFiberSection2d::getCopy(void)
{
  TimoshenkoFiberSection2d *theCopy = new TimoshenkoFiberSection2d ();
  ...
  if (numFibers != 0) {
    ...
    if (theCopy->theMaterials1 == 0) {
      opserr <<"TimoshenkoFiberSection2d::getCopy -- failed to allocate Material pointers\n";
      exit(-1);
    }
    ...
    if (theCopy->matData == 0) {
      opserr << "TimoshenkoFiberSection2d::getCopy -- failed to allocate double array for material data\n";
      exit(-1);
    }
	...
	opserr <<"TimoshenkoFiberSection2d::getCopy -- failed to get copy of a Material";
	exit(-1);
      }
    }  
  }
  ...
  if (numHFibers != 0) {
    ...
    if (theCopy->theHMaterials == 0) {
      opserr <<"TimoshenkoFiberSection2d::getCopy -- failed to allocate HMaterial pointers\n";
      exit(-1);
    }
    ...
    if (theCopy->matHData == 0) {
      opserr << "TimoshenkoFiberSection2d::getCopy -- failed to allocate double array for Hmaterial data\n";
      exit(-1);
    }
	...
	if (theCopy->theHMaterials[i * numHFibers + jj] == 0) {
	  opserr <<"TimoshenkoFiberSection2d::getCopy -- failed to get copy of a HMaterial";
	  exit(-1);
	}
      }
    }  
...
const ID&
TimoshenkoFiberSection2d::getType ()			
{
  return code;
}

int
TimoshenkoFiberSection2d::getOrder () const	
{
  return 3;								
}

int
TimoshenkoFiberSection2d::commitStateB(void)			
{
  int err = 0;
...
int
TimoshenkoFiberSection2d::revertToLastCommitB(double L)	
{
  int err = 0;
...
int
TimoshenkoFiberSection2d::revertToStartB(void)		
{
...
int
TimoshenkoFiberSection2d::sendSelf(int commitTag, Channel &theChannel)	
{
  int res = 0;
  ...
  if (res < 0) {
    opserr <<  "TimoshenkoFiberSection2d::sendSelf - failed to send ID data\n";
    return res;
  }    
  ...
  if (numFibers != 0) {
    ...
    if (res < 0) {
      opserr <<  "TimoshenkoFiberSection2d::sendSelf - failed to send material data\n";
      return res;
    }    
...
int
TimoshenkoFiberSection2d::recvSelf(int commitTag, Channel &theChannel,
			 FEM_ObjectBroker &theBroker)						
{
  ...
  if (res < 0) {
    opserr <<  "TimoshenkoFiberSection2d::recvSelf - failed to recv ID data\n";
    return res;
  }    
  this->setTag(data(0));
  ...
  if (data(1) != 0) {
    ...
    if (res < 0) {
      opserr <<  "TimoshenkoFiberSection2d::recvSelf - failed to recv material data\n";
      return res;
    }    
	...
	if (theMaterials1 == 0) {
	  opserr <<"TimoshenkoFiberSection2d::recvSelf -- failed to allocate Material pointers\n";
	  exit(-1);
	}
	...
	if (matData == 0) {
	  opserr <<"TimoshenkoFiberSection2d::recvSelf  -- failed to allocate double array for material data\n";
	  exit(-1);
	}
      }
    }
...
void
TimoshenkoFiberSection2d::Print(OPS_Stream &s, int flag)
{
...
Response*
TimoshenkoFiberSection2d::setResponse(const char **argv, int argc, OPS_Stream &s)	
{
...
int 
TimoshenkoFiberSection2d::getResponse(int responseID, Information &sectInfo)
{
...
// AddingSensitivity:BEGIN ////////////////////////////////////
const Vector &
TimoshenkoFiberSection2d::getSectionDeformationSensitivity(int gradNumber)
{
...
const Vector &
TimoshenkoFiberSection2d::getStressResultantSensitivity(int gradNumber, bool conditional)
{
	static Vector dummy(2);	
	return dummy;
}
...
const Matrix &
TimoshenkoFiberSection2d::getSectionTangentSensitivity(int gradNumber)
{
...
int
TimoshenkoFiberSection2d::commitSensitivity(const Vector& defSens, int gradNumber, int numGrads)
{
	return 0;
}
// AddingSensitivity:END ///////////////////////////////////
