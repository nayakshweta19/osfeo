...
// $Source: /scratch/slocal/chroot/cvsroot/openseescomp/OpenSEESComp/OpenSees/SRC/element/TclRCFTDBeamColumnCommand.cpp,v $
...
int
TclModelBuilder_addRCFTDBeamColumn(ClientData clientData, Tcl_Interp *interp,  
				   int argc, 
				   TCL_Char **argv, 
				   Domain*theTclDomain,
				   TclModelBuilder *theTclBuilder)
{
  ...
  if (ok == 0) {
    opserr << "WARNING -- NDM = " << ndm << " and NDF = " << ndf
	 << " not compatible with RCFTDBeamColumn element" << endln;
    return TCL_ERROR;
  }
  ...
  if (argc < 6) {
    ...
    opserr << "Want: element RCFTDBeamColumn eleTag? iNode? jNode? transfTag? ...\n";
    return TCL_ERROR;
  }
  ...
  if (Tcl_GetInt(interp, argv[2], &eleTag) != TCL_OK) {
    opserr << "WARNING invalid RCFTDBeamColumn eleTag" << endln;
    return TCL_ERROR;
  }
  
  if (Tcl_GetInt(interp, argv[3], &iNode) != TCL_OK) {
    opserr << "WARNING invalid iNode\n";
    opserr << "RCFTDBeamColumn element: " << eleTag << endln;
    return TCL_ERROR;
  }
  ...
  if ((strcmp(argv[6],"Lobatto") != 0)) {
    ...
    if ( theTclBuilder->getSection(secTag) == 0) {
      ...
      opserr << "\nRCFTDBeamColumn element: " << eleTag << endln;
      return TCL_ERROR;
    }
    ...
    if (ndm == 3) {
	theElement = new RCFTDBeamColumn3D(eleTag, iNode, jNode, nIP, sections,
  ...
  if (strcmp(argv[6],"Lobatto") == 0) {
    ...
    if (argc < 9) {
      opserr << "WARNING insufficient arguments\n";
      printCommand(argc, argv);
      opserr << "Want: element RCFTDBeamColumn eleTag? iNode? jNode? transfTag? Lobatto secTag? nIP?\n";
      return TCL_ERROR;
    }
