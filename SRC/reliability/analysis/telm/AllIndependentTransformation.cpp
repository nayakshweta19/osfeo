...
AllIndependentTransformation::x_to_z(const Vector &px)
{
	...
	NormalRV aStandardNormalRV(1, 0.0, 1.0);
...
//	opserr << " input something114 \n";
	for ( int i=0 ; i<nrv ; i++ )
	{
		...
		if (strcmp(theRV->getType(),"NORMAL")==0) {
			double mju = (theRV->getParameters())(0);
			double sigma = (theRV->getParameters())(1);
			(*z)(i) =   ( px(i) - mju ) / sigma;
		}
		else if (strcmp(theRV->getType(),"LOGNORMAL")==0) {
			double lambda = (theRV->getParameters())(0);
			double zeta = (theRV->getParameters())(1);
...
//	Vector DzDmean(x.Size());
	...
	if (strcmp(theRV->getType(),"NORMAL")==0) {
		double mu = (theRV->getParameters())(0);
		double sigma = (theRV->getParameters())(1);
		(*DzDmean)(rvNumber-1) = -1.0 / sigma;
	}
	else if (strcmp(theRV->getType(),"LOGNORMAL")==0) {
		double lambda = (theRV->getParameters())(0);
		double zeta = fabs((theRV->getParameters())(1)); // more here for negative lognormal?
		...
		double a = (theRV->getParameters())(0);
		double b = (theRV->getParameters())(1);
...
//	Matrix jacobianMatrix_z_x(nrv,nrv);
	for ( int i=0 ; i<nrv ; i++ )
	{
		...
		if (strcmp(theRV->getType(),"NORMAL")==0) {
			...
			double zeta = fabs((theRV->getParameters())(1));
			if (px(i) == 0.0) {
