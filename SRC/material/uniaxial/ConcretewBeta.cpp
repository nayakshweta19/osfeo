...
OPS_ConcretewBeta(void) {
  ...
  if (OPS_GetIntInput(&numData, &tag) != 0) {
    opserr << "WARNING invalid uniaxialMaterial ConcretewBeta tag" << endln;
    return 0;
  }
...
OPS_Stream &theOutput)
{
  ...
  else if (strstr(argv[0], "getPD") != 0) {
    double data = 0.0;
    theResponse = new MaterialResponse(this, 100, data);
  }
  else if (strstr(argv[0], "setWallVar") != 0) {
...
ConcretewBeta::getResponse(int responseID, Information &matInfo)
{
  ...
  case 101:
    X = (*matInfo.theVector)(0);
    K = (*matInfo.theVector)(1);
    D = (*matInfo.theVector)(2);
    beta = (*matInfo.theVector)(3);
    epslonTP = (*matInfo.theVector)(4);
    return 0;
