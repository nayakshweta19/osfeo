...
static int numSecantConcrete = 0;

void *
OPS_NewSecantConcreteMaterial(void)
{
  if (numSecantConcrete == 0) {
    numSecantConcrete++;
  }
  ...
  if (numArgs != 4 ) {
    opserr << "WARNING wrong arg number: uniaxialMaterial $tag $f $ec $eu " << endln;
    return 0;
  }
  ...
  if (OPS_GetDoubleInput(&numData, dData) != 0) {
    opserr << "WARNING invalid f, ec & eu\n";
    return 0;	
  }
   ...
   parameterID(0), SHVs(0)
{
  ...
  if ( f > 0.0 ) { f = -f; } // set f < 0
  if ( ec > 0.0 ) { ec = -ec; } // set ec < 0
  if ( eu > 0.0 ) { eu = -eu; } // set ec < 0
