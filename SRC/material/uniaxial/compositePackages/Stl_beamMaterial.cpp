...
// $Source: /scratch/slocal/chroot/cvsroot/openseescomp/OpenSEESComp/OpenSees/SRC/material/uniaxial/Stl_beamMaterial.cpp,v $
...
// cyclic uniaxial stress-strain relationship of steel girders
...
Stl_beamMaterial::Stl_beamMaterial( int tag, double Fy, double Fu, double es, double BF, double TF, double HW, double TW, double Eplo )
  :UniaxialMaterial(tag, MAT_TAG_Stl_beam), fy(Fy), fu(Fu), Ee(es), bf(BF), tf(TF), hw(HW), tw(TW), epo(Eplo)
{
       double sld;
       sld = sqrt( pow((bf/tf)*sqrt(fy/Ee),2) + pow((hw/tw)*sqrt(fy/Ee),2));
       if ( sld > 1.940 ){
         Ksft = 160;
         eplbf = 0.0159 - (fy/Ee + 0.002);
       }
       else if ( ( sld > 1.923 ) && ( sld < 1.940 ) ){
         Ksft = 3555.2 * sld - 6736.8;
         eplbf = - 1.333 * sld + 2.601 - (fy/Ee + 0.002);
       }
       else if ( sld < 1.923 ){
         Ksft = 90;
         eplbf = 0.0384 - (fy/Ee + 0.002); 
       }
       //cout<<sld<<"  "<<Ksft<<"  "<<eplbf + (fy/Ee + 0.002)<<endl;
       //Ksft = 160.0;
       //Ksft = 151.65;
       //eplbf = 0.0032 - 0.0027;
       //eplbf = ( 3.14 * ( pow ( (D/t) * sqrt(fy/Ee), -1.48 ) ) * (fy/Ee + 0.002) - (fy/Ee + 0.002) );
       //eplbf = 0.00592 - (fy/Ee + 0.002);
       //eplbf = 0.0300 - (fy/Ee + 0.002);
       //eplbf = 10.0;
       //eplbf = 0.0159 - (fy/Ee + 0.002);
       //eplbf = 0.01908 - (fy/Ee + 0.002);
       //eplbf = 0.03840 - (fy/Ee + 0.002);
       //eplbf = 0.0159 - (fy/Ee + 0.002);
       //eplbf = 0.05191 - (fy/Ee + 0.002);
       //eplbf = 0.01332 - (fy/Ee + 0.002);
       //eplbf = 0.01559 - (fy/Ee + 0.002);
       //eplbf = 0.02821 - (fy/Ee + 0.002);
       //eplbf = 0.01068 - (fy/Ee + 0.002);
       //eplbf = 0.01587 - (fy/Ee + 0.002);
       //if ( Ksft < 0.001 ){
       //  fres_n = fy * 0.9999;
       //}
       //else{
       ...
       //eresp_n = 69 * (fy/Ee + 0.002); 
       fres_n = 0.5 * fy;
       eresp_n = eplbf + (fy - fres_n)/Ksft;
       if(fy > 52){
         Rbso =  1.05 * fy;
...
Stl_beamMaterial::Stl_beamMaterial()
  :UniaxialMaterial(0,MAT_TAG_Stl_beam),
  fy(0.0), fu(0.0), Ee(0.0), bf(0.0), tf(0.0), hw(0.0), tw(0.0), Ksft(0.0), eplbf(0.0), Rbso(0.0),
...
int 
Stl_beamMaterial::setTrialStrain(double strain, double strainRate)
{ 
	  ...
	  /* IF ELASTIC LOADING TAKES PLACE RIGHT AFTER PLASTIC LOADING UPDATE STATE VARIABLES */
	      ...
	      /* IF MEMORY SURFACE IS BREACHED DETERMINE BOUNDING SURFACES */
              if ( ( strain_inc < 0.0 ) && ( memory == 0 ) ){
                  ...
                  //Tbs_n = - (Rbso/fy - (Rbso/fy - 1.0) * pow(3,5 * ebar_p)) * Rbso;
                  //Tbs_n = - ( ( -2.3826 * ebar_p ) + 1 ) * Rbso;
                  //Tbs_n = - ( -0.00808 * W + 1.0445 ) * Rbso;
                  if( buckled == 1 ){
                     Tbs_n = lbstrs;
                     if (lbstrs < - ( -0.010 * W + 1.0 ) * Rbso ){
                        Tbs_n = - ( -0.010 * W + 1.0 ) * Rbso;
                     }
                     if( Tbs_n >  -0.50 * Rbso ){
                        Tbs_n = - 0.50 * Rbso;
                     }
	      ...
	      /* IF MEMORY SURFACE IS BREACHED DETERMINE BOUNDING SURFACES */
              if ( ( strain_inc > 0.0 ) && ( memory == 0 ) ){
                  ...
                  //Tbs_p = (Rbso/fy - (Rbso/fy - 1.0) * pow(3,5 * ebar_p)) * Rbso;
                  //Tbs_p = ( ( -2.3826 * ebar_p ) + 1 ) * Rbso;
                  if( buckled == 1 ){
                    Tbs_p = ( -0.010 * W + 1.0 ) * Rbso;
                  }
                  if( Tbs_p < 0.50 * Rbso ){
                      Tbs_p = 0.50 * Rbso;
                  }
   ...
   if ( plastic == 1 ){
	  /* IMMEDIATELY AFTER PLASTIC LOADING, MEMORY SURFACE IS BREACHED, NO LOCAL BUCKLING OCCURED */  
		  ...
		  /* CHECK LOCAL BUCKLING */
                  if ( ( ep - ep_ref <= - eplbf ) && ( Tstrain < lbstrn ) && ( Tstress < lbstrs ) ){
                       lb = 1;
                       buckled = 1;
                  }
   ...
   //output<<Tstrain<<"   "<<Tstress<<"   "<<Ttangent<<"   "<<Cstrain<<"  "<<Cstress<<"  "<<Ctangent<<"  "<<Trule<<"  "<<Crule<<"   "<<Ksft<<"   "<<
   //fres_n<<"  "<<eplbf<<"  "<<eresp_n<<endl;
   //output<<Tstrain<<"   "<<Tstress<<"   "<<Ttangent<<"  "<<Cstrain<<"  "<<Cstress<<"   "<<Ctangent<<"  "<<Trule<<"  "<<Crule<<
    //       eresp_n<<"  "<<ep<<"  "<<Ep<<"  "<<ep_ref<<"  "<<lb<<"  "<<fres_n<<"  "<<Rlso<<"  "<<epo<<"  "<<Tls_n<<"  "<<Tls_p<<endl;
   //output<<Tstrain<<"   "<<Tstress<<"   "<<Ttangent<<"  "<<"eplbf "<<eplbf<<"  "<<"eresp_n "<<eresp_n<<"  "<<
   //"Rbso "<<Rbso<<"  "<<"fres_n "<<fres_n<<"  "<<"Tls_n "<<Tls_n<<endl;
//   output<<Tstrain<<"   "<<Tstress<<"   "<<Ttangent<<"   "<<Cstrain<<"  "<<Cstress<<"  "<<Ctangent<<"  "<<Trule<<"  "<<Crule<<"  "<<memory<<"  "<<elastic<<"   "<<plastic<<"  "<<lb<<"    "<<Tls_n<<"   "<<Tls_p<<"   "<<Ep<<"   "<<Epo<<"   "<<h<<"   "<<delta_in<<"  "<<ep<<"   "<<Tbs_n<<"   "<<Tbs_p<<"   "<<ebar_p<<"   "<<Rbso<<"   "<<Tvbs_n<<"    "<<Tvbs_p<<"   "<<delta<<"   "<<delta_y<<"    "<<delta_in<<"   "<<strain_inc<<"   "<<Tmem_n<<"   "<<Tmem_p<<"   "<<ep_ref<<"  "<<ld<<"  "<<Cld<<"  "<<Ksft<<" finished "<<endl;
   return 0;
}
double 
Stl_beamMaterial::getStress(void)
{
  return strs;
}

double 
Stl_beamMaterial::getTangent(void)
{
  return tgnt;
}

double 
Stl_beamMaterial::getInitialTangent(void)
{
  return Ee;
}

double 
Stl_beamMaterial::getStrain(void)
{
  return Tstrain;
}

int 
Stl_beamMaterial::commitState(void)
{
  Cstrain = Tstrain;
  Cstress = Tstress;
  Ctangent = Ttangent;
...
int 
Stl_beamMaterial::revertToLastCommit(void)
{
  Tstrain = Cstrain;
  Tstress = Cstress;
  Ttangent = Ctangent;
...
int 
Stl_beamMaterial::revertToStart(void)
{
  return lb;
}

void
Stl_beamMaterial::commitStatevar(void)
{
   CRlso = Rlso;
   CRls = Rls;
   Cls_p = Tls_p;
...
void
Stl_beamMaterial::backtocommitStatevar(void)
{
   Rlso = CRlso;
   Rls = CRls;
   Tls_p = Cls_p;
...
UniaxialMaterial *
Stl_beamMaterial::getCopy(void)
{
  Stl_beamMaterial *theCopy = new Stl_beamMaterial(this->getTag(), fy, fu, Ee, bf, tf, hw, tw, epo);
  theCopy->fy = fy; theCopy->fu = fu; theCopy->Ee = Ee; theCopy->bf = bf; theCopy->tf = tf; theCopy->hw = hw; theCopy->tw = tw;
...
int 
Stl_beamMaterial::sendSelf(int cTag, Channel &theChannel)
{
  static Vector data(104);
  data(0) = this->getTag();
  data(1) = fy; data(2) = fu; data(3) = Ee; data(4) = bf; data(5) = tf;
  ...
  data(99) = fres_n; data(100) = eresp_n; data(101) = epo; data(102) = hw; data(103) = tw;
  
  int res = theChannel.sendVector(this->getDbTag(), cTag, data);
  if(res < 0)
  opserr<<"Stl_beamMaterial::sendSelf() - failed to send data\n";
  return res;
}

int 
Stl_beamMaterial::recvSelf(int cTag, Channel &theChannel, 
			      FEM_ObjectBroker &theBroker)
{
  static Vector data(104);
  int res = theChannel.recvVector(this->getDbTag(),cTag, data);
  if(res<0) {
    opserr<<"Stl_beamMaterial::recvSelf() - failed to receive data \n";
    this->setTag(0);
  }
  else{
    this->setTag((int)data(0));
    fy = data(0);  fu = data(1); Ee = data(2); bf = data(3); tf = data(4);
    ...
    fres_n = data(99); eresp_n = data(100); epo = data(101); hw = data(102); tw = data(103);
...
void 
Stl_beamMaterial::Print(OPS_Stream &s, int flag){
  s<<"Stl_beam, tag:"<<this->getTag()<<endln;
  s<<" fy:"<<fy<<endln;
  s<<" fu:"<<fu<<endln;
  s<<" Ee: "<<Ee<<endln;
  s<<" Ksft:"<<Ksft<<endln;
