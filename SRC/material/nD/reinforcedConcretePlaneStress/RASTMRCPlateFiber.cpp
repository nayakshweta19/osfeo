...
Vector RASTMRCPlateFiber :: strain_vec(5);
Vector RASTMRCPlateFiber :: stress_vec(5);
Matrix RASTMRCPlateFiber :: tangent_matrix(5,5);

double RASTMRCPlateFiber :: epslonOne = 0.0;
double RASTMRCPlateFiber :: epslonTwo = 0.0;
double RASTMRCPlateFiber :: halfGammaOneTwo = 0.0;

double RASTMRCPlateFiber::sigmaOneC = 0.0;
double RASTMRCPlateFiber::sigmaTwoC = 0.0;

double RASTMRCPlateFiber::citaR = 0.0;      // principal strain direction
double RASTMRCPlateFiber::lastCitaR = 0.0;  // last converged principle strain direction
int    RASTMRCPlateFiber::steelStatus = 0;  // check if steel yield, 0 not yield, 1 yield
int    RASTMRCPlateFiber::dirStatus = 0; // check if principle direction has exceed 90 degree, 1 yes, 0 no
bool   RASTMRCPlateFiber::isSwapped = 0; // counter-clockwise = 0; clockwise = 1;
int    RASTMRCPlateFiber::lastDirStatus = 0; // 0, 1, 2, 3, 4, 5
double RASTMRCPlateFiber::beta = 0.0;
...
static int numRASTMRCPlateFiberMaterials = 0;

OPS_Export void *
OPS_NewRASTMRCPlateFiberMaterial()
{
  if (numRASTMRCPlateFiberMaterials == 0) {
    numRASTMRCPlateFiberMaterials++;
  }
  ...
  if (numRemainingArgs < 14) {
    opserr << "Invalid Args want: NDMaterial RASTMRCPlateFiber matTag? rho? UniaxiaMatTag1? UniaxiaMatTag2? UniaxiaMatTag3? UniaxiaMatTag4? angle1? angle2? rou1? rou2? fpc? fy? E0? epsc0?>\n";
    return 0;	
  }
  ...
  if (OPS_GetInt(&numData, &tag) != 0) {
    opserr << "WARNING invalid uniaxialMaterial RASTMRCPlateFiber tag" << endln;
    return 0;
  }

  numData = 1;
  if (OPS_GetDouble(&numData, &rho) != 0) {
    opserr << "Invalid Arg rho: uniaxialMaterial RASTMRCPlateFiber tag? fy? E0? fpc? rou? <ac?> <rc?>" << endln;
    return 0;	
  }
  ...
  //now create the RASTMRCPlateFiber
  theMaterial = new RASTMRCPlateFiber (tag, 
						     rho,
  ...
  if (theMaterial == 0) {
    ...
    opserr << "RASTMRCPlateFiber: " << tag << endln;
    return 0;
  }
...
RASTMRCPlateFiber ::RASTMRCPlateFiber (int      tag, 
							       double   RHO,
							       UniaxialMaterial *s1,
							       UniaxialMaterial *s2,
							       UniaxialMaterial *c1,
							       ...
							       double   E,
							       double   EPSC0) :
  NDMaterial(tag, ND_TAG_RASTMRCPlateFiber), 
  ...
  fpc(FPC), fy(FY), E0(E), epsc0(EPSC0)
{
  ...
  if ( theMaterial == 0 ) {
    opserr << " RASTMRCPlateFiber::RASTMRCPlateFiber - failed allocate material array\n";
    exit(-1);
  }
  ...
  // Check allocation    
  if ( theMaterial[0] == 0 ) {
    opserr << " RASTMRCPlateFiber::RASTMRCPlateFiber - failed to get a copy for steel1\n";
    exit(-1);
  }
  ...
  // Check allocation    
  if ( theMaterial[1] == 0 ) {
    opserr << " RASTMRCPlateFiber::RASTMRCPlateFiber - failed to get a copy for steel2\n";
    exit(-1);
  }
  ...
  // Check allocation    
  if ( theMaterial[2] == 0 ) {
    opserr << " RASTMRCPlateFiber::RASTMRCPlateFiber - failed to get a copy for concrete1\n";
		exit(-1);
  }
  ...
  theMaterial[3] = c2->getCopy();	
	// Check allocation    
  if ( theMaterial[3] == 0 ) {
    opserr << " RASTMRCPlateFiber::RASTMRCPlateFiber - failed to get a copy for concrete2\n";
    exit(-1);
  }
  ...
  if (theResponses == 0) {
    opserr << " RASTMRCPlateFiber::RASTMRCPlateFiber - failed allocate responses  array\n";
    exit(-1);
  }
  ...
  if ((theResponses[0] == 0) || (theResponses[1] == 0) ||
    ...
    opserr << " RASTMRCPlateFiber::RASTMRCPlateFiber - failed to set appropriate materials tag: " << tag << "\n";
    exit(-1);
  }
...
RASTMRCPlateFiber::RASTMRCPlateFiber()
 :NDMaterial(0, ND_TAG_RASTMRCPlateFiber)
{
  theMaterial = 0;
  theResponses = 0;
  determineTrialStress();
...
double 
RASTMRCPlateFiber::getRho(void)
{
  return rho;
}

int 
RASTMRCPlateFiber::setTrialStrain(const Vector &v)
{
...
int 
RASTMRCPlateFiber::setTrialStrain(const Vector &v, const Vector &r)
{
  opserr << "error: RASTMRCPlateFiber::setTrialStrain(&v, &r) -- not really responsibility" << endln;
  return 0;
}

int 
RASTMRCPlateFiber::setTrialStrainIncr(const Vector &v)
{
  opserr << "error: RASTMRCPlateFiber::setTrialStrainIncr(&v) -- not really responsibility" << endln;
  return 0;
}

int 
RASTMRCPlateFiber::setTrialStrainIncr(const Vector &v, const Vector &r)
{
  opserr << "error: RASTMRCPlateFiber::setTrialStrainIncr(&v, &r) -- not really responsibility" << endln;
  return 0;
}

const Matrix& 
RASTMRCPlateFiber::getTangent(void)
{
  return tangent_matrix;
}

const Vector& 
RASTMRCPlateFiber::getStress(void)
{
  return stress_vec;
}

const Vector& 
RASTMRCPlateFiber :: getStrain()
{
  return strain_vec;
}

const Vector& 
RASTMRCPlateFiber::getCommittedStress(void)
{
  return stress_vec;
}

const Vector& 
RASTMRCPlateFiber::getCommittedStrain(void)
{
  return strain_vec;
}

int 
RASTMRCPlateFiber::commitState(void)
{
  for (int i=0; i<4; i++)
...
int 
RASTMRCPlateFiber::revertToLastCommit(void)
{
  for (int i = 0; i < 4; i++)
...
int 
RASTMRCPlateFiber::revertToStart(void)
{
  int i;
  for (i=0; i<4; i++)
...
NDMaterial* 
RASTMRCPlateFiber::getCopy(void)
{
  RASTMRCPlateFiber* theCopy =
    new RASTMRCPlateFiber( this->getTag(), 
				           rho, 
                           theMaterial[0], 
                           theMaterial[1], 
                           theMaterial[2], 
...
NDMaterial* 
RASTMRCPlateFiber::getCopy(const char *type)
{
  RASTMRCPlateFiber* theModel =
    new RASTMRCPlateFiber( this->getTag(), 
				       rho, 
                       theMaterial[0], 
                       theMaterial[1], 
...
//end by LN
void 
RASTMRCPlateFiber::Print(OPS_Stream &s, int flag )
{
  s << "\n\tRASTMRCPlateFiber, material id: " << this->getTag() << endln;
...
int 
RASTMRCPlateFiber::sendSelf(int commitTag, Channel &theChannel)
{
  int res = 0;
  ...
  if (res < 0) {
    opserr << "WARNING RASTMRCPlateFiber::sendSelf() - " << this->getTag() << " failed to send Vector\n";
    return res;
  }	      
  ...
  for (i=0; i<4; i++) {
    ...
    if (res < 0) {
       opserr << "WARNING RASTMRCPlateFiber::sendSelf() - " << this->getTag() << " failed to send ID\n";
       return res;
	}
    ...
    for (i = 0; i < 4; i++) {
      ...
      if (res < 0) {
        opserr << "RASTMRCPlateFiber::sendSelf() - " << this->getTag() << " failed to send its Material\n";
        return res;
      }
    }
...
int 
RASTMRCPlateFiber::recvSelf(int commitTag, Channel &theChannel, FEM_ObjectBroker &theBroker)
{
  int res = 0;
  ...
  if (res < 0) {
    opserr << "WARNING RASTMRCPlateFiber::recvSelf() - failed to receive Vector\n";
    return res;
  }
  ...
  if (res < 0) {
    opserr << "WARNING RASTMRCPlateFiber::recvSelf() - " << this->getTag() << " failed to receive ID\n";
    return res;
  }
  ...
  if (theMaterial == 0) {
    ...
    theMaterial = new UniaxialMaterial *[4];
    if (theMaterial == 0) {
      opserr << "RASTMRCPlateFiber::recvSelf() - Could not allocate UniaxialMaterial* array\n";
      return -1;
    }
    for (int i = 0; i < 4; i++) {
      ...
      if (theMaterial[i] == 0) {
	     opserr << "RASTMRCPlateFiber::recvSelf() - Broker could not create NDMaterial of class type " << matClassTag << endln;
	     return -1;
      }
      ...
      if (res < 0) {
         opserr << "RASTMRCPlateFiber::recvSelf() - material " << i << "failed to recv itself\n";
	     return res;
      }
    }
  }
  ...
  // materials exist , ensure materials of correct type and recvSelf on them
    ...
    for (int i = 0; i < 4; i++) {
      ...
      if (theMaterial[i]->getClassTag() != matClassTag) {
	     ...
	     theMaterial[i] = theBroker.getNewUniaxialMaterial(matClassTag);
	     if (theMaterial[i] == 0) {
            opserr << "RASTMRCPlateFiber::recvSelf() - material " << i << "failed to create\n";
	        return -1;
		 }
	  }
...
int 
RASTMRCPlateFiber::determineTrialStress(void)
{
  ...
  if (fabs(Tstrain[0] - Tstrain[1]) < SMALL_STRAIN && fabs(Tstrain[2]) > SMALL_STRAIN) {
    ...
    if (fabs(Tstrain[2]) < SMALL_STRAIN)                    { citaR = 0;                   dirStatus = 1; }
      ...
      opserr << "RASTMRCPlateFiber::determineTrialStress: Failure to calculate citaR\n";
  ...
  tempStrain[3] = Tstrain[3];
  tempStrain[4] = Tstrain[4];
  ...
  /*if ((epslonOne > 0.0) && (epslonTwo > 0.0))   {
    ...
    opserr << "RASTMRCPlateFiber::determineTrialStress: failure to get Hsu/Zhu ratio!\n";
  }*/
  ...
  //if (v12*v21 == 1.0) {
  //  opserr << "RASTMRCPlateFiber::determineTrialStress: failure to get matrix [V]!\n";
  ...
  */
  ...
  // add for converge check
  if (D[2][2] < 0.01) { D[2][2] = 0.01; } // For some cases there is negative stiffness in shear
  ...
  Tstress[0] = pow(cos(citaR), 2)*sigmaOneC + pow(sin(citaR), 2)*sigmaTwoC
             - 2 * sin(citaR)*cos(citaR)*halfGammaOneTwo*GOneTwoC
             + pow(cos(citaL), 2)*rou1*stressSL + pow(cos(citaT), 2)*rou2*stressST;

  Tstress[1] = pow(sin(citaR), 2)*sigmaOneC + pow(cos(citaR), 2)*sigmaTwoC
             + 2 * sin(citaR)*cos(citaR)*halfGammaOneTwo*GOneTwoC
             + pow(sin(citaL), 2)*rou1*stressSL + pow(sin(citaT), 2)*rou2*stressST;

  Tstress[2] = cos(citaR)*sin(citaR)*sigmaOneC - cos(citaR)*sin(citaR)*sigmaTwoC
...
void
RASTMRCPlateFiber::determineConcreteStatus(int tempStatus)
{
  bool temp = 0;
