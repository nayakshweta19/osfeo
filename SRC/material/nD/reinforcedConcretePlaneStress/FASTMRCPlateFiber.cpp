...
Vector FASTMRCPlateFiber::strain_vec(5);
Vector FASTMRCPlateFiber::stress_vec(5);
Matrix FASTMRCPlateFiber::tangent_matrix(5, 5);

double FASTMRCPlateFiber::epslonOne = 0.0;
double FASTMRCPlateFiber::epslonTwo = 0.0;
double FASTMRCPlateFiber::halfGammaOneTwo = 0.0;

double FASTMRCPlateFiber::sigmaOneC = 0.0;
double FASTMRCPlateFiber::sigmaTwoC = 0.0;

double FASTMRCPlateFiber::citaR = 0.0;      // principal strain direction
double FASTMRCPlateFiber::lastCitaR = 0.0;  // last converged principle strain direction
int    FASTMRCPlateFiber::steelStatus = 0;  // check if steel yield, 0 not yield, 1 yield
int    FASTMRCPlateFiber::dirStatus = 0; // check if principle direction has exceed 90 degree, 1 yes, 0 no
bool   FASTMRCPlateFiber::isSwapped = 0; // counter-clockwise = 0; clockwise = 1;
int    FASTMRCPlateFiber::lastDirStatus = 0; // 0, 1, 2, 3, 4, 5
double FASTMRCPlateFiber::beta = 0.0;
...
static int numFASTMRCPlateFiberMaterials = 0;

OPS_Export void *
OPS_NewFASTMRCPlateFiberMaterial()
{
  if (numFASTMRCPlateFiberMaterials == 0) {
    numFASTMRCPlateFiberMaterials++;
  }
  ...
  if (numRemainingArgs < 14) {
    opserr << "Invalid Args want: NDMaterial FASTMRCPlateFiber matTag? rho? UniaxiaMatTag1? UniaxiaMatTag2? UniaxiaMatTag3? UniaxiaMatTag4? angle1? angle2? rou1? rou2? fpc? fy? E0? epsc0?>\n";
    return 0;	
  }
  ...
  if (OPS_GetInt(&numData, &tag) != 0) {
    opserr << "WARNING invalid tag nDMaterial FASTMRCPlateFiber tag" << endln;
    return 0;
  }

  numData = 1;
  if (OPS_GetDouble(&numData, &rho) != 0) {
    opserr << "Invalid Arg rho: uniaxialMaterial FASTMRCPlateFiber tag:" << tag << endln;
    return 0;	
  }

  numData = 4;
  if (OPS_GetInt(&numData, iData) != 0) {
    opserr << "WARNING invalid uniaxialMaterial FASTMRCPlateFiber tag: " << tag << endln;
    return 0;
  }

  numData = 8;
  if (OPS_GetDouble(&numData, dData) != 0) {
    opserr << "WARNING invalid data FASTMRCPlateFiber tag: " << tag << endln;
    return 0;
  }
  ...
  if (theUniaxialMaterial1 == 0) {
    ...
    opserr << "\nFASTMRCPlateFiber tag: " << tag << endln;
    return 0;
  }
  ...
  //now create the FASTMRCPlateFiber
  theMaterial = new FASTMRCPlateFiber (tag, 
						     rho,
  ...
  if (theMaterial == 0) {
    ...
    opserr << "FASTMRCPlateFiber: " << tag << endln;
    return 0;
  }
...
FASTMRCPlateFiber ::FASTMRCPlateFiber (int      tag, 
								   double   RHO,
								   UniaxialMaterial *s1,
								   UniaxialMaterial *s2,
								   UniaxialMaterial *c1,
								   ...
								   double   E,
								   double   EPSC0) :
  NDMaterial(tag, ND_TAG_FASTMRCPlateFiber), 
  ...
  fpc(FPC), fy(FY), E0(E), epsc0(EPSC0)
{
  ...
  if (theMaterial == 0) {
    opserr << " FASTMRCPlateFiber::FASTMRCPlateFiber - failed allocate material array\n";
    exit(-1);
  }
  ...
  // Check allocation    
  if (theMaterial[0] == 0) {
    opserr << " FASTMRCPlateFiber::FASTMRCPlateFiber - failed to get a copy for steel1\n";
    exit(-1);
  }
  ...
  // Check allocation    
  if (theMaterial[1] == 0) {
    opserr << " FASTMRCPlateFiber::FASTMRCPlateFiber - failed to get a copy for steel2\n";
    exit(-1);
  }
  ...
  // Check allocation    
  if (theMaterial[2] == 0) {
    opserr << " FASTMRCPlateFiber::FASTMRCPlateFiber - failed to get a copy for concrete1\n";
    exit(-1);
  }
  ...
  // Check allocation    
  if (theMaterial[3] == 0) {
    opserr << " FASTMRCPlateFiber::FASTMRCPlateFiber - failed to get a copy for concrete2\n";
    exit(-1);
  }
  ...
  if (theResponses == 0) {
    opserr << " FASTMRCPlateFiber::FASTMRCPlateFiber - failed allocate responses  array\n";
    exit(-1);
  }
  ...
  if ((theResponses[0] == 0) || (theResponses[1] == 0) ||
    ...
    opserr << " FASTMRCPlateFiber::FASTMRCPlateFiber - failed to set appropriate materials tag: " << tag << "\n";
    exit(-1);
  }
...
FASTMRCPlateFiber::FASTMRCPlateFiber()
 :NDMaterial(0, ND_TAG_FASTMRCPlateFiber)
{
  theMaterial = 0;
  theResponses = 0;
  determineTrialStress();
...
double 
FASTMRCPlateFiber::getRho(void)
{
	return rho;
}

int 
FASTMRCPlateFiber::setTrialStrain(const Vector &v)
{
...
int 
FASTMRCPlateFiber::setTrialStrain(const Vector &v, const Vector &r)
{
  opserr << "error: FASTMRCPlateFiber::setTrialStrainIncr(&v, &r) -- not really responsibility" << endln;
  return 0;
}

int 
FASTMRCPlateFiber::setTrialStrainIncr(const Vector &v)
{
  opserr << "error: FASTMRCPlateFiber::setTrialStrainIncr(&v) -- not really responsibility" << endln;
  return 0;
}

int 
FASTMRCPlateFiber::setTrialStrainIncr(const Vector &v, const Vector &r)
{
  opserr << "error: FASTMRCPlateFiber::setTrialStrainIncr(&v, &r) -- not really responsibility" << endln;
  return 0;
}
...
const Matrix& 
FASTMRCPlateFiber::getTangent(void)
{
  return tangent_matrix;
}

const Vector& 
FASTMRCPlateFiber::getStress(void)
{
  return stress_vec;
}

const Vector& 
FASTMRCPlateFiber :: getStrain()
{
  return strain_vec;
}
    
const Vector& 
FASTMRCPlateFiber::getCommittedStress(void)
{
  return stress_vec;
}

const Vector& 
FASTMRCPlateFiber::getCommittedStrain(void)
{
    return strain_vec;
}
    
int 
FASTMRCPlateFiber::commitState(void)
{
  for (int i=0; i<4; i++)
...
int 
FASTMRCPlateFiber::revertToLastCommit(void)
{
  for (int i=0; i<4; i++)
...
int 
FASTMRCPlateFiber::revertToStart(void)
{
  int i;
  for (i=0; i<4; i++)
...
NDMaterial* 
FASTMRCPlateFiber::getCopy(void)
{
  FASTMRCPlateFiber* theCopy =
    new FASTMRCPlateFiber( this->getTag(), 
					 rho, 
					 theMaterial[0], 
					 theMaterial[1], 
					 theMaterial[2], 
...
NDMaterial* 
FASTMRCPlateFiber::getCopy(const char *type)
{
	FASTMRCPlateFiber* theModel =
		new FASTMRCPlateFiber( this->getTag(), 
                              rho, 
                              theMaterial[0], 
                              theMaterial[1], 
...
//end by LN
void 
FASTMRCPlateFiber::Print(OPS_Stream &s, int flag )
{
  s << "\n\tFASTMRCPlateFiber, material id: " << this->getTag() << endln;
  ...
  s << "\t Principal Strain: citaStrain = " << citaStrain / PI*180.0 << endln;
  s << "\t Principal Stress: citaStress = " << citaStress / PI*180.0 << endln;
...
int 
FASTMRCPlateFiber::sendSelf(int commitTag, Channel &theChannel)
{
  int res = 0;
  ...
  if (res < 0) {
    opserr << "WARNING FASTMRCPlateFiber::sendSelf() - " << this->getTag() << " failed to send Vector\n";
    return res;
  }	      
  ...
  if (res < 0) {
    opserr << "WARNING FASTMRCPlateFiber::sendSelf() - " << this->getTag() << " failed to send ID\n";
    return res;
  }
  ...
  for (i = 0; i < 4; i++) {
    ...
    if (res < 0) {
      opserr << "FASTMRCPlateFiber::sendSelf() - " << this->getTag() << " failed to send its Material\n";
      return res;
    }
  }	
...
int 
FASTMRCPlateFiber::recvSelf(int commitTag, Channel &theChannel, FEM_ObjectBroker &theBroker)
{
  int res = 0;
  ...
  if (res < 0) {
    opserr << "WARNING FASTMRCPlateFiber::recvSelf() - failed to receive Vector\n";
    return res;
  }
  ...
  if (res < 0) {
    opserr << "WARNING FASTMRCPlateFiber::recvSelf() - " << this->getTag() << " failed to receive ID\n";
    return res;
  }
  ...
  if (theMaterial == 0) {
    ...
    theMaterial = new UniaxialMaterial *[4];
    if (theMaterial == 0) {
      opserr << "FASTMRCPlateFiber::recvSelf() - Could not allocate UniaxialMaterial* array\n";
      return -1;
    }
    for (int i = 0; i < 4; i++) {
      ...
      if (theMaterial[i] == 0) {
	     opserr << "FASTMRCPlateFiber::recvSelf() - Broker could not create NDMaterial of class type " << matClassTag << endln;
	     return -1;
      }
      ...
      if (res < 0) {
         opserr << "FASTMRCPlateFiber::recvSelf() - material " << i << "failed to recv itself\n";
	     return res;
      }
    }
  }
  ...
  // materials exist , ensure materials of correct type and recvSelf on them
    ...
    for (int i = 0; i < 4; i++) {
      ...
      if (theMaterial[i]->getClassTag() != matClassTag) {
	     ...
	     theMaterial[i] = theBroker.getNewUniaxialMaterial(matClassTag);
	     if (theMaterial[i] == 0) {
            opserr << "FASTMRCPlateFiber::recvSelf() - material " << i << "failed to create\n";
	        return -1;
		 }
	  }
...
int 
FASTMRCPlateFiber::determineTrialStress(void)
{ 
  ...
  if ( status == 0 ) { // does not get converged after iteration
    ...
    //if ( minError > 0.05 )
      opserr << "FASTMRCPlateFiber::determineTrialStress: Warning, failure to get converged principal stress direction\n";
  } 
...
double 
FASTMRCPlateFiber::getAngleError(double inputCita)
{
...
double 
FASTMRCPlateFiber::getPrincipalStressAngle(double inputAngle)
{   
    ...
    */
    ...
    if ((epslonOne > 0.0) && (epslonTwo > 0.0)) {
      ...
      opserr << "FASTMRCPlateFiber::getPrincipalStressAngle: failure to get Hsu/Zhu ratio!\n";
    }
    ...
    //if (v12*v21 == 1.0) {
    //  opserr << "FASTMRCPlateFiber::getPrincipalStressAngle: failure to get matrix [V]!\n";
    ...
    if (fabs(Tstress[0] - Tstress[1]) < SMALL_STRESS)         citaOut = 0.25*PI;
    else { // Tstrain(0) != Tstrain(1)
...
void
FASTMRCPlateFiber::determineConcreteStatus(int tempStatus)
{
  bool temp = 0;
