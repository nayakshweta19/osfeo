...
//Matrix FAReinforcedConcretePlaneStress::DC(3, 3);
//Matrix FAReinforcedConcretePlaneStress::DC_bar(3, 3);
//Vector FAReinforcedConcretePlaneStress::Tstrain(3);
//Vector FAReinforcedConcretePlaneStress::Tstress(3);
//Vector FAReinforcedConcretePlaneStress::lastStress(3);
//
...
//
//double FAReinforcedConcretePlaneStress::sigmaOneC = 0.0;
//double FAReinforcedConcretePlaneStress::sigmaTwoC = 0.0;
//
//double FAReinforcedConcretePlaneStress::citaR = 0.0;      // principal strain direction
//double FAReinforcedConcretePlaneStress::lastCitaR = 0.0;  // last converged principle strain direction
//int    FAReinforcedConcretePlaneStress::steelStatus = 0;  // check if steel yield, 0 not yield, 1 yield
//int    FAReinforcedConcretePlaneStress::dirStatus = 0; // check if principle direction has exceed 90 degree, 1 yes, 0 no
//int    FAReinforcedConcretePlaneStress::afterFirstIter = 0; // find the equibium principle strees angle, the first iteration
//int    FAReinforcedConcretePlaneStress::gt90 = 0; // angle great than 90 degree
//bool   FAReinforcedConcretePlaneStress::isSwapped = 0; // counter-clockwise = 0; clockwise = 1;
//int    FAReinforcedConcretePlaneStress::lastDirStatus = 0; // 0, 1, 2, 3, 4, 5
//double FAReinforcedConcretePlaneStress::beta = 0.0;
...
FAReinforcedConcretePlaneStress::FAReinforcedConcretePlaneStress(int      tag,
  ...
  lastStress(3), loaded(false), lastCitaR(0)
{
  ...
  if (theResponses == 0) {
    opserr << " FAReinforcedConcretePlaneStress::FAReinforcedConcretePlaneStress - failed allocate responses  array\n";
    exit(-1);
  }
  ...
  if ((theResponses[0] == 0) || (theResponses[1] == 0) ||
    ...
    opserr << " FAReinforcedConcretePlaneStress::FAReinforcedConcretePlaneStress - failed to set appropriate materials tag: " << tag << "\n";
    exit(-1);
  }
...
double
FAReinforcedConcretePlaneStress::getRho(void)
{
  return rho;
}

int
FAReinforcedConcretePlaneStress::setTrialStrain(const Vector &v)
{
...
{
  opserr << "error: FAReinforcedConcretePlaneStress::setTrialStrainIncr(&v, &r) -- not really responsibility" << endln;
  return 0;
}
...
{
  opserr << "error: FAReinforcedConcretePlaneStress::setTrialStrainIncr(&v) -- not really responsibility" << endln;
  return 0;
}
...
const Vector&
FAReinforcedConcretePlaneStress::getStress(void)
{
  return stress_vec;
}

const Vector&
FAReinforcedConcretePlaneStress::getStrain()
{
  return strain_vec;
}

const Vector&
FAReinforcedConcretePlaneStress::getCommittedStress(void)
{
  return stress_vec;
}

const Vector&
FAReinforcedConcretePlaneStress::getCommittedStrain(void)
{
  return strain_vec;
}

int
FAReinforcedConcretePlaneStress::commitState(void)
{
  for (int i = 0; i < 4; i++)
  ...
  //opserr << buffer;
  sprintf_s(buffer, "T1=%7.5f T2=%7.5f T12=%7.5f; e1=%7.5f e2=%7.5f g12/2=%7.5f; ce1=%7.5f ce2=%7.5f; ThetaE=%4.2f ThetaS=%4.2f beta=%4.2f DC0=%7.1f DC1=%7.1f Dc_bar0=%7.1f Dc_bar1=%7.1f E00=%7.1f E11=%7.1f E22=%7.1f\n",
    Tstrain[0], Tstrain[1], Tstrain[2], epslonOne, epslonTwo, halfGammaOneTwo, theMaterial[2]->getStrain(), theMaterial[3]->getStrain(),
    citaStrain / PI * 180, citaStress / PI * 180, beta / PI * 180, DC(0, 0), DC(1, 1), DC_bar(0, 0), DC_bar(1, 1), tangent_matrix(0, 0), tangent_matrix(1, 1), tangent_matrix(2, 2));
  //dir=%d swap=%d //dirStatus, int(isSwapped), //90. / PI*atan2(2.0*Tstrain[2], Tstrain[1] - Tstrain[0]), 90. / PI*atan2(2.0*Tstress[2], Tstress[1] - Tstress[0]));
  opserr << buffer;
#endif
...
int
FAReinforcedConcretePlaneStress::revertToLastCommit(void)
{
  for (int i = 0; i < 4; i++)
...
NDMaterial*
FAReinforcedConcretePlaneStress::getCopy(void)
{
...
NDMaterial*
FAReinforcedConcretePlaneStress::getCopy(const char *type)
{
  FAReinforcedConcretePlaneStress* theModel =
    new FAReinforcedConcretePlaneStress(this->getTag(),
    rho, theMaterial[0], theMaterial[1], theMaterial[2], theMaterial[3], angle1, angle2, rouL, rouT, fpc, fy, E0, epsc0);
  theModel->strain_vec = strain_vec;
  theModel->stress_vec = stress_vec;
  theModel->lastCitaR = lastCitaR;
  theModel->loaded = loaded;
...
//added by Ln
Response*
FAReinforcedConcretePlaneStress::setResponse(const char **argv, int argc, OPS_Stream &output)
{

#ifdef _G3DEBUG
  opserr << "FAReinforcedConcretePlaneStress::setResponse(...)" << endln;
#endif
...
int
FAReinforcedConcretePlaneStress::getResponse(int responseID, Information &matInfo)
{
#ifdef _G3DEBUG
  opserr << "FAReinforcedConcretePlaneStress::getResponse(...)" << endln;
#endif
...
void
FAReinforcedConcretePlaneStress::Print(OPS_Stream &s, int flag)
{
...
int
FAReinforcedConcretePlaneStress::sendSelf(int commitTag, Channel &theChannel)
{
  int res = 0;
...
int
FAReinforcedConcretePlaneStress::recvSelf(int commitTag, Channel &theChannel, FEM_ObjectBroker &theBroker)
{
  int res = 0;
...
int
FAReinforcedConcretePlaneStress::determineTrialStress(void)
{
  ...
  if (Tstrain.Norm() > 0. && loaded == false) loaded = true; // determine whether the material is loaded before
  ...
  if ((eYPoint[0] == eXPoint[0]) && (eYPoint[1] == eXPoint[1])) {
    if (loaded && (Tstrain.Norm() > 0.)) {
      opserr << "FAReinforcedConcretePlaneStress::determineTrialStress: Failure to calculate the pole point of strain\n";
    }
    else if (loaded && (Tstrain.Norm() == 0.)) {
      opserr << "FAReinforcedConcretePlaneStress::determineTrialStress: zero strain loading\n";
      citaR = 0.;
    }
  }
  else {
...
//    else if ((Tstrain(0) >= Tstrain(1)) && (Tstrain(2) <  0))                 dirStatus = 2;
...
//    }
  ...
  //temp_citaR = atan2(2.0e6*Tstrain[2] , 1.0e6*Tstrain[0] - 1.0e6*Tstrain[1]);
  //if (dirStatus == 0 && temp_citaR < 0) gt90 = -1;

  citaStrain = citaR;

  int status = 0; // status to check if iteration for principal stress direction
  double tolerance = 0.0088; // tolerance for iteration, equals to 0.25 degree
  int iteration_counter = 0;
  double error;

  error = fabs(getAngleError(citaR)); // first try citaR
  if (error < tolerance)
    status = 1;
  //afterFirstIter = 1;
  ...
  double minError  = citaR;
  double citaFinal = citaR;
  double citaLow   = citaStrain - PI/2.0;
  double citaHigh  = citaStrain + PI/2.0;
  if ( citaLow < -0.5 * PI ) citaLow = -0.5 * PI;
  if ( citaHigh > 0.5 * PI ) citaHigh = 0.5 * PI;

  while ((status == 0) && ( citaOne > citaLow || citaTwo < citaHigh)) {
    citaOne = citaOne - PI / 360.0;
    citaTwo = citaTwo + PI / 360.0;

    if (citaOne > citaLow) {
      error = fabs(getAngleError(citaOne));
      if (minError > error) {
        minError = error;
        citaFinal = citaOne;
      }
    ...
    if (status == 0 && citaTwo < citaHigh) {
      error = fabs(getAngleError(citaTwo));
      if (minError > error) {
        minError = error;
        citaFinal = citaTwo;
      }
  ...
  if (status == 0) { // does not get converged after iteration
    ...
    while ((status == 0) && (citaOne > citaLow || citaTwo < citaHigh)) {
      ...
      if (citaOne > citaLow) {
        ...
        opserr << "\tcitaE1 = " << citaOne / PI * 180 << "; citaS = " << citaStress / PI * 180 << "; error = " << error / PI * 180 << "; citaFinal = " << citaFinal / PI * 180 << endln;
      }
      ...
      if (status == 0 && citaTwo < citaHigh) {
        ...
        opserr << "\tcitaE2 = " << citaTwo / PI * 180 << "; citaS = " << citaStress / PI * 180 << "; error = " << error / PI * 180 << "; citaFinal = " << citaFinal / PI * 180 << endln;
      }
    }

    error = fabs(getAngleError(citaFinal));
    // if ( minError > 0.05 )
    opserr << "FAReinforcedConcretePlaneStress::determineTrialStress: Warning, failure to get converged principal stress. error="
      << error / PI * 180 << endln;
  }
...
double
FAReinforcedConcretePlaneStress::getAngleError(double inputCita)
{
...
double
FAReinforcedConcretePlaneStress::getPrincipalStressAngle(double inputAngle)
{
  double citaIn = inputAngle; // Trial principal stress direction, obtained from input

//  if (dirStatus == 0) {
//  //  if (temp_citaR < 0) citaIn = -0.25*PI;
//  }
//  else if (dirStatus == 2) {
//  //  citaIn = 0.5*PI - inputAngle;
//  }
//  else if (dirStatus == 3) {
//  //  citaIn = PI - inputAngle;
//  }
//  else if (dirStatus == 4) {
//  //  citaIn = 0.5*PI - inputAngle;
//  }
//  else if (dirStatus == 5) {
//  //  citaIn = inputAngle - 0.5*PI;
//  }
  ...
  double rou = (rouL < rouT ? rouL : rouT);
...
//  else                halfGammaOneTwo = 0.0; // for minor error in the angle calculation
  ...
  //For FA - STM theory the ratio contains I matrix
  ...
  V.Zero();
  if (miu12*miu21 == 1.0) {
    opserr << "FAReinforcedConcretePlaneStress::determineTrialStress: failure to get matrix [V]!\n";
...
//  }
  ...
  DC_bar(2, 1) = 0.0;
  //DC_bar(2, 2) = GOneTwoC;
  DC_bar(2, 2) = .25*(DC_bar(0,0) + DC_bar(1,1) - .4*sqrt(DC_bar(0,0)*DC_bar(1,1)))/(1-.2*.2); // 0.25*(E1+E2-2*niu*sqrt(E1*E2)/(1-niu^2)
  ...
  Tstress(0) += pow(cos(citaL), 2)*rouL*stressSL + pow(cos(citaT), 2)*rouT*stressST;
  Tstress(1) += pow(sin(citaL), 2)*rouL*stressSL + pow(sin(citaT), 2)*rouT*stressST;
  Tstress(2) += cos(citaL)*sin(citaL)*rouL*stressSL + cos(citaT)*sin(citaT)*rouT*stressST;
  ...
  if ((sYPoint[0] == sXPoint[0]) && (sYPoint[1] == sXPoint[1])) {
    if (loaded)
      opserr << "FAReinforcedConcretePlaneStress::determineTrialStress: Failure to calculate the pole point of stress\n";
  }
  else {
    try
    {
...
/*void
FAReinforcedConcretePlaneStress::determineConcreteStatus(int nowStatus)
{
  bool temp = 0;
