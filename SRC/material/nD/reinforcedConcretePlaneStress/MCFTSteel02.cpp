...
// MCFTSteel02, Based on Steel03


#include "MCFTSteel02.h"
#include <Vector.h>
#include <Matrix.h>
#include <Channel.h>
...
OPS_Export void *
OPS_NewMCFTSteel02Material(void)
{
  ...
  if (numRemainingArgs < 4 || numRemainingArgs > 8) {
    opserr << "Invalid Args want: uniaxialMaterial MCFTSteel02 tag? fy? E0? b? "
  ...
  if (OPS_GetIntInput(&numData, iData) != 0) {
    opserr << "WARNING invalid uniaxialMaterial MCFTSteel02 tag" << endln;
    return 0;
  }
  ...
  if (numRemainingArgs == 3) {
    if (OPS_GetDoubleInput(&numRemainingArgs, dData) != 0) {
      opserr << "Invalid Args want: uniaxialMaterial MCFTSteel02 tag? fy? E? b? " << endln;
      return 0;	
    } else {
      ...
      a1=0;
      a2=0;
      theMaterial = new MCFTSteel02(iData[0], dData[0], dData[1], dData[2], r, coeffR1,coeffR2, a1, a2);
	}
  } else if (numRemainingArgs == 8) {
    if (OPS_GetDoubleInput(&numRemainingArgs, dData) != 0) {
      opserr << "Invalid Args want: uniaxialMaterial MCFTSteel02 tag? fy? E? b? "
		     << "r? coeffR1? coeffR2? a1? a2? " << endln;
      return 0;	
    } else
      theMaterial = new MCFTSteel02(iData[0], dData[0], dData[1], dData[2], dData[3], dData[4], dData[5], dData[6], dData[7]);
  } else {
	return 0;
  }

  if (theMaterial == 0) {
    opserr << "WARNING could not create uniaxialMaterial of type MCFTSteel02\n";
    return 0;
  }
...
MCFTSteel02::MCFTSteel02(void):
UniaxialMaterial(0, MAT_TAG_MCFTSteel02)
{

}

MCFTSteel02::~MCFTSteel02()
{
  if (SHVs != 0)  delete SHVs;
}

UniaxialMaterial*
MCFTSteel02::getCopy(void)
{
   MCFTSteel02* theCopy = new MCFTSteel02(this->getTag(), fy, E0, b, r0, coeffR1, coeffR2, a1, a2);
...
int
MCFTSteel02::setTrialStrain(double strain, double strainRate)
{
...
int
MCFTSteel02::setTrial (double strain, double &stress, double &tangent, double strainRate)
{
   // Reset history variables to last converged state
   TminStrain = CminStrain;
   TmaxStrain = CmaxStrain;
...
void 
MCFTSteel02::determineTrialState (double dStrain)
{
...
double 
MCFTSteel02::getStrain(void)
{
  return Tstrain;
}

double 
MCFTSteel02::getStress(void)
{
  return Tstress;
}

double 
MCFTSteel02::getTangent(void)
{
  return Ttangent;
}
...
int 
MCFTSteel02::revertToLastCommit(void)
{
...
int 
MCFTSteel02::revertToStart(void)
{
   // History variables
   CminStrain = -fy/E0;
   CmaxStrain = fy/E0;
...
int 
MCFTSteel02::sendSelf(int commitTag, Channel &theChannel)
{
   int res = 0;
   ...
   if (res < 0) 
      opserr << "MCFTSteel02::sendSelf() - failed to send data\n";

   return res;
}

int 
MCFTSteel02::recvSelf(int commitTag, Channel &theChannel, 
	     FEM_ObjectBroker &theBroker)
{
   ...
   if (res < 0) {
      opserr << "MCFTSteel02::recvSelf() - failed to receive data\n";
      this->setTag(0);      
   }
   else {
...
Response* 
MCFTSteel02::setResponse(const char **argv, int argc,
...
int 
MCFTSteel02::getResponse(int responseID, Information &matInfo)
{
...
void 
MCFTSteel02::Print(OPS_Stream &s, int flag)
{
   s << "\t\tMCFTSteel02 tag: " << this->getTag() << endln;
   s << "\t\t  fy: " << fy << " ";
   s << "\t\t  E0: " << E0 << " ";
   s << "\t\t  b: " << b << " ";
   s << "\t\t  r0:  " << r0 << endln;
...
// AddingSensitivity:BEGIN ///////////////////////////////////
int
MCFTSteel02::setParameter(const char **argv, int argc, Parameter &param)
{
	if (argc < 1)
		return -1;
	...
	if (strcmp(argv[0],"b") == 0) {
		...
		opserr << "WARNING: Could not set parameter in MCFTSteel02. " << endln;
               
	return -1;
}

int
MCFTSteel02::updateParameter(int parameterID, Information &info)
{
	switch (parameterID) {
	case -1:
		return -1;
...
int
MCFTSteel02::activateParameter(int passedParameterID)
{
...
double MCFTSteel02::getStrainSensitivity(int gradNumber){
	if (SHVs ==0) {
			opserr<<"warning:MCFTSteel02::getStrainsSensitivity, SHVs =0 "<<endln;	
			return 0.0;
		} 
...
double
MCFTSteel02::getStressSensitivity(int gradNumber, bool conditional){

/*	if (conditional==false)     {
		if (SHVs ==0) {
			opserr<<"warning:MCFTSteel02::getStressSensitivity, SHVs =0 "<<endln;	
			return 0.0;
		} 
...
//	  if ((Cloading == 0) && (dStrain ==0)) {/*opserr<<"warning: MCFTSteel02::getStressSensitivity, dStrain is zero! "<<endln; return 0.0;*/} 
...
/*
double
MCFTSteel02::getInitialTangentSensitivity(int gradNumber)
{
...
int
MCFTSteel02::commitSensitivity(double TstrainSensitivity, int gradNumber, int numGrads)
{
	if (SHVs == 0) {
...
//--------------------------------------------------------------------------
	  double strain =Tstrain;
	  double dStrain = strain - Cstrain;
//	  if (dStrain ==0){opserr<<"warning: MCFTSteel02::getStressSensitivity, dStrain is zero! "<<endln;} 
//	  if ((Cloading == 0) && (dStrain ==0)) {/*opserr<<"warning: MCFTSteel02::getStressSensitivity, dStrain is zero! "<<endln; return 0.0;*/} 
