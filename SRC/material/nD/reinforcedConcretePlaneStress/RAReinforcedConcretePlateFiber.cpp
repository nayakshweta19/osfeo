...
#include "RAReinforcedConcretePlateFiber.h"
...
static int numRAReinforcedConcretePlateFiberMaterials = 0;
...
RAReinforcedConcretePlateFiber ::RAReinforcedConcretePlateFiber (int      tag, 
							       double   RHO,
							       UniaxialMaterial *s1,
							       UniaxialMaterial *s2,
							       UniaxialMaterial *c1,
							       ...
							       double   E,
							       double   EPSC0) :
  NDMaterial(tag, ND_TAG_RAReinforcedConcretePlateFiber), 
  ...
  fpc(FPC), fy(FY), E0(E), epsc0(EPSC0), strain_vec(5), stress_vec(5),tangent_matrix(5,5)
{
  ...
  if ( theMaterial == 0 ) {
    opserr << " RAReinforcedConcretePlateFiber::RAReinforcedConcretePlateFiber - failed allocate material array\n";
    exit(-1);
  }
  ...
  // Check allocation    
  if ( theMaterial[0] == 0 ) {
    opserr << " RAReinforcedConcretePlateFiber::RAReinforcedConcretePlateFiber - failed to get a copy for steel1\n";
    exit(-1);
  }
  ...
  // Check allocation    
  if ( theMaterial[1] == 0 ) {
    opserr << " RAReinforcedConcretePlateFiber::RAReinforcedConcretePlateFiber - failed to get a copy for steel2\n";
    exit(-1);
  }
  ...
  // Check allocation    
  if ( theMaterial[2] == 0 ) {
    opserr << " RAReinforcedConcretePlateFiber::RAReinforcedConcretePlateFiber - failed to get a copy for concrete1\n";
		exit(-1);
  }
  ...
  theMaterial[3] = c2->getCopy();	
	// Check allocation    
  if ( theMaterial[3] == 0 ) {
    opserr << " RAReinforcedConcretePlateFiber::RAReinforcedConcretePlateFiber - failed to get a copy for concrete2\n";
    exit(-1);
  }
    ...
    if ( theResponses == 0) {
      opserr << " RAReinforcedConcretePlateFiber::RAReinforcedConcretePlateFiber - failed allocate responses  array\n";
      exit(-1);
    }
	...
	(theResponses[4] == 0) || (theResponses[5] == 0)) {
      opserr << " RAReinforcedConcretePlateFiber::RAReinforcedConcretePlateFiber - failed to set appropriate materials tag: " << tag << "\n";
      exit(-1);
    }
...
RAReinforcedConcretePlateFiber::RAReinforcedConcretePlateFiber()
 :NDMaterial(0, ND_TAG_RAReinforcedConcretePlateFiber), strain_vec(5),
...
double RAReinforcedConcretePlateFiber::getRho(void)
{
  return rho;
}


int RAReinforcedConcretePlateFiber::setTrialStrain(const Vector &v)
{
...
///*
int RAReinforcedConcretePlateFiber::setTrialStrain(const Vector &v, const Vector &r)
{
  opserr << "error: RAReinforcedConcretePlateFiber::setTrialStrain(&v, &r) -- not really responsibility" << endln;
  return 0;
}


int RAReinforcedConcretePlateFiber::setTrialStrainIncr(const Vector &v)
{
  opserr << "error: RAReinforcedConcretePlateFiber::setTrialStrainIncr(&v) -- not really responsibility" << endln;
  return 0;
}


int RAReinforcedConcretePlateFiber::setTrialStrainIncr(const Vector &v, const Vector &r)
{
  opserr << "error: RAReinforcedConcretePlateFiber::setTrialStrainIncr(&v, &r) -- not really responsibility" << endln;
  return 0;
}
//*/

const Matrix& RAReinforcedConcretePlateFiber::getTangent(void)
{
  return tangent_matrix;
}

const Vector& RAReinforcedConcretePlateFiber::getStress(void)
{
...
const Vector& RAReinforcedConcretePlateFiber :: getStrain()
{
  return strain_vec;
}


const Vector& RAReinforcedConcretePlateFiber::getCommittedStress(void)
{
  return stress_vec;
}


const Vector& RAReinforcedConcretePlateFiber::getCommittedStrain(void)
{
  return strain_vec;
}


int RAReinforcedConcretePlateFiber::commitState(void)
{
  for (int i=0; i<4; i++)
    {
...
int RAReinforcedConcretePlateFiber::revertToLastCommit(void)
{
...
int RAReinforcedConcretePlateFiber::revertToStart(void)
{
  int i;
  for (i=0; i<4; i++)
    {
...
NDMaterial* RAReinforcedConcretePlateFiber::getCopy(void)
{
  /*
    RAReinforcedConcretePlateFiber *clone;
    clone = new RAReinforcedConcretePlateFiber();
    *clone = *this;
    return clone;
  //*/
  
  RAReinforcedConcretePlateFiber* theCopy =
    new RAReinforcedConcretePlateFiber( this->getTag(), 
...
NDMaterial* RAReinforcedConcretePlateFiber::getCopy(const char *type)
{
  /*
    RAReinforcedConcretePlateFiber *clone;
    clone = new RAReinforcedConcretePlateFiber();
    *clone = *this;
    return clone;
  //*/
  RAReinforcedConcretePlateFiber* theModel =
    new RAReinforcedConcretePlateFiber( this->getTag(), 
				       rho, theMaterial[0], theMaterial[1], theMaterial[2], theMaterial[3], angle1, angle2, rou1, rou2, fpc, fy, E0, epsc0 );
...
void RAReinforcedConcretePlateFiber::Print(OPS_Stream &s, int flag )
{
  s << "\n\tRAReinforcedConcretePlateFiber, material id: " << this->getTag() << endln;
  /*
    s << "\tRho: " << rho << endln;
...
int RAReinforcedConcretePlateFiber::sendSelf(int commitTag, Channel &theChannel)
{
  int res = 0;
  ...
  if (res < 0) {
    opserr << "WARNING RAReinforcedConcretePlateFiber::sendSelf() - " << this->getTag() << " failed to send Vector\n";
    return res;
  }	      
  ...
  for (i=0; i<4; i++)
    ...
    if (res < 0) {
       opserr << "WARNING RAReinforcedConcretePlateFiber::sendSelf() - " << this->getTag() << " failed to send ID\n";
       return res;
	}
   ...
   for (i = 0; i < 4; i++) {
    ...
    if (res < 0) {
      opserr << "RAReinforcedConcretePlateFiber::sendSelf() - " << this->getTag() << " failed to send its Material\n";
      return res;
    }
   }	
...
int RAReinforcedConcretePlateFiber::determineTrialStress(void)
{   
  // Define i, j, k
  int i, j, k;
  ...
  if ( fabs(Tstrain[0]-Tstrain[1]) < 1e-7 )
	...
	{
	  opserr << "RAReinforcedConcretePlateFiber::determineTrialStress: Failure to calculate citaR\n";
  ...
  */
  ...
  {
    opserr << "RAReinforcedConcretePlateFiber::determineTrialStress: failure to get Hsu/Zhu ratio!\n";
  }
  ...
  if ( v12*v21==1.0 )
    {
      opserr << "RAReinforcedConcretePlateFiber::determineTrialStress: failure to get matrix [V]!\n";
  ...
  */
  ...
  for (i=0;i<3;i++)
	...
	else // Tstrain[0] != Tstrain[1]
	{
		...
		{
			opserr << "RAReinforcedConcretePlateFiber::determineTrialStress: Failure to calculate principal stress direction\n";
