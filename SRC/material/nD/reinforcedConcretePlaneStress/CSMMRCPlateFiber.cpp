...
// $Source: /usr/local/cvs/OpenSees/SRC/material/nD/CSMMRCPlateFiber.cpp,v $
// File: CSMMRCPlateFiber.cpp
//
...
//Vector CSMMRCPlateFiber :: strain_vec(3);
//Vector CSMMRCPlateFiber :: stress_vec(3);
//Matrix CSMMRCPlateFiber :: tangent_matrix(3,3);
...
static int numCSMMRCPlateFiberMaterials = 0;

OPS_Export void *
OPS_NewCSMMRCPlateFiberMaterial()
{
  if (numCSMMRCPlateFiberMaterials == 0) {
    numCSMMRCPlateFiberMaterials++;
    //OPS_Error("CSMMRCPlateFiber uniaxial material - Written by J.Zhong, Thomas T.C. Hsu and Y.L. Mo - Copyright@2009\n", 1);
  }
  ...
  if (numRemainingArgs < 14) {
    opserr << "Invalid Args want: NDMaterial CSMMRCPlateFiber matTag? rho? UniaxiaMatTag1? UniaxiaMatTag2? UniaxiaMatTag3? UniaxiaMatTag4? \
  ...
  if (OPS_GetInt(&numData, &tag) != 0) {
    opserr << "WARNING invalid nDMaterial CSMMRCPlateFiber tag" << endln;
    return 0;
  }

  numData = 1;
  if (OPS_GetDouble(&numData, &rho) != 0) {
    opserr << "WARNING invalid parameter rho CSMMRCPlateFiber tag:" << tag << endln;
    return 0;	
  }

  numData = 4;
  if (OPS_GetInt(&numData, iData) != 0) {
    opserr << "WARNING invalid nDMaterial CSMMRCPlateFiber tag: " << tag << endln;
    return 0;
  }

  numData = 8;
  if (OPS_GetDouble(&numData, dData) != 0) {
    opserr << "WARNING invalid data CSMMRCPlateFiber tag: " << tag << endln;
    return 0;
  }
  ...
  if (theUniaxialMaterial1 == 0) {
    ...
    opserr << "\nCSMMRCPlateFiber tag: " << tag << endln;
    return 0;
  }
  ...
  //now create the CSMMRCPlateFiber
  theMaterial = new CSMMRCPlateFiber (tag, 
						     rho,
  ...
  if (theMaterial == 0) {
    ...
    opserr << "CSMMRCPlateFiber: " << tag << endln;
    return 0;
  }
...
CSMMRCPlateFiber ::CSMMRCPlateFiber (int      tag, 
								   double   RHO,
								   UniaxialMaterial *s1,
								   UniaxialMaterial *s2,
								   UniaxialMaterial *c1,
								   ...
								   double   E,
								   double   EPSC0) :
  NDMaterial(tag, ND_TAG_CSMMRCPlateFiber), 
  rho(RHO), angle1(ANGLE1), angle2(ANGLE2), rou1(ROU1), rou2(ROU2),
  fpc(FPC), fy(FY), E0(E), epsc0(EPSC0), lastStress(5), Tstress(5), 
  strain_vec(5), stress_vec(5), tangent_matrix(5,5)
{
    steelStatus = 0;
    dirStatus = 0;
    G12 = 0;
    ...
    if ( theMaterial == 0 ) {
      opserr << " CSMMRCPlateFiber::CSMMRCPlateFiber - failed allocate material array\n";
      exit(-1);
    }
    ...
    // Check allocation    
    if ( theMaterial[0] == 0 ) {
      opserr << " CSMMRCPlateFiber::CSMMRCPlateFiber - failed to get a copy for steel1\n";
      exit(-1);
    }
    ...
    // Check allocation    
    if ( theMaterial[1] == 0 ) {
      opserr << " CSMMRCPlateFiber::CSMMRCPlateFiber - failed to get a copy for steel2\n";
      exit(-1);
    }
    ...
    // Check allocation    
    if ( theMaterial[2] == 0 ) {
      opserr << " CSMMRCPlateFiber::CSMMRCPlateFiber - failed to get a copy for concrete1\n";
      exit(-1);
    }
    ...
    // Check allocation    
    if ( theMaterial[3] == 0 ) {
      opserr << " CSMMRCPlateFiber::CSMMRCPlateFiber - failed to get a copy for concrete2\n";
      exit(-1);
    }
    ...
    if ( theResponses == 0) {
      opserr << " CSMMRCPlateFiber::CSMMRCPlateFiber - failed allocate responses array\n";
      exit(-1);
    }
	...
	(theResponses[4] == 0) || (theResponses[5] == 0)) {
      opserr << " CSMMRCPlateFiber::CSMMRCPlateFiber - failed to set appropriate materials tag: " << tag << "\n";
      exit(-1);
    }
...
CSMMRCPlateFiber::CSMMRCPlateFiber()
 :NDMaterial(0, ND_TAG_CSMMRCPlateFiber), strain_vec(3),
...
double CSMMRCPlateFiber::getRho(void)
{
	return rho;
}


int CSMMRCPlateFiber::setTrialStrain(const Vector &v)
{
...
int CSMMRCPlateFiber::setTrialStrain(const Vector &v, const Vector &r)
{
  opserr << "error: CSMMRCPlateFiber::setTrialStrain(&v, &r) -- not really responsibility" << endln;
  return 0;
}


int CSMMRCPlateFiber::setTrialStrainIncr(const Vector &v)
{
  opserr << "error: CSMMRCPlateFiber::setTrialStrainIncr(&v) -- not really responsibility" << endln;
  return 0;
}


int CSMMRCPlateFiber::setTrialStrainIncr(const Vector &v, const Vector &r)
{
  opserr << "error: CSMMRCPlateFiber::setTrialStrainIncr(&v, &r) -- not really responsibility" << endln;
  return 0;
}


const Matrix& CSMMRCPlateFiber::getTangent(void)
{
  return tangent_matrix;
}

const Vector& CSMMRCPlateFiber::getStress(void)
{
...
const Vector& CSMMRCPlateFiber :: getStrain()
{
  return strain_vec;
}
    

const Vector& CSMMRCPlateFiber::getCommittedStress(void)
{
  return stress_vec;
}


const Vector& CSMMRCPlateFiber::getCommittedStrain(void)
{
  return strain_vec;
}
    

int CSMMRCPlateFiber::commitState(void)
{
  for (int i=0; i<4; i++)
  {
...
int CSMMRCPlateFiber::revertToLastCommit(void)
{
...
int CSMMRCPlateFiber::revertToStart(void)
{
  int i;
  for (i=0; i<4; i++)
  {
...
NDMaterial* CSMMRCPlateFiber::getCopy(void)
{
  CSMMRCPlateFiber* theCopy =
    new CSMMRCPlateFiber( this->getTag(), 
					 rho, 
					 theMaterial[0], 
					 theMaterial[1], 
					 theMaterial[2], 
...
NDMaterial* CSMMRCPlateFiber::getCopy(const char *type)
{
	CSMMRCPlateFiber* theModel =
		new CSMMRCPlateFiber( this->getTag(), 
...
void CSMMRCPlateFiber::Print(OPS_Stream &s, int flag )
{
	s << "\n\tCSMMRCPlateFiber, material id: " << this->getTag() << endln;
    /*
	s << "\tRho: " << rho << endln;
...
int CSMMRCPlateFiber::sendSelf(int commitTag, Channel &theChannel)
{
  int res = 0;
  ...
  if (res < 0) {
    opserr << "WARNING CSMMRCPlateFiber::sendSelf() - " << this->getTag() << " failed to send Vector\n";
    return res;
  }	      
  ...
  if (res < 0) {
    opserr << "WARNING CSMMRCPlateFiber::sendSelf() - " << this->getTag() << " failed to send ID\n";
    return res;
  }
  ...
  for (i = 0; i < 4; i++) {
    ...
    if (res < 0) {
      opserr << "CSMMRCPlateFiber::sendSelf() - " << this->getTag() << " failed to send its Material\n";
      return res;
    }
  }	
...
int CSMMRCPlateFiber::recvSelf(int commitTag, Channel &theChannel, FEM_ObjectBroker &theBroker)
{
  int res = 0;
  ...
  if (res < 0) {
    opserr << "WARNING CSMMRCPlateFiber::recvSelf() - failed to receive Vector\n";
    return res;
  }
  ...
  if (res < 0) {
    opserr << "WARNING CSMMRCPlateFiber::recvSelf() - " << this->getTag() << " failed to receive ID\n";
    return res;
  }
  ...
  if (theMaterial == 0) {
    ...
    theMaterial = new UniaxialMaterial *[4];
    if (theMaterial == 0) {
      opserr << "CSMMRCPlateFiber::recvSelf() - Could not allocate UniaxialMaterial* array\n";
      return -1;
    }
    for (int i = 0; i < 4; i++) {
      ...
      if (theMaterial[i] == 0) {
	     opserr << "CSMMRCPlateFiber::recvSelf() - Broker could not create NDMaterial of class type " << matClassTag << endln;
	     return -1;
      }
      ...
      if (res < 0) {
         opserr << "CSMMRCPlateFiber::recvSelf() - material " << i << "failed to recv itself\n";
	     return res;
      }
    }
  }
  ...
  // materials exist , ensure materials of correct type and recvSelf on them
    ...
    for (int i = 0; i < 4; i++) {
      ...
      if (theMaterial[i]->getClassTag() != matClassTag) {
	     ...
	     theMaterial[i] = theBroker.getNewUniaxialMaterial(matClassTag);
	     if (theMaterial[i] == 0) {
            opserr << "CSMMRCPlateFiber::recvSelf() - material " << i << "failed to create\n";
	        return -1;
		 }
	  }
...
int CSMMRCPlateFiber::determineTrialStress(void)
{ 
  // Get Principal strain direction first
  
  Vector Tstrain(5);     //epslonx,epslony,0.5*gammaxy,0.5*gammaxz,0.5*gammayz
  // Get strain values from strain of element
  Tstrain(0) = strain_vec(0);
  Tstrain(1) = strain_vec(1);
  Tstrain(2) = 0.5*strain_vec(2);
  Tstrain(3) = 0.5*strain_vec(3);
  Tstrain(4) = 0.5*strain_vec(4);
  ...
  if ( fabs(Tstrain(0)-Tstrain(1)) < 1e-7 ) {
    ...
    if ( fabs(Tstrain(2)) < 1e-7 ) {
	  ...
	  opserr << "CSMMRCPlateFiber::determineTrialStress: Failure to calculate citaR\n";
...
double CSMMRCPlateFiber::getAngleError(double inputCita)
{
	double outputCita;
...
double CSMMRCPlateFiber::getPrincipalStressAngle(double inputAngle)
{   
	...
	static Matrix TOne(3,3);     // T(citaOne)
	static Matrix TMOne(3,3);    // T(minus citaOne)
	static Matrix TMSL(3,3);     // T(minus citaSL)
	static Matrix TSL_One(3,3);  // T(citaSL minus citaOne)
	static Matrix TMST(3,3);     // T(minus citaST)
	static Matrix TST_One(3,3);  // T(citaST minus citaOne)

	static Matrix V(3,3);        //Matrix considering Hsu/Zhu ratios

	//Strain and stress
	static Vector Tstrain(3);     //epslonx,epslony,0.5*gammaxy	
	static Vector tempStrain(3); //temp values of strains
	double stressSL, stressST; // stress of steel layers in L and T direction

	//stiffness of element
	static Matrix D(3,3);      //tangent stiffness matrix
	static Matrix DC(3,3);     //concrete part
	static Matrix DSL(3,3);    //steel part in L direction
	static Matrix DST(3,3);    //steel part in T direction
	static Matrix DC_bar(3,3);   //partial differentiation matrix of concrete part Eq.(49)	
	static Matrix tempD(3,3);  //temp matrix for data transfer
    ...
    */
	...
	if ( (epslonOne > 0.0) && (epslonTwo > 0.0) ) {
      ...
      opserr << "CSMMRCPlateFiber::getPrincipalStressAngle: failure to get Hsu/Zhu ratio!\n";
    }
	...
	V.Zero();
    if ( v12*v21==1.0 ) {
	  opserr << "CSMMRCPlateFiber::getPrincipalStressAngle: failure to get matrix [V]!\n";
    ...
    //********** get [DC]**************
    DC.addMatrixProduct(0.0, V, TOne, 1.0); // ???
    ...
    double xx,kk;
    //if ( ((fabs(lastStress[0])+fabs(lastStress[0])+fabs(lastStress[0]))==0.0) || comment by Ln
	...
	tempD.addMatrixProduct(0.0, DC_bar, DC, 1.0);
	DC = tempD;
	
    //update [DC]=[TMOne]*[Dc_bar]*[V]*[TOne]
    tempD.addMatrixProduct(0.0, TMOne, DC, 1.0);
	DC = tempD;
    ...
    tangent_matrix(0,0) = tempD(0,0);
	tangent_matrix(0,1) = tempD(0,1);
	tangent_matrix(1,0) = tempD(1,0);
	tangent_matrix(1,1) = tempD(1,1);
	tangent_matrix(1,2) = 0.5*tempD(1,2);
	tangent_matrix(2,1) = 0.5*tempD(2,1);
	tangent_matrix(2,2) = 0.5*tempD(2,2);
	tangent_matrix(3,3) = 0.5*tempD(2,2);
	tangent_matrix(4,4) = 0.5*tempD(2,2);
    ...
    if ( fabs(Tstress(0)-Tstress(1)) < 1e-7 ) {
	  ...
	  if ( fabs(Tstress(2)) < 1e-7 ) {
	  	...
	  	opserr << "CSMMRCPlateFiber::getPrincipalStressAngle: Failure to calculate principal stress direction\n";
