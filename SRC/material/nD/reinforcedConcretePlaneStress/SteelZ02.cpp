...
UniaxialMaterial(tag, MAT_TAG_SteelZ02),
...
SteelZ02::SteelZ02(): UniaxialMaterial(0, MAT_TAG_SteelZ02),
...
int SteelZ02::revertToLastCommit ()
{
...
double SteelZ02::getCommittedStrain()
{
   return Cstrain;
}


double SteelZ02::getStrain ()
{
   return Tstrain;
}

double SteelZ02::getStress ()
{
   return Tstress;
}

double SteelZ02::getTangent ()
{
   // temp add
   if ( Ttangent == 0.0 )
   {
   	   opserr << " SteelZ02:getTangent() -- Ttangent = 0.0\n";
...
int SteelZ02::setTrialStrain (double strain, double strainRate)
{
...
int SteelZ02::setTrial (double strain, double &stress, double &tangent, double strainRate)
{
   // Reset history variables to last converged state
	TminStrain = CminStrain;  
    TmaxStrain = CmaxStrain;  
...
void SteelZ02::determineTrialState (double dStrain)
{
    tt1 = Tstrain;
	tt2 = 0.0;
	ttStrain = dStrain;
	...
	else if ( TloadingState == 4 ) // In the loop reversed from tension envelope
	{
		...
		else
		{
			...
			//{
				//opserr << " SteelZ02 : determinTrialState \n";
	...
	{
		opserr << "SteelZ02::determineTrialState -- Improper TloadingState : " 
			 << TloadingState << " for SteelZ02\n" ;
	}
...
void SteelZ02::compressionEnvelope( )
{
	double epsy = fy/E0;
	Tstress = 0.001*E0*(Tstrain+epsy) -fy;
	/*Ttangent = Tstress/Tstrain;
...
void SteelZ02::determineTrialLoop(double dStrain)
{
	...
	else // create a new inner loop or continue on previous loop
	{
		 ...
		 if (( TloopPathState == 2 || TloopPathState == 3 ) && ( dStrain > 0)) // reverse up at down path
		{
			...
			{
				opserr << " SteelZ02::determineTrialLoop -- overflowed the size of the array storing the loop!\n"
		...
		else // continue on previous path of the loop
		{
			...
			{
				  opserr << " SteelZ02::determineTrialLoop -- improper TloopPathState : " 
					   << TloopPathState << "\n" ;
			}
		}
...
void SteelZ02::determineDownPathPoint( )
{
...
void SteelZ02::determineUpPathPoint( )
{
	double epsy = fy/E0;
	double fcr = 0.31*sqrt(fpc);
	if ( rou < 0.0025 ) rou = 0.0025;
...
void SteelZ02::reverseFromTenEnvelope( )
{   
...
void SteelZ02::reverseFromComEnvelope( )
{
...
void SteelZ02::reverseLoopSetZero( )
{
	TloopPathState = 0;
	TreverseTopNum = 0;
	TreverseBottomNum = 0;
...
UniaxialMaterial* SteelZ02::getCopy ()
{
   SteelZ02* theCopy = new SteelZ02(this->getTag(), fy, E0, fpc, rou, ac,rc);     
...
int SteelZ02::sendSelf (int commitTag, Channel& theChannel)
{
   int res = 0;
   ...
   if (res < 0) 
      opserr << "SteelZ02::sendSelf() - failed to send data\n";
...
int SteelZ02::recvSelf (int commitTag, Channel& theChannel,
                                FEM_ObjectBroker& theBroker)
{
   ...
   if (res < 0) {
      opserr << "SteelZ02::recvSelf() - failed to receive data\n";
      this->setTag(0);      
   }
   else {
...
Response* 
SteelZ02::setResponse(const char **argv, int argc,
...
int 
SteelZ02::getResponse(int responseID, Information &matInfo)
{
  if (responseID == 100) {
...
void SteelZ02::Print (OPS_Stream& s, int flag)
{
   s << "\tSteelZ02 tag: " << this->getTag() << endln;
