...
#include "RARCPlaneStressFiber.h"
...
//Vector RARCPlaneStressFiber :: strain_vec(3);
//Vector RARCPlaneStressFiber :: stress_vec(3);
//Matrix RARCPlaneStressFiber :: tangent_matrix(3,3);
...
static int numRARCPlaneStressFiberMaterials = 0;

#define ND_TAG_RARCPlaneStressFiber 132

OPS_Export void *
OPS_NewRARCPlaneStressFiberMaterial()
{
  if (numRARCPlaneStressFiberMaterials == 0) {
    numRARCPlaneStressFiberMaterials++;
    //opserr << "RARCPlaneStressFiber unaxial material - Written by J.Zhong, Thomas T.C. Hsu and Y.L. Mo - Copyright@2009\n";
  }
  ...
  if (numRemainingArgs < 14) {
    opserr << "Invalid Args want: NDMaterial RARCPlaneStressFiber matTag? rho? UniaxiaMatTag1? UniaxiaMatTag2? UniaxiaMatTag3? UniaxiaMatTag4? angle1? angle2? rou1? rou2? fpc? fy? E0? epsc0?>\n";
    return 0;	
  }
  ...
  if (OPS_GetInt(&numData, &tag) != 0) {
    opserr << "WARNING invalid uniaxialMaterial RARCPlaneStressFiber tag" << endln;
    return 0;
  }

  numData = 1;
  if (OPS_GetDouble(&numData, &rho) != 0) {
    opserr << "Invalid Arg rho: uniaxialMaterial RARCPlaneStressFiber tag? fy? E0? fpc? rou? <ac?> <rc?>" << endln;
    return 0;	
  }
  ...
  if (theUniaxialMaterial1 == 0) {
    ...
    opserr << "\nRARCPlaneStressFiber: " << tag << endln;
    return 0;
  }
  ...
  //now create the RARCPlaneStressFiber
  theMaterial = new RARCPlaneStressFiber (tag, 
						     rho,
  ...
  if (theMaterial == 0) {
    ...
    opserr << "RARCPlaneStressFiber: " << tag << endln;
    return 0;
  }
  ...
  fiberStrain(2), fiberStress(2), fiberTangent(2,2)
{
  ...
  if ( theMaterial == 0 ) {
    opserr << " RARCPlaneStressFiber::RARCPlaneStressFiber - failed allocate material array\n";
    exit(-1);
  }
  ...
  // Check allocation    
  if ( theMaterial[0] == 0 ) {
    opserr << " RARCPlaneStressFiber::RARCPlaneStressFiber - failed to get a copy for steel1\n";
    exit(-1);
  }
  ...
  // Check allocation    
  if ( theMaterial[1] == 0 ) {
    opserr << " RARCPlaneStressFiber::RARCPlaneStressFiber - failed to get a copy for steel2\n";
    exit(-1);
  }
  ...
  // Check allocation    
  if ( theMaterial[2] == 0 ) {
    opserr << " RARCPlaneStressFiber::RARCPlaneStressFiber - failed to get a copy for concrete1\n";
		exit(-1);
  }
  ...
  theMaterial[3] = c2->getCopy();	
	// Check allocation    
  if ( theMaterial[3] == 0 ) {
    opserr << " RARCPlaneStressFiber::RARCPlaneStressFiber - failed to get a copy for concrete2\n";
    exit(-1);
  }
  ...
  if ( theResponses == 0) {
    opserr << " RARCPlaneStressFiber::RARCPlaneStressFiber - failed allocate responses  array\n";
    exit(-1);
  }
  ...
  if ((theResponses[0] == 0) || (theResponses[1] == 0) ||
    ...
    opserr << " RARCPlaneStressFiber::RARCPlaneStressFiber - failed to set appropriate materials tag: " << tag << "\n";
    exit(-1);
  }
...
double
RARCPlaneStressFiber::getRho(void)
{
  return rho;
}
...
///*
int
RARCPlaneStressFiber::setTrialStrain(const Vector &v, const Vector &r)
{
  opserr << "error: RARCPlaneStressFiber::setTrialStrain(&v, &r) -- not really responsibility" << endln;
  return 0;
}

int 
RARCPlaneStressFiber::setTrialStrainIncr(const Vector &v)
{
  opserr << "error: RARCPlaneStressFiber::setTrialStrainIncr(&v) -- not really responsibility" << endln;
  return 0;
}

int
RARCPlaneStressFiber::setTrialStrainIncr(const Vector &v, const Vector &r)
{
  opserr << "error: RARCPlaneStressFiber::setTrialStrainIncr(&v, &r) -- not really responsibility" << endln;
  return 0;
}
//*/

const Matrix& 
RARCPlaneStressFiber::getTangent(void)
{
  return tangent_matrix;
}

const Vector& 
RARCPlaneStressFiber::getStress(void)
{
  return stress_vec;
}
...
const Vector& 
RARCPlaneStressFiber::getCommittedStress(void)
{
  return stress_vec;
}

const Vector& 
RARCPlaneStressFiber::getCommittedStrain(void)
{
  return strain_vec;
}

int 
RARCPlaneStressFiber::commitState(void)
{
  for (int i=0; i<4; i++)
...
int 
RARCPlaneStressFiber::revertToLastCommit(void)
{
  for (int i=0; i<4; i++)
...
int 
RARCPlaneStressFiber::revertToStart(void)
{
  for (int i=0; i<4; i++)
...
NDMaterial* 
RARCPlaneStressFiber::getCopy(void)
{
  /*
    RARCPlaneStressFiber *clone;
    clone = new RARCPlaneStressFiber();
    *clone = *this;
    return clone;
  //*/
  
  RARCPlaneStressFiber* theCopy =
    new RARCPlaneStressFiber( this->getTag(), 
...
NDMaterial* RARCPlaneStressFiber::getCopy(const char *type)
{
  /*
    RARCPlaneStressFiber *clone;
    clone = new RARCPlaneStressFiber();
...
void RARCPlaneStressFiber::Print(OPS_Stream &s, int flag )
{
  s << "\n\tRARCPlaneStressFiber, material id: " << this->getTag() << endln;
  /*
    s << "\tRho: " << rho << endln;
...
int
RARCPlaneStressFiber::sendSelf(int commitTag, Channel &theChannel)
{
  int res = 0;
  ...
  if (res < 0) {
    opserr << "WARNING RARCPlaneStressFiber::sendSelf() - " << this->getTag() << " failed to send Vector\n";
    return res;
  }	      
  ...
  for (i=0; i<4; i++)
    ...
    if (res < 0) {
       opserr << "WARNING RARCPlaneStressFiber::sendSelf() - " << this->getTag() << " failed to send ID\n";
       return res;
	}
   ...
   for (i = 0; i < 4; i++) {
    ...
    if (res < 0) {
      opserr << "RARCPlaneStressFiber::sendSelf() - " << this->getTag() << " failed to send its Material\n";
      return res;
    }
   }	
...
int 
RARCPlaneStressFiber::recvSelf(int commitTag, Channel &theChannel, FEM_ObjectBroker &theBroker)
{
  int res = 0;
  ...
  if (res < 0) {
    opserr << "WARNING RARCPlaneStressFiber::recvSelf() - failed to receive Vector\n";
    return res;
  }
  ...
  if (res < 0) {
    opserr << "WARNING RARCPlaneStressFiber::recvSelf() - " << this->getTag() << " failed to receive ID\n";
    return res;
  }
  ...
  if (theMaterial == 0) {
    ...
    theMaterial = new UniaxialMaterial *[4];
    if (theMaterial == 0) {
      opserr << "RARCPlaneStressFiber::recvSelf() - Could not allocate UniaxialMaterial* array\n";
      return -1;
    }
    for (int i = 0; i < 4; i++) {
      ...
      if (theMaterial[i] == 0) {
	     opserr << "RARCPlaneStressFiber::recvSelf() - Broker could not create NDMaterial of class type " << matClassTag << endln;
	     return -1;
      }
      ...
      if (res < 0) {
         opserr << "RARCPlaneStressFiber::recvSelf() - material " << i << "failed to recv itself\n";
	     return res;
      }
    }
  }
  ...
  // materials exist , ensure materials of correct type and recvSelf on them
    ...
    for (int i = 0; i < 4; i++) {
      ...
      if (theMaterial[i]->getClassTag() != matClassTag) {
	     ...
	     theMaterial[i] = theBroker.getNewUniaxialMaterial(matClassTag);
	     if (theMaterial[i] == 0) {
            opserr << "RARCPlaneStressFiber::recvSelf() - material " << i << "failed to create\n";
	        return -1;
		 }
	  }
...
int
RARCPlaneStressFiber::setTrialStrain(const Vector &v)
{
...
int
RARCPlaneStressFiber::determineTrialStress(void)
{   
  // Define i, j, k
  int i, j, k;
  ...
  else // Tstrain[0] != Tstrain[1]
  {
    ...
    if ( fabs(Tstrain[2]) < 1e-7 ) {
	  ...
	  opserr << "RARCPlaneStressFiber::determineTrialStress: Failure to calculate citaR\n";
  ...
  if (strainSL > strainST)  strainSF = strainSL;
  else                      strainSF = strainST;
  ...
  if ( (epslonOne > 0.0) && (epslonTwo > 0.0) ) {
	...
	if (steelStatus == 1) {
      ...
      opserr << "RARCPlaneStressFiber::determineTrialStress: failure to get Hsu/Zhu ratio!\n";
  }
  ...
  if ( v12*v21==1.0 ) {
      opserr << "RARCPlaneStressFiber::determineTrialStress: failure to get matrix [V]!\n";
