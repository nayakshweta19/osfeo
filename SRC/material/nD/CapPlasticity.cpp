...
double
CapPlasticity::getRho(void) {
	return rho;
};

int
CapPlasticity::setTrialStrain(const Vector &pStrain) {  // strain from element is eng. strain!
...
int
CapPlasticity::setTrialStrainIncr(const Vector &pStrainRate) {
...
int
CapPlasticity::setTrialStrainIncr(const Vector &pStrainRate, const Vector &r) {
	return setTrialStrainIncr(pStrainRate);
};

const Vector &
CapPlasticity::getStrain(void) {
...
int
CapPlasticity::commitState(void)  {
	CStrain = strain;
	CStress = stress;
...
int
CapPlasticity::revertToLastCommit(void)  {
	return 0;
};

int
CapPlasticity::revertToStart(void)  {
	CStrain.Zero();
	CPlastStrain.Zero();
	CStress.Zero();
	strain.Zero();
...
const char *
CapPlasticity::getType(void) const  {
	return (ndm == 2) ? "PlaneStrain" : "ThreeDimensional";
};

int
CapPlasticity::getOrder(void) const  {
	return (ndm == 2) ? 3 : 6;
};

int
CapPlasticity::sendSelf(int commitTag, Channel &theChannel)  { return 0; };

int
CapPlasticity::recvSelf(int commitTag, Channel &theChannel,
...
int
CapPlasticity::getResponse(int responseID, Information &matInfo)  {
...
void
CapPlasticity::Print(OPS_Stream &s, int flag)  { return; };

// --------------

double
CapPlasticity::CapBoundL(double k){
	if (k > 0)
		return k;
	return 0;
};
...
// --------------
double
CapPlasticity::CapBoundX(double k){  // function of X
	return k + R*failureEnvelop(k);
};

// --------------
double
CapPlasticity::failureEnvelop(double I){
...
// --------------
double
CapPlasticity::CapSurface(double normS, double I, double k){  // Fc
...
// --------------
double
CapPlasticity::failureEnvelopDeriv(double I){
	return lambda*beta*exp(-beta*I) + theta;
};

// --------------
double
CapPlasticity::hardeningParameter_H(double k2, double k1){
	return W*(exp(-D*CapBoundX(k1)) - exp(-D*CapBoundX(k2)));
};

int
CapPlasticity::findMode(double normS, double I1, double k){
	int mode = -1;
...
// --------------
double
CapPlasticity::Newton_k(double tol, int mode /*, double normS, double I1, double k */){
...
const Matrix &
CapPlasticity::getInitialTangent(void) {
	return getTangent();
};

const Vector &
CapPlasticity::getStress(void) {
...
Matrix &
CapPlasticity::dF2dSigma(int mode){ // the returned matrix has been consistent with notation (i.e., last 3 columns multiplied by 2) 
...
Vector &
CapPlasticity::dFdSigma(int mode){
...
double
CapPlasticity::dFdk(int OrderOfDerivative){
...
Vector
CapPlasticity::dF2dSigmadk(void){
...
double
CapPlasticity::dHdk(double hardening_k){
	double result = 0.0;
...
///////////////////////// add sensitivity ///////////////////////////////////
int
CapPlasticity::setParameter(const char **argv, int argc, Information &info)
{
	if (argc < 1)
		return -1;
...
int
CapPlasticity::updateParameter(int passedParameterID, Information &info){
...
int
CapPlasticity::activateParameter(int passedParameterID){
