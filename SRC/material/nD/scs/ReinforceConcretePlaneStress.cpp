...
#include <ReinforceConcretePlaneStress.h>
...
//Vector ReinforceConcretePlaneStress :: strain_vec(3);
//Vector ReinforceConcretePlaneStress :: stress_vec(3);
//Matrix ReinforceConcretePlaneStress :: tangent_matrix(3,3);

#define ND_TAG_ReinforceConcretePlaneStress 30218
...
OPS_NewReinforcedConcretePlaneStressMaterial()
{
	...
	theMaterial = new ReinforceConcretePlaneStress (tag, 
		rho,
...
ReinforceConcretePlaneStress ::
ReinforceConcretePlaneStress ( int      tag, 
	                           double   RHO,
							   UniaxialMaterial *s1,
							   UniaxialMaterial *s2,
							   UniaxialMaterial *c1,
							   ...
							   double   E,
							   double   EPSC0) :
NDMaterial(tag, ND_TAG_ReinforceConcretePlaneStress), 
...
fpc(FPC), fy(FY), E0(E), epsc0(EPSC0), strain_vec(3), stress_vec(3),tangent_matrix(3,3)
{
	...
	if ( theMaterial == 0 ) {
		opserr << " ReinforceConcretePlaneStress::ReinforceConcretePlaneStress - failed allocate material array\n";
		exit(-1);
	}
	...
	// Check allocation    
	if ( theMaterial[0] == 0 ) {
		opserr << " ReinforceConcretePlaneStress::ReinforceConcretePlaneStress - failed to get a copy for steel1\n";
		exit(-1);
	}
	...
	// Check allocation    
	if ( theMaterial[1] == 0 ) {
		opserr << " ReinforceConcretePlaneStress::ReinforceConcretePlaneStress - failed to get a copy for steel2\n";
		exit(-1);
	}
	...
	// Check allocation    
	if ( theMaterial[2] == 0 ) {
		opserr << " ReinforceConcretePlaneStress::ReinforceConcretePlaneStress - failed to get a copy for concrete1\n";
		exit(-1);
	}
	...
	// Check allocation    
	if ( theMaterial[3] == 0 ) {
		opserr << " ReinforceConcretePlaneStress::ReinforceConcretePlaneStress - failed to get a copy for concrete2\n";
		exit(-1);
	}
...
ReinforceConcretePlaneStress::ReinforceConcretePlaneStress()
:NDMaterial(0, ND_TAG_ReinforceConcretePlaneStress), strain_vec(3),
...
double ReinforceConcretePlaneStress::getRho(void)
{
	return rho;
}


int ReinforceConcretePlaneStress::setTrialStrain(const Vector &v)
{
...
///*
int ReinforceConcretePlaneStress::setTrialStrain(const Vector &v, const Vector &r)
{
   	// Set values for strain_vec
	strain_vec(0) = v(0);
	strain_vec(1) = v(1);
...
int ReinforceConcretePlaneStress::setTrialStrainIncr(const Vector &v)
{
	// Set values for strain_vec
	strain_vec(0) = v(0);
	strain_vec(1) = v(1);
...
int ReinforceConcretePlaneStress::setTrialStrainIncr(const Vector &v, const Vector &r)
{
	// Set values for strain_vec
	strain_vec(0) = v(0);
	strain_vec(1) = v(1);
...
const Matrix& ReinforceConcretePlaneStress::getTangent(void)
{
	return tangent_matrix;
}

const Vector& ReinforceConcretePlaneStress::getStress(void)
{
...
const Vector& ReinforceConcretePlaneStress :: getStrain()
{
	return strain_vec;
}
    

const Vector& ReinforceConcretePlaneStress::getCommittedStress(void)
{
    return stress_vec;
}


const Vector& ReinforceConcretePlaneStress::getCommittedStrain(void)
{
    return strain_vec;
}
    

int ReinforceConcretePlaneStress::commitState(void)
{
    for (int i=0; i<4; i++)
	{
...
int ReinforceConcretePlaneStress::revertToLastCommit(void)
{
...
int ReinforceConcretePlaneStress::revertToStart(void)
{
    for (int i=0; i<4; i++)
	{
...
NDMaterial* ReinforceConcretePlaneStress::getCopy(void)
{
	/*
	ReinforceConcretePlaneStress *clone;
	clone = new ReinforceConcretePlaneStress();
	*clone = *this;
	return clone;
	//*/

	ReinforceConcretePlaneStress* theCopy =
		new ReinforceConcretePlaneStress( this->getTag(), 
...
NDMaterial* ReinforceConcretePlaneStress::getCopy(const char *type)
{
    /*
	ReinforceConcretePlaneStress *clone;
	clone = new ReinforceConcretePlaneStress();
	*clone = *this;
	return clone;
	//*/
	ReinforceConcretePlaneStress* theModel =
		new ReinforceConcretePlaneStress( this->getTag(), 
           rho, theMaterial[0], theMaterial[1], theMaterial[2], theMaterial[3], angle1, angle2, rou1, rou2, fpc, fy, E0, epsc0 );
...
void ReinforceConcretePlaneStress::Print(OPS_Stream &s, int flag )
{
	s << "\n\tReinforceConcretePlaneStress, material id: " << this->getTag() << endln;
    /*
	s << "\tRho: " << rho << endln;
...
int ReinforceConcretePlaneStress::determineTrialStress(void)
{   
	// Define i, j, k
	int i, j, k;
	...
	else // Tstrain[0] != Tstrain[1]
	{
		...
		{
			opserr << "ReinforceConcretePlaneStress::determineTrialStress: Failure to calculate citaR\n";
	...
	{
    	opserr << "ReinforceConcretePlaneStress::determineTrialStress: failure to get Hsu/Zhu ratio!\n";
	}
	...
	if ( v12*v21==1.0 )
	{
		opserr << "ReinforceConcretePlaneStress::determineTrialStress: failure to get matrix [V]!\n";
	...
	else // Tstrain[0] != Tstrain[1]
	{
		...
		{
			opserr << "ReinforceConcretePlaneStress::determineTrialStress: Failure to calculate principal stress direction\n";
