...
#include <FAReinforceConcretePlaneStress.h>
...
//Vector FAReinforceConcretePlaneStress :: strain_vec(3);
//Vector FAReinforceConcretePlaneStress :: stress_vec(3);
//Matrix FAReinforceConcretePlaneStress :: tangent_matrix(3,3);

#define ND_TAG_FAReinforceConcretePlaneStress 30216
...
OPS_NewFAReinforcedConcretePlaneStressMaterial()
{
	...
	theMaterial = new FAReinforceConcretePlaneStress (tag, 
		rho,
...
FAReinforceConcretePlaneStress ::
FAReinforceConcretePlaneStress ( int      tag, 
	                           double   RHO,
							   UniaxialMaterial *s1,
							   UniaxialMaterial *s2,
							   UniaxialMaterial *c1,
	...
	fpc(FPC), fy(FY), E0(E), epsc0(EPSC0), strain_vec(3), stress_vec(3),tangent_matrix(3,3)
{
	...
	if ( theMaterial == 0 ) {
		opserr << " FAReinforceConcretePlaneStress::FAReinforceConcretePlaneStress - failed allocate material array\n";
		exit(-1);
	}
	...
	// Check allocation    
	if ( theMaterial[0] == 0 ) {
		opserr << " FAReinforceConcretePlaneStress::FAReinforceConcretePlaneStress - failed to get a copy for steel1\n";
		exit(-1);
	}
	...
	// Check allocation    
	if ( theMaterial[1] == 0 ) {
		opserr << " FAReinforceConcretePlaneStress::FAReinforceConcretePlaneStress - failed to get a copy for steel2\n";
		exit(-1);
	}
	...
	// Check allocation    
	if ( theMaterial[2] == 0 ) {
		opserr << " FAReinforceConcretePlaneStress::FAReinforceConcretePlaneStress - failed to get a copy for concrete1\n";
		exit(-1);
	}
	...
	// Check allocation    
	if ( theMaterial[3] == 0 ) {
		opserr << " FAReinforceConcretePlaneStress::FAReinforceConcretePlaneStress - failed to get a copy for concrete2\n";
		exit(-1);
	}
...
FAReinforceConcretePlaneStress::FAReinforceConcretePlaneStress()
...
double FAReinforceConcretePlaneStress::getRho(void)
{
	return rho;
}


int FAReinforceConcretePlaneStress::setTrialStrain(const Vector &v)
{
...
///*
int FAReinforceConcretePlaneStress::setTrialStrain(const Vector &v, const Vector &r)
{
   	// Set values for strain_vec
	strain_vec(0) = v(0);
	strain_vec(1) = v(1);
...
int FAReinforceConcretePlaneStress::setTrialStrainIncr(const Vector &v)
{
	// Set values for strain_vec
	strain_vec(0) = v(0);
	strain_vec(1) = v(1);
...
int FAReinforceConcretePlaneStress::setTrialStrainIncr(const Vector &v, const Vector &r)
{
	// Set values for strain_vec
	strain_vec(0) = v(0);
	strain_vec(1) = v(1);
...
const Matrix& FAReinforceConcretePlaneStress::getTangent(void)
{
	return tangent_matrix;
}

const Vector& FAReinforceConcretePlaneStress::getStress(void)
{
...
const Vector& FAReinforceConcretePlaneStress :: getStrain()
{
	return strain_vec;
}
    

const Vector& FAReinforceConcretePlaneStress::getCommittedStress(void)
{
    return stress_vec;
}


const Vector& FAReinforceConcretePlaneStress::getCommittedStrain(void)
{
    return strain_vec;
}
    

int FAReinforceConcretePlaneStress::commitState(void)
{
    for (int i=0; i<4; i++)
	{
...
int FAReinforceConcretePlaneStress::revertToLastCommit(void)
{
...
int FAReinforceConcretePlaneStress::revertToStart(void)
{
    for (int i=0; i<4; i++)
	{
...
NDMaterial* FAReinforceConcretePlaneStress::getCopy(void)
{
	/*
	FAReinforceConcretePlaneStress *clone;
	clone = new FAReinforceConcretePlaneStress();
	*clone = *this;
	return clone;
	//*/

	FAReinforceConcretePlaneStress* theCopy =
		new FAReinforceConcretePlaneStress( this->getTag(), 
...
NDMaterial* FAReinforceConcretePlaneStress::getCopy(const char *type)
{
    /*
	FAReinforceConcretePlaneStress *clone;
	clone = new FAReinforceConcretePlaneStress();
	*clone = *this;
	return clone;
	//*/
	FAReinforceConcretePlaneStress* theModel =
		new FAReinforceConcretePlaneStress( this->getTag(), 
           rho, theMaterial[0], theMaterial[1], theMaterial[2], theMaterial[3], angle1, angle2, rou1, rou2, fpc, fy, E0, epsc0 );
...
void FAReinforceConcretePlaneStress::Print(OPS_Stream &s, int flag )
{
...
int FAReinforceConcretePlaneStress::sendSelf(int commitTag, Channel &theChannel)
{
	int res = 0;
    ...
    if (res < 0) {
      opserr << "WARNING FAReinforceConcretePlaneStress::sendSelf() - " << this->getTag() << " failed to send Vector\n";
      return res;
	}	      
    ...
    if (res < 0) {
       opserr << "WARNING FAReinforceConcretePlaneStress::sendSelf() - " << this->getTag() << " failed to send ID\n";
       return res;
	}
   ...
   for (int i = 0; i < 4; i++) {
    ...
    if (res < 0) {
      opserr << "FAReinforceConcretePlaneStress::sendSelf() - " << this->getTag() << " failed to send its Material\n";
      return res;
    }
   }	
...
int FAReinforceConcretePlaneStress::recvSelf(int commitTag, Channel &theChannel, FEM_ObjectBroker &theBroker)
{
  int res = 0;
  ...
  if (res < 0) {
    opserr << "WARNING FAReinforceConcretePlaneStress::recvSelf() - failed to receive Vector\n";
    return res;
  }
  ...
  if (res < 0) {
    opserr << "WARNING FAReinforceConcretePlaneStress::recvSelf() - " << this->getTag() << " failed to receive ID\n";
    return res;
  }
  ...
  if (theMaterial == 0) {
    ...
    theMaterial = new UniaxialMaterial *[4];
    if (theMaterial == 0) {
      opserr << "FAReinforceConcretePlaneStress::recvSelf() - Could not allocate UniaxialMaterial* array\n";
      return -1;
    }
    for (int i = 0; i < 4; i++) {
      ...
      if (theMaterial[i] == 0) {
	     opserr << "FAReinforceConcretePlaneStress::recvSelf() - Broker could not create NDMaterial of class type " << matClassTag << endln;
	     return -1;
      }
      ...
      if (res < 0) {
         opserr << "FAReinforceConcretePlaneStress::recvSelf() - material " << i << "failed to recv itself\n";
	     return res;
      }
    }
  }
  ...
  // materials exist , ensure materials of correct type and recvSelf on them
    ...
    for (int i = 0; i < 4; i++) {
      ...
      if (theMaterial[i]->getClassTag() != matClassTag) {
	     ...
	     theMaterial[i] = theBroker.getNewUniaxialMaterial(matClassTag);
	     if (theMaterial[i] == 0) {
            opserr << "FAReinforceConcretePlaneStress::recvSelf() - material " << i << "failed to create\n";
	        return -1;
		 }
	  }
...
int FAReinforceConcretePlaneStress::determineTrialStress(void)
{ 
...
double FAReinforceConcretePlaneStress::getAngleError(double inputCita)
{
	double pi = 3.14159265359;
	double outputCita;
...
double FAReinforceConcretePlaneStress::getPrincipalStressAngle(double inputAngle)
{   
