...
#include <PrestressConcretePlaneStress.h>
...
//Vector PrestressConcretePlaneStress :: strain_vec(3);
//Vector PrestressConcretePlaneStress :: stress_vec(3);
//Matrix PrestressConcretePlaneStress :: tangent_matrix(3,3);

#define ND_TAG_PrestressConcretePlaneStress 30217
...
PrestressConcretePlaneStress ::
PrestressConcretePlaneStress ( int      tag, 
	                           double   RHO,
							   UniaxialMaterial *t1,
							   UniaxialMaterial *s1,
							   UniaxialMaterial *c1,
							   ...
							   double   E,
							   double   EPSC0) :
	NDMaterial(tag, ND_TAG_PrestressConcretePlaneStress), 
	...
	fpc(FPC), fy1(FY1), fy2(FY2), E0(E), epsc0(EPSC0), strain_vec(3), stress_vec(3),tangent_matrix(3,3)
{
	...
	if ( theMaterial == 0 ) {
		opserr << " PrestressConcretePlaneStress::PrestressConcretePlaneStress - failed allocate material array\n";
		exit(-1);
	}
	...
	// Check allocation    
	if ( theMaterial[0] == 0 ) {
		opserr << " PrestressConcretePlaneStress::PrestressConcretePlaneStress - failed to get a copy for tendon1\n";
		exit(-1);
	}
	...
	// Check allocation    
	if ( theMaterial[1] == 0 ) {
		opserr << " PrestressConcretePlaneStress::PrestressConcretePlaneStress - failed to get a copy for steel1\n";
		exit(-1);
	}
	...
	// Check allocation    
	if ( theMaterial[2] == 0 ) {
		opserr << " PrestressConcretePlaneStress::PrestressConcretePlaneStress - failed to get a copy for concrete1\n";
		exit(-1);
	}
	...
	// Check allocation    
	if ( theMaterial[3] == 0 ) {
		opserr << " PrestressConcretePlaneStress::PrestressConcretePlaneStress - failed to get a copy for concrete2\n";
		exit(-1);
	}
...
PrestressConcretePlaneStress::PrestressConcretePlaneStress()
:NDMaterial(0, ND_TAG_PrestressConcretePlaneStress), strain_vec(3),
...
double PrestressConcretePlaneStress::getRho(void)
{
	return rho;
}


int PrestressConcretePlaneStress::setTrialStrain(const Vector &v)
{
...
///*
int PrestressConcretePlaneStress::setTrialStrain(const Vector &v, const Vector &r)
{
   	// Set values for strain_vec
	strain_vec(0) = v(0);
	strain_vec(1) = v(1);
...
int PrestressConcretePlaneStress::setTrialStrainIncr(const Vector &v)
{
	// Set values for strain_vec
	strain_vec(0) = v(0);
	strain_vec(1) = v(1);
...
int PrestressConcretePlaneStress::setTrialStrainIncr(const Vector &v, const Vector &r)
{
	// Set values for strain_vec
	strain_vec(0) = v(0);
	strain_vec(1) = v(1);
...
const Matrix& PrestressConcretePlaneStress::getTangent(void)
{
	return tangent_matrix;
}

const Vector& PrestressConcretePlaneStress::getStress(void)
{
...
const Vector& PrestressConcretePlaneStress :: getStrain()
{
	return strain_vec;
}
    

const Vector& PrestressConcretePlaneStress::getCommittedStress(void)
{
    return stress_vec;
}


const Vector& PrestressConcretePlaneStress::getCommittedStrain(void)
{
    return strain_vec;
}
    

int PrestressConcretePlaneStress::commitState(void)
{
    for (int i=0; i<4; i++)
	{
...
int PrestressConcretePlaneStress::revertToLastCommit(void)
{
...
int PrestressConcretePlaneStress::revertToStart(void)
{
    for (int i=0; i<4; i++)
	{
...
NDMaterial* PrestressConcretePlaneStress::getCopy(void)
{
	/*
	PrestressConcretePlaneStress *clone;
	clone = new PrestressConcretePlaneStress();
	*clone = *this;
	return clone;
	//*/

	PrestressConcretePlaneStress* theCopy =
		new PrestressConcretePlaneStress( this->getTag(), 
...
NDMaterial* PrestressConcretePlaneStress::getCopy(const char *type)
{
    /*
	PrestressConcretePlaneStress *clone;
	clone = new PrestressConcretePlaneStress();
	*clone = *this;
	return clone;
	//*/
	PrestressConcretePlaneStress* theModel =
		new PrestressConcretePlaneStress( this->getTag(), 
           rho, theMaterial[0], theMaterial[1], theMaterial[2], theMaterial[3], angle1, angle2, rou1, rou2, pstrain, fpc, fy1, fy2, E0, epsc0 );
...
void PrestressConcretePlaneStress::Print(OPS_Stream &s, int flag )
{
	s << "\n\tPrestressConcretePlaneStress, material id: " << this->getTag() << endln;
    /*
	s << "\tRho: " << rho << endln;
...
int PrestressConcretePlaneStress::sendSelf(int commitTag, Channel &theChannel)
{
	int res = 0;
    ...
    if (res < 0) {
      opserr << "WARNING PrestressConcretePlaneStress::sendSelf() - " << this->getTag() << " failed to send Vector\n";
      return res;
	}	      
    ...
    if (res < 0) {
       opserr << "WARNING PrestressConcretePlaneStress::sendSelf() - " << this->getTag() << " failed to send ID\n";
       return res;
	}
   ...
   for (int i = 0; i < 4; i++) {
    ...
    if (res < 0) {
      opserr << "PrestressConcretePlaneStress::sendSelf() - " << this->getTag() << " failed to send its Material\n";
      return res;
    }
   }	
...
int PrestressConcretePlaneStress::recvSelf(int commitTag, Channel &theChannel, FEM_ObjectBroker &theBroker)
{
  int res = 0;
  ...
  if (res < 0) {
    opserr << "WARNING PrestressConcretePlaneStress::recvSelf() - failed to receive Vector\n";
    return res;
  }
  ...
  if (res < 0) {
    opserr << "WARNING PrestressConcretePlaneStress::recvSelf() - " << this->getTag() << " failed to receive ID\n";
    return res;
  }
  ...
  if (theMaterial == 0) {
    ...
    theMaterial = new UniaxialMaterial *[4];
    if (theMaterial == 0) {
      opserr << "PrestressConcretePlaneStress::recvSelf() - Could not allocate UniaxialMaterial* array\n";
      return -1;
    }
    for (int i = 0; i < 4; i++) {
      ...
      if (theMaterial[i] == 0) {
	     opserr << "PrestressConcretePlaneStress::recvSelf() - Broker could not create NDMaterial of class type " << matClassTag << endln;
	     return -1;
      }
      ...
      if (res < 0) {
         opserr << "PrestressConcretePlaneStress::recvSelf() - material " << i << "failed to recv itself\n";
	     return res;
      }
    }
  }
  ...
  // materials exist , ensure materials of correct type and recvSelf on them
    ...
    for (int i = 0; i < 4; i++) {
      ...
      if (theMaterial[i]->getClassTag() != matClassTag) {
	     ...
	     theMaterial[i] = theBroker.getNewUniaxialMaterial(matClassTag);
	     if (theMaterial[i] == 0) {
            opserr << "PrestressConcretePlaneStress::recvSelf() - material " << i << "failed to create\n";
	        return -1;
		 }
	  }
...
int PrestressConcretePlaneStress::determineTrialStress(void)
{   
	// Define i, j, k
	int i, j, k;
	...
	*/
	...
	else // Tstrain[0] != Tstrain[1]
	{
		...
		{
			opserr << "PrestressConcretePlaneStress::determineTrialStress: Failure to calculate citaR\n";
	...
	{
    	opserr << "PrestressConcretePlaneStress::determineTrialStress: failure to get Hsu/Zhu ratio!\n";
	}
	...
	if ( v12*v21==1.0 )
	{
		opserr << "PrestressConcretePlaneStress::determineTrialStress: failure to get matrix [V]!\n";
	...
	else // Tstrain[0] != Tstrain[1]
	{
		...
		{
			opserr << "PrestressConcretePlaneStress::determineTrialStress: Failure to calculate principal stress direction\n";
