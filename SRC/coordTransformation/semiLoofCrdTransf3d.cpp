...
// $Source: /usr/local/cvs/OpenSees/SRC/coordTransformation/semiLoofCrdTransf3d.cpp,v $
...
// semiLoofCrdTransf3d class. semiLoofCrdTransf3d is a linear
...
#include <Node.h>
#include <Channel.h>
#include <semiLoofCrdTransf3d.h>

// initialize static variables
Matrix semiLoofCrdTransf3d::Tlg(12, 12);
Matrix semiLoofCrdTransf3d::kg(12, 12);

// constructor:
semiLoofCrdTransf3d::semiLoofCrdTransf3d(int tag, const Vector &vecInLocXZPlane):
CrdTransf(tag, CRDTR_TAG_semiLoofCrdTransf3d),
nodeIPtr(0), nodeJPtr(0), nodeKPtr(0),
nodeIOffset(0), nodeJOffset(0), L(0),
nodeIInitialDisp(0), nodeJInitialDisp(0), nodeKInitialDisp(0), initialDispChecked(false)
{
    for (int i = 0; i < 2; i++)
        for (int j = 0; j < 3; j++)
            R[i][j] = 0.0;
...
nodeIInitialDisp(0), nodeJInitialDisp(0), nodeKInitialDisp(0), initialDispChecked(false)
{
    for (int i = 0; i < 2; i++)
        ...
        if (&rigJntOffset1 == 0 || rigJntOffset1.Size() != 3 ) {
            opserr << "semiLoofCrdTransf3d::semiLoofCrdTransf3d:  Invalid rigid joint offset vector for node I\n";
        ...
        if (&rigJntOffset2 == 0 || rigJntOffset2.Size() != 3 ) {
            opserr << "semiLoofCrdTransf3d::semiLoofCrdTransf3d:  Invalid rigid joint offset vector for node J\n";
            opserr << "Size must be 3\n";      
        }
...
semiLoofCrdTransf3d::semiLoofCrdTransf3d():
CrdTransf(0, CRDTR_TAG_semiLoofCrdTransf3d),
nodeIPtr(0), nodeJPtr(0), nodeKPtr(0),
nodeIOffset(0), nodeJOffset(0), L(0),
nodeIInitialDisp(0), nodeJInitialDisp(0), nodeKInitialDisp(0), initialDispChecked(false)
{
...
int
semiLoofCrdTransf3d::commitState(void)
{
    return 0;
}
...
int
semiLoofCrdTransf3d::revertToLastCommit(void)
{
    return 0;
}
...
int
semiLoofCrdTransf3d::revertToStart(void)
{
    return 0;
}
...
int 
semiLoofCrdTransf3d::initialize(Node *nodeIPointer, Node *nodeJPointer)
{       
    int error;
    
    nodeIPtr = nodeIPointer;
    nodeJPtr = nodeJPointer;
//    nodeKPtr = nodeKPointer;

    if ((!nodeIPtr) || (!nodeJPtr)) // || (!nodeKPtr)
    {
        opserr << "\nsemiLoofCrdTransf3d::initialize";
...
int
semiLoofCrdTransf3d::update(void)
{
    return 0;
}
...
int 
semiLoofCrdTransf3d::computeElemtLengthAndOrient()
{
    // element projection
    static Vector dx(3);
    ...
    if (L == 0.0) {
        opserr << "\nsemiLoofCrdTransf3d::computeElemtLengthAndOrien: 0 length\n";
        return -2;  
    }
...
void
semiLoofCrdTransf3d::compTransfMatrixLocalGlobal(Matrix &Tlg)
{
...
int
semiLoofCrdTransf3d::getLocalAxes(Vector &XAxis, Vector &YAxis, Vector &ZAxis)
{
    // Compute y = v cross x
    ...
    if (ynorm == 0) {
        opserr << "\nsemiLoofCrdTransf3d::getLocalAxes";
...
double 
semiLoofCrdTransf3d::getInitialLength(void)
{
    return L;
}
...
double 
semiLoofCrdTransf3d::getDeformedLength(void)
{
    return L;
}
...
const Vector &
semiLoofCrdTransf3d::getBasicTrialDisp(void)
{
...
const Vector &
semiLoofCrdTransf3d::getBasicIncrDisp(void)
{
    // determine global displacements
...
const Vector &
semiLoofCrdTransf3d::getBasicIncrDeltaDisp(void)
{
    // determine global displacements
...
const Vector &
semiLoofCrdTransf3d::getBasicTrialVel(void)
{
...
const Vector &
semiLoofCrdTransf3d::getBasicTrialAccel(void)
{
...
const Vector &
semiLoofCrdTransf3d::getGlobalResistingForce(const Vector &pb, const Vector &p0)
{
...
const Matrix &
semiLoofCrdTransf3d::getGlobalStiffMatrix(const Matrix &KB, const Vector &pb)
{
...
const Matrix &
semiLoofCrdTransf3d::getInitialGlobalStiffMatrix (const Matrix &KB)
{
    static double kb[6][6];		// Basic stiffness
    static double kl[12][12];	// Local stiffness
    static double tmp[12][12];	// Temporary storage
...
CrdTransf *
semiLoofCrdTransf3d::getCopy3d(void)
{
    // create a new instance of semiLoofCrdTransf3d 
    
    semiLoofCrdTransf3d *theCopy;
    ...
    theCopy = new semiLoofCrdTransf3d(this->getTag(), xz, offsetI, offsetJ);
...
int 
semiLoofCrdTransf3d::sendSelf(int cTag, Channel &theChannel)
{
    int res = 0;
    ...
    if (res < 0) {
        opserr << "semiLoofCrdTransf3d::sendSelf - failed to send Vector\n";
...
int 
semiLoofCrdTransf3d::recvSelf(int cTag, Channel &theChannel, FEM_ObjectBroker &theBroker)
{
    int res = 0;
    ...
    if (res < 0) {
        opserr << "semiLoofCrdTransf3d::recvSelf - failed to receive Vector\n";
...
const Matrix &
semiLoofCrdTransf3d::getGlobalMatrixFromLocal(const Matrix &ml)
{
...
const Vector &
semiLoofCrdTransf3d::getPointGlobalCoordFromLocal(const Vector &xl)
{
    static Vector xg(3);
...
const Vector &
semiLoofCrdTransf3d::getPointGlobalDisplFromBasic(double xi, const Vector &uxb)
{
    // determine global displacements
    const Vector &disp1 = nodeIPtr->getTrialDisp();
    const Vector &disp2 = nodeJPtr->getTrialDisp();
...
void
semiLoofCrdTransf3d::Print(OPS_Stream &s, int flag)
{
    s << "\nCrdTransf: " << this->getTag() << " Type: semiLoofCrdTransf3d";
    if (nodeIOffset)
