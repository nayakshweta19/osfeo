/* ****************************************************************** **
**    OpenSees - Open System for Earthquake Engineering Simulation    **
**          Pacific Earthquake Engineering Research Center            **
**                                                                    **
**                                                                    **
** (C) Copyright 1999, The Regents of the University of California    **
** All Rights Reserved.                                               **
**                                                                    **
** Commercial use of this program without express permission of the   **
** University of California, Berkeley, is strictly prohibited.  See   **
** file 'COPYRIGHT'  in main directory for information on usage and   **
** redistribution,  and for a DISCLAIMER OF ALL WARRANTIES.           **
**                                                                    **
** Developed by:                                                      **
**   Frank McKenna (fmckenna@ce.berkeley.edu)                         **
**   Gregory L. Fenves (fenves@ce.berkeley.edu)                       **
**   Filip C. Filippou (filippou@ce.berkeley.edu)                     **
**                                                                    **
** ****************************************************************** */

// $Revision: 1.2 $
// $Date: 2003/02/14 23:00:41 $
// $Source: /usr/local/cvs/OpenSees/SRC/analysis/algorithm/eigenAlgo/RitzAlgorithm.h,v $


// File: ~/analysis/algorithm/eigenAlgo/RitzAlgorithm.h
//
// Written: Jun Peng
// Created: Wed Jan 27, 1999
// Revision: A
//
// Description: This file contains the class definition of RitzAlgorithm.
// RitzAlgorithm is a class which performs a eigen solution algorithm
// to solve the equations. 
//
// This class is inheritanted from the base class of SolutionAlgorithm
// which was created by fmk (Frank).


#ifndef RitzAlgorithm_h
#define RitzAlgorithm_h

#include <SolutionAlgorithm.h>
#include <OPS_Globals.h>

class AnalysisModel;
class RitzSOE;
class RitzIntegrator;

class RitzAlgorithm : public SolutionAlgorithm
{
public:
	RitzAlgorithm(int classTag);
	virtual ~RitzAlgorithm();

	// public functions defined for subclasses
	virtual void setLinks(AnalysisModel &theModel,
		RitzIntegrator &theIntegrator,
		RitzSOE &theSOE);

	// pure virtural functions
	virtual int solveCurrentStep(int numRitz) = 0;
	virtual void Print(OPS_Stream &s, int flag=0) = 0;

	virtual AnalysisModel *getAnalysisModelPtr() const;
	virtual RitzIntegrator *getRitzIntegratorPtr() const;
	virtual RitzSOE *getRitzSOEptr() const;

protected:

private:
	AnalysisModel  *theModel;
	RitzIntegrator *theIntegrator;
	RitzSOE *theSOE;

};

#endif
